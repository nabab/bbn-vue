...
(function($, bbn){
  ...
  Vue.component('bbn-tree2', {
    ...
    created(){
      let cp = bbn.vue.closest(this, 'bbn-tree2');
      if ( !cp ){
        this.isRoot = true;
        this.node = false;
        this.tree = this;
        ...
        while ( cp && cp.level ){
          cp = bbn.vue.closest(cp, 'bbn-tree2');
        }
        if ( cp && !cp.level ){
          this.tree = cp;
    ...
    components: {
      'bbn-tree-node': {
        ...
        created(){
          this.parent = bbn.vue.closest(this, 'bbn-tree2');
        ...
        watch: {
          ...
          filterString(newVal){
            ...
            if ( !newVal ){
              ...
              let ctrl = false,
                  childrens = this.findAll('bbn-tree-node');
              if ( this.isMatch ){
                let vm = this;
              ...
              else if ( !this.isMatch && this.excludedSectionFilter ){
                if ( childrens.length ){
                  ctrl = false;
                  childrens.forEach( node => {
                    if ( node.excludedFilter === true ){
                      ctrl = true;
                    }
                  });
                  if ( ctrl ){
