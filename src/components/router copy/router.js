    ...
    methods: {
      ...
      */
      ...
      getList(bc){
        ...
        let list = [],
            parents = bbn.fn.map(bc.itsMaster && (bc.baseURL !== bc.itsMaster.baseURL) ? bc.getParents() : [], p => p.views[p.selected]);
        bbn.fn.each(bc.views, (t, i) => {
          if ( !t.hidden && (t.idx !== bc.selected) ){
            list.push({
              text: t.title,
              icon: t.icon,
              key: t.url,
              ...
              static: t.static,
              parents: parents,
              children: bbn.fn.map(bc.getBreadcrumbs(i), c => c.views[c.selected]),
              action: () => {
                bc.activateIndex(t.idx);
              },
              closeAction: () => {
                bc.close(t.idx)
              }
