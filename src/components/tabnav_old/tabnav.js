  ...
  Vue.component("bbn-tabnav", {
    ...
    methods: {
      getTabColor(idx){
        ...
        if ( el && (!bbn.fn.isArray(el) || el.length) ){
...
// Particles Around the Parent
      ...
      load(url, force){
        ...
        return bbn.fn.post(finalURL, {_bbn_baseURL: vm.fullBaseURL}, (d) => {
          ...
          let idx = vm.search(url);
          this.close(idx);
          appui.notify(bbn._("The request has been aborted"));
        })
      },
      ...
      getMenuFn(idx){
        let items = [],
            tmp = ((bbn.fn.isFunction(this.tabs[idx].menu) ? this.tabs[idx].menu() : this.tabs[idx].menu) || []).slice(),
            others = false;
        $.each(this.tabs, (i, a) => {
          if ( (i !== idx) && !a.static ){
            others = true;
        ...
        items.push({
          text: bbn._("Enlarge"),
          key: "enlarge",
          icon: "nf nf-fa-expand_arrows_alt",
          command: () => {
        ...
        mounted(){
          ...
          if ( this.imessages.length ){
            this.getPopup().open({
              ...
              title: '<i class="w3-large nf nf-fa-envelope"> </i> <span class="bbn-iblock">' + bbn._('Internal message') + '</span>',
              width: '90%',
              height: '90%'
            });
          }
