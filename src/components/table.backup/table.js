    ...
    methods: {
      ...
      _delete(index){
        if ( this.currentSet[index] && this.currentData[this.currentSet[index].index] ){
          ...
          if ( this.url ){
            bbn.fn.post(this.url, bbn.fn.extend({}, this.currentData[index], {action: 'delete'}), (d) => {
              if ( d.success ){
      ...
      openColumnsPicker(){
        ...
        this.getPopup().open({
          ...
          component: {
            template: `
<div class="bbn-table-column-picker bbn-overlay">
...
`,
      ...
      edit(row, winOptions, index){
        ...
        if ( this.editMode === 'popup' ){
          ...
          let popup = bbn.fn.extend({title: bbn._('Row edition'), width: 700}, winOptions ? winOptions : {}, {
            source: {
              row: row,
          ...
          // A URL is given and in this case the form will be created automatically with this URL as action
            ...
            let table = this;
            let o = bbn.fn.extend({}, this.data, {action: table.tmpRow ? 'insert' : 'update'});
            popup.component = {
              data(){
                let fields = [];
                table.cols.map((a) => {
...
<bbn-form class="bbn-overlay"
      ...
      saveInline(){
        if ( this.tmpRow || this.editedRow ){
          if ( this.url ){
            let o = bbn.fn.extend({}, this.data, this.tmpRow || this.editedRow, {action: this.tmpRow ? 'insert' : 'update'});
        ...
        if ( this.isAjax && !this.isLoading ){
          ...
          this.$nextTick(() => {
            let data = {
              limit: this.currentLimit,
              start: this.start,
              data: this.data ? (bbn.fn.isFunction(this.data) ? this.data() : this.data) : {}
            };
            if ( this.sortable ){
        ...
        if ( col.type ){
          switch ( col.type ){
            case "date":
              break;
            case "email":
              bbn.fn.extend(res, {type: 'email'});
              break;
            case "url":
              bbn.fn.extend(res, {type: 'url'});
              break;
            case "number":
              break;
            case "money":
            ...
            case "bool":
            case "boolean":
              bbn.fn.extend(res, {value: 1, novalue: 0});
              break;
          }
        }
        if ( col.source ){
          bbn.fn.extend(res, {source: col.source});
        }
        else if ( col.editor ){
          res.source = data;
        }
