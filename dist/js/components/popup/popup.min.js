...
if(d){if(d.url){return this.load(d);}
if(!d.uid){d.uid='bbn-popup-'+bbn.fn.timestamp().toString()+'-'+bbn.fn.randomString(4,6);}
d.index=this.items.length;this.setResizeMeasures();this.setContainerMeasures();this.items.push(d);return d.uid;}
...
d.index=this.items.length;this.setResizeMeasures();this.setContainerMeasures();this.items.push(d);this.makeWindows();}})}
...
if(a.draggable===undefined){a.draggable=true;}
...
let win=this.getWindow(idx);if(win&&!force){return win.close(idx);}
if(this.items[idx]){this.items.splice(idx,1);this.$forceUpdate();}},getIndexByUID(uid){return bbn.fn.search(this.items,{uid:uid});},alert(){let has_msg=false;let has_title=false;let has_width=false;let has_callback=false;let okText;let onOpen;let onClose;let o={};for(let i=0;i<arguments.length;i++){if(!has_msg&&(typeof(arguments[i])==='string')){o.content=arguments[i];has_msg=1;}
...
else if(bbn.fn.isVue(arguments[i])){o.opener=arguments[i];}
...
o.content='<div class="'+(this.isMobile||this.isTablet?'bbn-padded':'bbn-lpadded')+' bbn-large bbn-c" style="min-width: '+(this.isMobile||this.isTablet?'15':'30')+'em">'+o.content+'</div>';o.buttons=[{text:okText,cls:'bbn-primary',icon:'nf nf-fa-check_circle',focused:true,action($ev,btn){if(onClose){onClose($ev,btn);}
btn.closest('bbn-floater').close(true);}}];this.open(bbn.fn.extend(o,{maximizable:false,scrollable:false,resizable:false}));}},confirm(){let onYes=false;let onNo=false;let yesText=bbn._('Yes');let noText=bbn._('No');let o={};let options={};let has_msg=false;let has_yes=false;let has_width=false;let i;if(bbn.fn.isObject(arguments[0])){o=arguments[0];}
...
if(!o.title){o.title=false;}
o.content='<div class="'+(this.isMobile||this.isTablet?'bbn-padded':'bbn-lpadded')+' bbn-large bbn-c" style="min-width: '+(this.isMobile||this.isTablet?'15':'30')+'em">'+o.content+'</div>';o.buttons=[{text:noText,icon:'nf nf-fa-times_circle',focused:true,action($ev,btn){btn.closest('bbn-floater').close(true);if(onNo){setTimeout(()=>{onNo($ev,btn);},0);}}},{text:yesText,cls:'bbn-primary',icon:'nf nf-fa-check_circle',action($ev,btn){btn.closest('bbn-floater').close(true);setTimeout(()=>{onYes($ev,btn);},0);}}];this.open(bbn.fn.extend(o,options,{resizable:false,maximizable:false,scrollable:false}));}},makeWindows(){this.$forceUpdate();},getWindow(idx){if(this.popups.length){if(idx===undefined){idx=this.popups.length-1;}
if(this.popups[idx]){return bbn.vue.getChildByKey(this,this.popups[idx].uid);}}
return false;}},created(){this.componentClass.push('bbn-resize-emitter');},mounted(){this.onResize();bbn.fn.each(this.popups,a=>this.open(a));},watch:{items(){this.makeWindows();},numPopups(v){if(v&&!this.ready){this.ready=true;}}}});})(bbn);if(bbn_resolve){bbn_resolve("ok");}})(bbn);}