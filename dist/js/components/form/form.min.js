...
r.push(a);}});}
return r;},_post(){this.isPosted=true;this.isLoading=true;if(this.action){this[this.blank||this.self||this.target?'postOut':'post'](this.action,bbn.fn.extend(true,{},this.data||{},this.source||{}),d=>{this.originalData=bbn.fn.clone(this.source||{});if(this.successMessage&&p){p.alert(this.successMessage);bbn.fn.info(this.successMessage,p);}
let e=new Event('success',{cancelable:true});this.dirty=false;this.isLoading=false;if(d&&(d.success===false)){}
else{this.$emit('success',d,e);if(!e.defaultPrevented&&this.window){this.$nextTick(()=>{this.window.close(true);});}}},!this.blank&&!this.self&&!this.target?(xhr,textStatus,errorThrown)=>{this.$emit('failure',xhr,textStatus,errorThrown);this.isLoading=false;}:(this.self?'_self':(this.blank?'_blank':this.target)));}
