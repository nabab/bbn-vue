...
else if(d&&d.success){this.$emit('success',d,e);if(!e.defaultPrevented&&this.window){this.$nextTick(()=>{this.window.close(true);});}}},!this.blank&&!this.self&&!this.target?(xhr,textStatus,errorThrown)=>{this.$emit('failure',xhr,textStatus,errorThrown);this.isLoading=false;}:(this.self?'_self':(this.blank?'_blank':this.target)));}
