...
this.dirty=false;this.isLoading=false;}
if(this.window){this.$nextTick(()=>{this.window.close(true);});}}},!this.blank&&!this.self&&!this.target?(xhr,textStatus,errorThrown)=>{this.$emit('failure',xhr,textStatus,errorThrown);this.isLoading=false;}:(this.self?'_self':(this.blank?'_blank':this.target)));}
...
if(this.window&&bbn.fn.isArray(this.window.currentButtons)&&(this.mode==='big')){this.window.currentButtons.splice(0,this.window.currentButtons.length);}
bbn.fn.each(this.getRealButtons(),b=>{this.realButtons.push(b);if(this.window&&bbn.fn.isArray(this.window.currentButtons)&&(this.mode==='big')){this.window.currentButtons.push(b);}});},getModifications(){let data=this.getData(this.$el)||{},res={};for(let n in data){if((this.sendModel&&(data[n]!==this.originalData[n]))||(!this.sendModel&&(data[n]!=this.originalData[n]))){res[n]=data[n];}}
...
if(this.tab){this.tab.dirty=v;}}}});})(bbn);if(bbn_resolve){bbn_resolve("ok");}})(bbn);}