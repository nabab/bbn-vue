...
this.dirty=false;this.isLoading=false;this.$emit('success',d,e);if(!e.defaultPrevented){if(this.window){this.window.close();}}},!this.blank&&!this.self&&!this.target?(xhr,textStatus,errorThrown)=>{this.$emit('failure',xhr,textStatus,errorThrown);this.isLoading=false;}:(this.self?'_self':(this.blank?'_blank':this.target)));}
...
this.dirty=false;this.isLoading=false;if(!e.defaultPrevented){if(this.window){this.window.close();}}}},_execCommand(button,ev){if(button.action){button.action(this.source,this,ev)}},updateButtons(){if(this.realButtons.length){this.realButtons.splice(0,this.realButtons.length);}
...
if(!force){let ev=new Event('submit',{cancelable:true});this.$emit('submit',ev,this);if(ev.defaultPrevented){return false;}}
