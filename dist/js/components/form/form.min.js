...
r.push(a);}});}
return r;},_post(){this.isPosted=true;this.isLoading=true;if(this.action&&!this.target){let data=bbn.fn.extend(true,{},this.data||{},this.source||{});let method=this.blank||this.self||this.target?'postOut':'post';this[method](this.action,data,d=>{this.originalData=bbn.fn.clone(this.source||{});if(this.successMessage&&p){p.alert(this.successMessage);bbn.fn.info(this.successMessage,p);}
let e=new Event('success',{cancelable:true});this.dirty=false;this.isLoading=false;if(d&&(d.success===false)){}
else if(d){this.$emit('success',d,e);if(!e.defaultPrevented&&this.window){this.$nextTick(()=>{this.window.close(true);});}}},!this.blank&&!this.self&&!this.target?(xhr,textStatus,errorThrown)=>{this.$emit('failure',xhr,textStatus,errorThrown);this.isLoading=false;}:(this.self?'_self':(this.blank?'_blank':this.target)));}
...
this.confirm(this.confirmLeave,()=>{this.reinit();this.$nextTick(()=>{if(this.window){this.window.close(true,true);this.isClosing=false;}});});}
