...
</div>`;script.setAttribute('id','bbn-tpl-component-values');script.setAttribute('type','text/x-template');document.body.insertAdjacentElement('beforeend',script);let css=document.createElement('link');css.setAttribute('rel',"stylesheet");css.setAttribute('href',bbn.vue.libURL+"dist/js/components/values/values.css");document.head.insertAdjacentElement('beforeend',css);((bbn)=>{"use strict";Vue.component('bbn-values',{mixins:[bbn.vue.basicComponent,bbn.vue.inputComponent,],props:{max:{type:Number},min:{type:Number},validator:{type:[String,Function]},json:{type:Boolean}},data(){let isJSON=bbn.fn.isString(this.value);return{isJSON:isJSON,obj:isJSON?JSON.parse(this.value):this.value,currentKey:'',currentValue:'',items:[]};},methods:{setItems(){this.items.splice(0,this.items.length);bbn.fn.iterate(this.obj,(o,n)=>{this.items.push({key:n,value:o});});},update(){this.setItems();this.$forceUpdate();},remove(key){delete this.obj[key];if(this.isJSON){this.emitInput(JSON.stringify(this.obj)||'{}');}
