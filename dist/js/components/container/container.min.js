...
this.ready=true;}},showMenu(){return this.router.getMenuFn(this.currentIndex);},setScreenshot(){setTimeout(()=>{if(this.currentIndex===this.router.selected){this.takeScreenshot();}
this._screenshotInterval=setInterval(()=>{this.takeScreenshot();},300000);},3000);},unsetScreenshot(){if(this._screenshotInterval){clearInterval(this._screenshotInterval)
this._screenshotInterval=false;}},takeScreenshot(num_tries=0){if((this.currentIndex===this.router.selected)&&this.router.db&&window.html2canvas&&document.hasFocus()){let scroll=this.getRef('scroll');if(!scroll){if(num_tries<=10){setTimeout(()=>{this.takeScreenshot(num_tries+1);},1000);}
return;}
...
this.$nextTick(()=>{this.selfEmit(true)})},dirty(v){let view=this.router.getView(this.url);if(view){view.dirty=v;this.router.retrieveDirtyContainers();}},isInit(v){if(!v){this.isLoaded=!this.load||this.loaded;this.dirty=false;}}},});})(bbn,Vue);if(bbn_resolve){bbn_resolve("ok");}})(bbn);}