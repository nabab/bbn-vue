...
if(this.visual){this.takeScreenshot();}},2000);this.ready=true;}},showMenu(){return this.router.getMenuFn(this.idx);},takeScreenshot(num_tries=0){if(this.visual&&this.router.db&&window.html2canvas){setTimeout(()=>{let prom;let scroll=this.getRef('scroll');if(!scroll){if(num_tries<=10){this.takeScreenshot(num_tries+1);}
return;}
...
catch(e){bbn.fn.log("Error");ct.style.width=null;ct.style.height=null;return;}
prom.then(canvas=>{ct.style.width=null;ct.style.height=null;let img=bbn.fn.canvasToImage(canvas);if(!img){bbn.fn.log("Error for screenshot image");return;}
let ctx=canvas.getContext('2d');let size=Math.min(w,h);let num=Math.min(this.router.numVisualCols,this.router.numVisualRows);let msize=Math.ceil(size / num);ctx.drawImage(img,0,0,size,size,0,0,msize,msize);this.router.db.insert('containers',{url:this.getFullURL(),image:img.src});}).catch(e=>{bbn.fn.log("ERROR",e);});},1000)}},},created(){this.componentClass.push('bbn-resize-emitter');if(this.isComponent){componentsList.push(this.componentName);}
...
else{this.router.register(this);}},beforeDestroy(){this.router.unregister(this);if(this.isComponent){let idx=componentsList.indexOf(this.componentName);if(idx>-1){componentsList.splice(idx,1);}}},watch:{loaded(v){this.isLoaded=v;},loading(v){this.isLoading=v;},selected(v){this.visible=v;},idx(){if(this.visual){this.isOver=false;}},current(newVal){if(newVal.indexOf(this.url)===0){this.currentURL=newVal;}},currentURL(newVal,oldVal){if(!newVal||(newVal.indexOf(this.url)!==0)){this.currentURL=this.url;}},ready(v){if(v){if(this.onMount){this.onMount(this.$el,this.source);}}},load(nv,ov){},visible(nv,ov){this.$nextTick(()=>{this.$emit(nv?'view':'unview',this);if(nv){this.$nextTick(()=>{this.onResize();});}
...
this.$nextTick(()=>{this.selfEmit(true)})},dirty(v){this.router.views[this.idx].dirty=v;this.router.retrieveDirtyContainers();}},});})(bbn,Vue);if(bbn_resolve){bbn_resolve("ok");}})(bbn);}