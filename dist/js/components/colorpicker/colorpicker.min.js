(bbn_resolve)=>{((bbn)=>{let script_dep=document.createElement('script');script_dep.setAttribute('src','https://cdn.jsdelivr.net/combine/gh/jaames/iro.js@5.0.0/dist/iro.js');script_dep.onload=()=>{let script=document.createElement('script');script.innerHTML=`<span :class="['bbn-iblock', componentClass, {'bbn-disabled': !!isDisabled}]">
  <input :value="value"
...
`;script.setAttribute('id','bbn-tpl-component-colorpicker');script.setAttribute('type','text/x-template');document.body.insertAdjacentElement('beforeend',script);let css=document.createElement('link');css.setAttribute('rel','stylesheet');css.setAttribute('href',bbn.vue.libURL+'dist/js/components/colorpicker/colorpicker.css');document.head.insertAdjacentElement('beforeend',css);(bbn=>{"use strict";Vue.component('bbn-colorpicker',{mixins:[bbn.vue.basicComponent,bbn.vue.inputComponent,bbn.vue.eventsComponent,bbn.vue.resizerComponent],props:{value:{type:String,},wheel:{type:Boolean,default:true},slider:{tyoe:Boolean,default:false},brightness:{type:Boolean,default:true},saturation:{type:Boolean,default:true},onlyPalette:{type:Boolean,default:false},color:{type:String,default:'#FDFDFD'},showCodes:{type:Boolean,default:true},palette:{type:Array,default(){return bbn.var.colors?Object.values(bbn.var.colors):[]}},emptyButton:{type:Boolean,default:true},codeColor:{type:String,default:"hex",validator:c=>['hex','rgb','rgba','hsl'].includes(c)},autoSelect:{type:Boolean,default:false}},data(){return{widget:false,showFloater:false,showPalette:this.onlyPalette||this.palette.length,currentValue:this.value,currentHex:'',currentRgb:'',currentRgba:'',currentHsl:'',initTimeout:false}},computed:{currentCfg(){let obj={handleRadius:6,color:this.value||this.color,layout:[]};if(this.wheel&&!this.slider&&!this.onlyPalette){obj.layout.push({component:iro.ui.Wheel});}
...
this.showFloater=false},setColor(color){this.widget.color.set(color);if(this.autoSelect){this.$nextTick(()=>{this.save();})}},fromInput(event){this.setColor(event.target.value);},openCloseFloater(){if(this.showFloater){this.showFloater=false;}
else if(!this.isDisabled&&!this.readonly){this.showFloater=true;}}},mounted(){this.ready=true;},beforeDestroy(){this.destroy();}});})(bbn);if(bbn_resolve){bbn_resolve("ok");}};document.head.insertAdjacentElement("beforeend",script_dep);})(bbn);}