(bbn_resolve)=>{((bbn)=>{let script=document.createElement('script');script.innerHTML=`<div :class="[{'bbn-overlay': true}, componentClass]"
     :style="{
...
</div>`;script.setAttribute('id','bbn-tpl-component-splitter');script.setAttribute('type','text/x-template');document.body.insertAdjacentElement('beforeend',script);let css=document.createElement('link');css.setAttribute('rel',"stylesheet");css.setAttribute('href',bbn.vue.libURL+"dist/js/components/splitter/splitter.css");document.head.insertAdjacentElement('beforeend',css);(function(bbn){"use strict";Vue.component('bbn-splitter',{mixins:[bbn.vue.basicComponent,bbn.vue.resizerComponent],props:{orientation:{type:String,default:'auto'},resizable:{type:Boolean,default:false},collapsible:{type:Boolean,default:false},scrollable:{type:Boolean,default:false},resizerSize:{type:Number,default:15},resizerClass:{type:[String,Function]},minPaneSize:{type:Number,default:40}},data(){return{initTimeout:0,isResized:false,isResizing:false,resizeCfg:null,resizers:[],currentOrientation:this.orientation,panes:[]};},computed:{isResizable(){return(this.resizable||this.collapsible)&&(bbn.fn.count(this.panes,{resizable:true})>=2)},columnsCfg(){return this.panes.length&&(this.currentOrientation==='horizontal')?this.getFormatted():'auto';},rowsCfg(){return this.panes.length&&(this.currentOrientation==='vertical')?this.getFormatted():'auto';},currentAxis(){return this.currentOrientation==='horizontal'?'x':'y'},currentSizeType(){return this.currentOrientation==='horizontal'?'Width':'Height';},currentOffsetType(){return this.currentOrientation==='horizontal'?'left':'top';},currentSize(){return this['lastKnown'+this.currentSizeType];},availableSize(){let availableSize=this.currentSize;bbn.fn.each(this.resizers,()=>{availableSize-=this.resizerSize;});return availableSize;},},methods:{getFormatted(){let pos=1,lastVisibleResizer=false,tmp=this.panes.map((a)=>{let diff=a.currentDiff+a.savedDiff+a.tmpDiff,sz='';while(a.position>pos){sz+=lastVisibleResizer?'0 ':this.resizerSize+'px ';lastVisibleResizer=true;pos++;}
if(a.collapsed){sz+='0 ';}
...
return this.resizerClass||'';},getOrientation(){return this.lastKnownCtWidth>this.lastKnownCtHeight?'horizontal':'vertical';},onResize(){if(!this.isResizing){this.isResizing=true;this.setContainerMeasures();this.setResizeMeasures();if(this.orientation==='auto'){let o=this.getOrientation();if(o!==this.currentOrientation){this.currentOrientation=o;}}
...
else if((typeof props.size==='string')&&(props.size.substr(-1)==='%')){isPercent=true;hasPercent=true;}
else if((typeof props.size==='string')&&(props.size.substr(-2)==='px')){isNumber=true;props.size=parseInt(props.size);}
