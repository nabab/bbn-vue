(bbn_resolve)=>{((bbn)=>{let script=document.createElement('script');script.innerHTML=`<div v-if="ready && (numPages || extraControls)"
...
</div>`;script.setAttribute('id','bbn-tpl-component-pager');script.setAttribute('type','text/x-template');document.body.insertAdjacentElement('beforeend',script);let css=document.createElement('link');css.setAttribute('rel','stylesheet');css.setAttribute('href',bbn.vue.libURL+'dist/js/components/pager/pager.css');document.head.insertAdjacentElement('beforeend',css);(bbn=>{"use strict";Vue.component('bbn-pager',{mixins:[bbn.vue.basicComponent],props:{element:{type:Vue,required:true},buttons:{type:Boolean,default:true},forceMobile:{type:Boolean,default:false},forceTablet:{type:Boolean,default:false},pageName:{type:String,default:bbn._("page")},itemName:{type:String,default:bbn._("records")},extraControls:{type:Boolean,default:true},limit:{type:Boolean,default:true},numericSelector:{type:Boolean,default:true}},data(){return{numericTimeout:false,currentNumericPage:this.element.currentPage,numPages:this.element.numPages}},computed:{currentPage:{get(){return this.element.currentPage;},set(v){this.element.currentPage=v;}}},methods:{updatePager(){bbn.fn.log('update pager');this.currentNumericPage=this.element.currentPage;this.numPages=this.element.numPages;bbn.fn.log('update pager',this.numPages);},firstPage(){if(this.element&&('currentPage'in this.element)&&(this.element.currentPage!==1)){this.element.currentPage=1;}},nextPage(){if(this.element&&('currentPage'in this.element)&&('numPages'in this.element)&&(this.element.currentPage<this.element.numPages)){this.element.currentPage++;}},prevPage(){if(this.element&&('currentPage'in this.element)&&(this.element.currentPage>1)){this.element.currentPage--;}},lastPage(){if(this.element&&('currentPage'in this.element)&&('numPages'in this.element)&&(this.element.currentPage!==this.element.numPages)){this.element.currentPage=this.element.numPages;}}},created(){if(this.forceMobile){this.isMobile=true;}
if(this.forceTablet){this.isTablet=true;}},mounted(){if(this.element){if(this.element.ready&&!this.ready){this.ready=true;}
else{this.element.$on('ready',()=>{this.ready=true;})}
this.element.$on('dataloaded',this.updatePager);}},beforeDestroy(){this.element.$off('dataloaded');},watch:{currentPage(v){if(this.currentNumericPage!==v){this.currentNumericPage=v;}},currentNumericPage(v){if(this.numericTimeout){clearTimeout(this.numericTimeout);}
this.numericTimeout=setTimeout(()=>{if(this.currentPage!==v){this.currentPage=v;}},500);}}});})(bbn);if(bbn_resolve){bbn_resolve("ok");}})(bbn);}