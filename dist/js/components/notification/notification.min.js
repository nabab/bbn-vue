...
</div>`;script.setAttribute('id','bbn-tpl-component-notification');script.setAttribute('type','text/x-template');document.body.insertAdjacentElement('beforeend',script);let css=document.createElement('link');css.setAttribute('rel',"stylesheet");css.setAttribute('href',bbn.vue.libURL+"dist/js/components/notification/notification.css");document.head.insertAdjacentElement('beforeend',css);((bbn)=>{"use strict";Vue.component('bbn-notification',{mixins:[bbn.vue.basicComponent],props:{delay:{type:Number,default:5000},pinned:{},position:{type:String,default:'bottom-left'},successMessage:{type:[String,Function],default:bbn._('Success')},warningMessage:{type:[String,Function],default:bbn._('Warning')},errorMessage:{type:[String,Function],default:bbn._('Error')},infoMessage:{type:[String,Function],default:bbn._('Info')},successIcon:{type:[String,Boolean],default:'nf nf-fa-check_square'},warningIcon:{type:[String,Boolean],default:'nf nf-fa-warning'},errorIcon:{type:[String,Boolean],default:'nf nf-fa-exclamation_circle'},infoIcon:{type:[String,Boolean],default:'nf nf-mdi-information'}},data:function(){let bits=this.position.split('-');let pos={v:{top:false,bottom:true},h:{left:false,right:true}};bbn.fn.each(bits,(bit)=>{bbn.fn.iterate(pos,(o,dir)=>{if(o[bit.toLowerCase()]){bbn.fn.iterate(o,(b,k)=>{if(bit===k){pos[dir][k]=true;}
else{pos[dir][k]=false;}});}});});return{items:[],isTop:pos.v.top,isLeft:pos.h.left,positions:{}};},methods:{_sanitize(obj,type,timeout){if(!bbn.fn.isObject(obj)||(!obj.id)){if(typeof obj==='string'){obj={content:obj};}
else if(!obj){obj={};}
...
o=this._sanitize(o,'success',timeout);this.add(o);},error(o,timeout){o=this._sanitize(o,'error',timeout);this.add(o);},warning(o,timeout){o=this._sanitize(o,'warning',timeout);this.add(o);},show(o,type,timeout){o=this._sanitize(o,type,timeout);this.add(o);},info(o,timeout){o=this._sanitize(o,'info',timeout);this.add(o);},},beforeMount(){this._updatePositions();}});})(window.bbn);if(bbn_resolve){bbn_resolve("ok");}})(bbn);}