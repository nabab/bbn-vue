(bbn_resolve)=>{((bbn)=>{let script_dep=document.createElement('script');script_dep.setAttribute('src','https://cdn.jsdelivr.net/combine/gh/ajaxorg/ace-builds@v1.4.12/src-min-noconflict/ace.js');script_dep.onload=()=>{let script=document.createElement('script');script.innerHTML=`<div :class="[{'bbn-reset': true, 'bbn-overlay': isFullScreen}, componentClass]"
     @keydown.enter.stop=""
...
`;script.setAttribute('id','bbn-tpl-component-code3');script.setAttribute('type','text/x-template');document.body.insertAdjacentElement('beforeend',script);let css=document.createElement('link');css.setAttribute('rel','stylesheet');css.setAttribute('href',bbn.vue.libURL+'dist/js/components/code3/code3.css');document.head.insertAdjacentElement('beforeend',css);(bbn=>{"use strict";Vue.component('bbn-code3',{mixins:[bbn.vue.basicComponent,bbn.vue.inputComponent,bbn.vue.eventsComponent],props:{ecma:{type:Number,default:6},mode:{type:[String,Object],default:'php'},theme:{type:String,},fill:{type:Boolean,default:true},cfg:{type:Object,default:function(){return{};}},themeButton:{type:Boolean,default:false}},data(){return{widget:null,isFullScreen:false};},computed:{currentTheme(){return this.theme?this.theme:this.defaultTheme;}},methods:{getMode(mode){if(modes[mode]){let o=bbn.fn.clone(modes[mode]);o.gutters=["CodeMirror-linenumbers","CodeMirror-foldgutter"];if(o.lint){o.gutters=["CodeMirror-lint-markers"]}
return o;}
return false;},getOptions(){let tmp,cfg=bbn.fn.extend({},baseCfg,{mode:this.mode,theme:this.currentTheme,value:this.value},this.cfg);if(this.readonly||this.isDisabled){cfg.readOnly=true;}
