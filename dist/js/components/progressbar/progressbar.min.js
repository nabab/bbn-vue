...
return st;},},beforeMount(){if(this.type==='percent'){this.unit='%';this.percent=this.value;}
...
if((this.orientation==='horizontal')&&(this.width)){st+='height: 1.9em; min-width: '+(bbn.fn.isNumber(this.width)?(this.width+'px'):this.width);}
this.orientationStyle=st+=';';if(this.type==='chunk'){this.chunknumber=(this.max-this.min)/ this.step,this.selectedChunks=(this.value / 100)*this.chunknumber;}
...
else{this.chunkStyle+='grid-template-rows:repeat('+this.chunknumber+',1fr);grid-template-columns: 1fr; min-height: '+this.height+'px';}}}});})(bbn);if(bbn_resolve){bbn_resolve("ok");}})(bbn);}