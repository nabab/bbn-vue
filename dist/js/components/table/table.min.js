...
return false}},methods:{_defaultRow(initialData){let res={},data=initialData?bbn.fn.clone(initialData):{};bbn.fn.each(this.cols,function(a){if(a.field){if(data[a.field]!==undefined){res[a.field]=data[a.field];}
...
if(this.sortable&&(this.currentOrder!==cfg.order)){this.currentOrder=cfg.order;}
...
if(col.minWidth&&(tmp<col.minWidth)){tmp=col.minWidth;}
if(col.maxWidth&&(tmp>col.maxWidth)){tmp=col.maxWidth;}
col.realWidth=tmp;}
...
if(a.width){if(bbn.fn.isString(a.width)&&(a.width.substr(-1)==='%')){a.realWidth=Math.floor(parentWidth*parseFloat(a.width)/ 100);if(a.realWidth<(bbn.fn.isMobile()?this.minimumColumnWidthMobile:this.minimumColumnWidth)){a.realWidth=bbn.fn.isMobile()?this.minimumColumnWidthMobile:this.minimumColumnWidth;}}
else{a.realWidth=parseFloat(a.width);}}
else{a.realWidth=bbn.fn.isMobile()?this.minimumColumnWidthMobile:this.minimumColumnWidth;numUnknown++;}
if(a.minWidth&&(a.realWidth<a.minWidth)){a.realWidth=a.minWidth;}
if(a.maxWidth&&(a.realWidth>a.maxWidth)){a.realWidth=a.maxWidth;}
...
else{this.$once('dataloaded',()=>{this.ready=true;});this.$nextTick(()=>{this.init(!!this.isAutobind);})}},watch:{observerDirty(v){if(v&&!this.editedRow){this.observerDirty=false;this.updateData();}},allRowsChecked(v){if(v){this.checkAll();}
else{this.uncheckAll();}},currentHidden:{deep:true,handler(){if(this.ready){this.setConfig(true);this.$forceUpdate();}}},group(){this.currentExpandedValues=[];this.currentExpanded=[];this.init();},focusedRow(newIndex,oldIndex){if(bbn.fn.isNumber(oldIndex)){this.$emit('focusout',oldIndex,this.items[oldIndex]?this.items[oldIndex].index:undefined);}
