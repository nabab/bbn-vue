(bbn_resolve)=>{((bbn)=>{let script=document.createElement('script');script.innerHTML=`<div :class="[{'bbn-overlay': scrollable, 'bbn-block': !scrollable}, componentClass, 'bbn-bordered']">
...
`;script.setAttribute('id','bbn-tpl-component-table');script.setAttribute('type','text/x-template');document.body.insertAdjacentElement('beforeend',script);let css=document.createElement('link');css.setAttribute('rel','stylesheet');css.setAttribute('href',bbn.vue.libURL+'dist/js/components/table/table.css');document.head.insertAdjacentElement('beforeend',css);(function(bbn,Vue){"use strict";Vue.component('bbn-table',{mixins:[bbn.vue.basicComponent,bbn.vue.resizerComponent,bbn.vue.editableListComponent,bbn.vue.listComponent,bbn.vue.dataEditorComponent,bbn.vue.localStorageComponent,bbn.vue.observerComponent,bbn.vue.keepCoolComponent,bbn.vue.dataComponent],props:{titles:{type:Boolean,default:true},noData:{default:'<h3>'+bbn._('No Data')+'</h3>'},titleGroups:{type:[Array,Function]},scrollable:{type:Boolean,default:true},resizable:{type:Boolean,default:false},showable:{type:Boolean,default:false},saveable:{type:Boolean,default:false},groupable:{type:Boolean,default:false},serverGrouping:{type:Boolean,default:true},localGrouping:{type:Boolean,default:true},minimumColumnWidth:{type:Number,default:30},minimumColumnWidthMobile:{type:Number,default:100},defaultColumnWidth:{type:Number,default:150},trClass:{type:[String,Function,Object]},trStyle:{type:[String,Function,Object]},confirmMessage:{type:[String,Function,Boolean],default:bbn._('Are you sure you want to delete this row?')},expander:{type:[Object,String,Function]},loader:{type:[String,Boolean],default:true},fixedDefaultSide:{type:String,default:'left'},toolbar:{type:[Array,Object,String,Function]},columns:{type:Array,default:function(){return[];}},groupBy:{type:Number},expandedValues:{type:[Array,Function]},expanded:{type:[Boolean,Array],default(){return[];}},footer:{type:[String,Object,Boolean,Function],default:true},groupFooter:{type:[String,Object,Function]},aggregateExp:{type:Object,default(){return{tot:bbn._('Total'),med:bbn._('Average'),num:bbn._('Count'),max:bbn._('Maximum'),min:bbn._('Minimum'),};}},aggregate:{type:[String,Array]},loadedConfig:{type:Object},footerButtons:{type:Boolean,default(){return!bbn.fn.isMobile()||bbn.fn.isTabletDevice();}},pageName:{type:String,default:bbn._("page")},itemName:{type:String,default:bbn._("rows")},buttonMode:{type:String,default:'buttons',validator(v){return['buttons','dropdown','menu'].includes(v);}},buttonIcon:{type:String,default:'nf nf-mdi-dots_vertical'},zoomable:{type:Boolean,default:false},maxRowHeight:{type:Number},toolbarSlotBefore:{type:Boolean,default:true}},data(){return{_observerReceived:false,allRowsChecked:false,groupCols:[{name:'left',width:0,visible:0,cols:[]},{name:'main',width:0,visible:0,cols:[]},{name:'right',width:0,visible:0,cols:[]}],initReady:false,currentConfig:{},savedConfig:false,defaultConfig:bbn.fn.extend({filters:this.filters,limit:this.limit,order:this.order,hidden:this.hidden||null,},this.loadedConfig||{}),editedFilter:false,floatingFilterX:0,floatingFilterY:0,floatingFilterTimeOut:0,currentHidden:this.hidden||[],group:this.groupBy===undefined?false:this.groupBy,cols:[],table:false,colButtons:false,scrollableContainer:null,hiddenScroll:true,popups:[],isAggregated:false,aggregatedColumns:[],updaterTimeout:false,allExpanded:this.expanded===true?true:false,groupInit:false,currentExpanded:Array.isArray(this.expanded)?this.expanded:[],currentExpandedValues:Array.isArray(this.expandedValues)?this.expandedValues:[],focusedRow:false,rowIndexTimeOut:null,containerPadding:(bbn.fn.getScrollBarSize()?bbn.fn.getScrollBarSize():'0')+'px',container:null,currentScrollTop:0,marginStyleSheet:null,cssRuleName:bbn.fn.randomString().toLowerCase(),initStarted:false,inTable:null,filterElement:null,hasScrollX:false,hasScrollY:false,borderLeft:0,borderRight:0,focusedElement:undefined,focusedElementX:0,focusedElementY:0,isTableDataUpdating:false,currentLoaderText:bbn.fn.isString(this.loader)?this.loader:bbn._('Loading')+'...',isResizingWidth:false,portalElement:document.body};},computed:{realButtons(){if(this.cols.length&&this.cols[this.colButtons]&&this.cols[this.colButtons].buttons){if(bbn.fn.isFunction(this.cols[this.colButtons].buttons)){return this.cols[this.colButtons].buttons;}
...
else{res.push(a)}});if(this.cols[this.colButtons].notext){bbn.fn.each(res,a=>{a.notext=true;return a;});}
return res;}}
...
return(this.groupCols[0].width+this.groupCols[1].width+this.groupCols[2].width)+'px';},hasToolbar(){return this.toolbarButtons.length||bbn.fn.isObject(this.toolbar)||bbn.fn.isFunction(this.toolbar)||bbn.fn.isString(this.toolbar);},hasPager(){return(this.pageable||this.saveable||this.filterable||this.isAjax||this.showable)&&(this.footer===true);},shownFields(){let r=[];bbn.fn.each(this.cols,a=>{if(a.field&&!a.hidden){r.push(a.field);}});return r;},jsonConfig(){return JSON.stringify(this.currentConfig);},isSaved(){return this.jsonConfig===this.savedConfig;},isChanged(){return JSON.stringify(this.currentConfig)!==this.initialConfig;},toolbarButtons(){let r=[],ar=[];if(this.toolbar){ar=bbn.fn.isFunction(this.toolbar)?this.toolbar():(Array.isArray(this.toolbar)?this.toolbar.slice():[]);if(!Array.isArray(ar)){ar=[];}
bbn.fn.each(ar,a=>{let o;if(bbn.fn.isString(a)){switch(a){case'insert':o={text:bbn._('Add'),action:this.insert,icon:'nf nf-fa-plus'};break;case'export':o={};break;case'print':o={};break;default:o=a;}}
else if(bbn.fn.isObject(a)){o=bbn.fn.clone(a);}
if(o&&bbn.fn.isObject(o)&&bbn.fn.isString(o.action)){o.action=()=>{this._execCommand(a);}}
if(!o){throw new Error(bbn._("Wrong parameter for toolbar"))}
r.push(o);});}
...
currentLink=res.length;if(!isExpanded&&data[i-1]&&(currentGroupValue===data[i-1].data[groupField])){if(res.length){res.push(tmp);}}
else{tmp.expanded=isExpanded;if(this.expander){expanderIndex=tmp.index;}
res.push(tmp);rowIndex++;}}}
else if(this.expander){let exp=bbn.fn.isFunction(this.expander)?this.expander(data[i],i):this.expander;isExpanded=exp?(this.currentExpanded.includes(data[i].index)||this.allExpanded):false;if(!isGroup){let tmp={index:data[i].index,data:a,rowIndex:rowIndex,rowKey:data[i].key,expander:true,expanded:isExpanded};if(this.selection&&(!bbn.fn.isFunction(this.selection)||this.selection(tmp))){tmp.selected=(!this.uid&&this.currentSelected.includes(data[i].index))||(this.uid&&this.currentSelected.includes(data[i].data[this.uid]));tmp.selection=true;groupNumCheckboxes++;if(tmp.selected){groupNumChecked++;}}
res.push(tmp);rowIndex++;}}
if(!isGroup||isExpanded||!currentGroupValue||(this.expander&&(!bbn.fn.isFunction(this.expander)||this.expander(data[i],i)))){o={index:data[i].index,data:a,rowIndex:rowIndex,rowKey:data[i].key};if(isGroup){if(!currentGroupValue){o.expanded=true;}
else{o.isGrouped=true;o.link=currentLink;o.rowKey=o.rowIndex+'-'+o.rowKey;}}
if(this.expander&&(!bbn.fn.isFunction(this.expander)||this.expander(data[i],i))){o.expansion=true;o.expanderIndex=expanderIndex;o.rowKey=rowIndex+'-'+data[i].key;}
if(this.selection&&((!o.isGrouped&&!this.expander)||(o.isGrouped&&!o.expander)||(o.isGrouped&&o.expansion))&&(!bbn.fn.isFunction(this.selection)||this.selection(o))){o.selected=(!this.uid&&this.currentSelected.includes(data[i].index))||(this.uid&&this.currentSelected.includes(data[i].data[this.uid]));o.selection=true;groupNumCheckboxes++;if(o.selected){groupNumChecked++;}}
res.push(o);rowIndex++;if(isGroup&&this.groupable&&this.groupFooter&&!this.expander&&(!data[i+1]||(data[i+1].data[groupField]!==data[i].data[groupField]))){res.push({index:data[i].index,data:bbn.fn.filter(data,v=>{return v.data[groupField]===data[i].data[groupField];}),rowIndex:rowIndex,rowKey:data[i].key,isGrouped:true,footer:true,selection:false,expander:false});rowIndex++;}}
else{end++;}
...
i++;}
let fdata=[];res.forEach(d=>{if(d.group||d.expander||this.isExpanded(d)||d.aggregated||(this.isExpanded(d)&&d.groupAggregated)||!!d.isFooter||(!d.expander&&!!d.expansion&&this.isExpanded(bbn.fn.getRow(res,{index:d.expanderIndex,expander:true}))&&(!bbn.fn.isFunction(this.expander)||!!this.expander(d)))){if(fdata.length){d.rowIndex=fdata[fdata.length-1].rowIndex+1;}
fdata.push(d)}});return fdata;},hasExpander(){return this.expander||(this.groupable&&(typeof this.group==='number')&&this.cols[this.group]);},currentColumns(){let r=[],cols=bbn.fn.extend(true,[],this.groupCols);bbn.fn.each(cols,(a,i)=>{bbn.fn.each(a.cols,b=>{r.push(bbn.fn.extend(true,{},b,{fixed:i!==1,isLeft:i===0,isRight:i===2,realWidth:b.realWidth?b.realWidth+'px':'auto'}));});});return r;},expanderColumnVisible(){if(this.items&&this.items.length){return!!this.items.filter(i=>!!i.expander).length}
return false},currentMaxRowHeight(){return!!this.maxRowHeight?this.maxRowHeight+'px':'auto';}},methods:{buttonSource(){if(bbn.fn.isFunction(this.realButtons)){return this.realButtons(...arguments);}
if(bbn.fn.isArray(this.realButtons)){return this.realButtons;}
return[];},convertActions(arr,data,col,idx){return bbn.fn.map(arr,a=>{let b=bbn.fn.clone(a);if(a.action&&bbn.fn.isFunction(a.action)){b.action=e=>{this._execCommand(a,data,col,idx,e);};}
return b;});},getTrClass(row){if(bbn.fn.isFunction(this.trClass)){return this.trClass(row);}
...
else if(a.nullable){res[a.field]=null;}
else if(a.type){switch(a.type){case'number':case'money':res[a.field]=a.min>0?a.min:0;break;default:res[a.field]='';}}
else{res[a.field]='';}
...
return data;},showQuery(){if(this.currentQuery){this.getPopup({title:bbn._('Database query and parameters'),scrollable:true,component:{template:`
<div class="bbn-block bbn-spadded">
...
if((e.clientX<this.floatingFilterX)||(e.clientX>this.floatingFilterX+600)||(e.clientY<this.floatingFilterY)||(e.clientY>this.floatingFilterY+200)){if(!this.floatingFilterTimeOut){this.floatingFilterTimeOut=setTimeout(()=>{this.currentFilter=false;this.editedFilter=false;},500);}}else{this.floatingFilterTimeOut=0;}}},getPopup(){if(this.popup){return arguments.length?this.popup.open(...arguments):this.popup;}
return Vue.options.methods.getPopup.apply(this,arguments);},getFilterOptions(){if(this.currentFilter){let o=this.editorGetComponentOptions(this.currentFilter);if(o.field){o.conditions=this.getColFilters(this.currentFilter);}
if(o.conditions.length){o.value=o.conditions[0].value;o.operator=o.conditions[0].operator;this.editedFilter=o.conditions[0];}
o.multi=false;return o;}},openMultiFilter(){this.currentFilter=false;let table=this;this.getPopup({title:bbn._('Multiple filters'),component:{template:`<bbn-scroll><bbn-filter v-bind="source" @change="changeConditions" :multi="true"></bbn-filter></bbn-scroll>`,props:['source'],methods:{changeConditions(o){table.currentFilters.logic=o.logic;table.currentFilters.conditions=o.conditions;}},},width:'90%',height:'90%',source:{fields:bbn.fn.filter(this.cols,a=>{return(a.filterable!==false)&&!a.buttons;}),conditions:this.currentFilters.conditions,logic:this.currentFilters.logic}});},getColFilters(col){let r=[];if(col.field){bbn.fn.each(this.currentFilters.conditions,a=>{if(a.field===col.field){r.push(a);}})}
return r;},showFilter(col,ev){this.filterElement=ev.target
this.floatingFilterX=(ev.pageX-10)<0?0:((ev.pageX-10+600)>this.$el.clientWidth?this.$el.clientWidth-600:ev.pageX-10);this.floatingFilterY=(ev.pageY-10)<0?0:((ev.pageY-10+200)>this.$el.clientHeight?this.$el.clientHeight-200:ev.pageY-10);this.currentFilter=col;},pickableColumnList(){return this.cols.slice().map(a=>{return a.showable!==false;});},openColumnsPicker(){let table=this;this.getPopup({title:bbn._("Columns' picker"),height:'90%',width:'90%',component:{template:`
...
return false;},getConfig(){return{searchValue:this.searchValue,limit:this.currentLimit,order:this.currentOrder,filters:this.currentFilters,hidden:this.currentHidden};},getColumnsConfig(){return JSON.parse(JSON.stringify(this.cols));},setConfig(cfg,no_storage){if(cfg===false){cfg=bbn.fn.clone(this.defaultConfig);}
...
if(this.search){this.searchValue=cfg.searchValue||'';}
...
this.currentConfig={searchValue:this.searchValue,limit:this.currentLimit,order:this.currentOrder,filters:this.currentFilters,hidden:this.currentHidden};if(!no_storage){this.setStorage(this.currentConfig);}
this.$forceUpdate();}},save(){this.savedConfig=this.jsonConfig;},beforeSelect(index,ev){this.$emit('beforeselect',ev,index,this.items[index]);},checkSelection(index,state){if(this.cancelSelection){this.cancelSelection=false;return;}
let row=this.items[index];if(row){if(this.groupable&&row.group){if(row.expanded){bbn.fn.fori((d,i)=>{if(d&&d.selection&&(d.data[this.cols[this.group].field]===row.value)){this.checkSelection(i,state)}},this.items,index+row.num,index+1)}}
...
if(this.isAjax){this.updateData();}}},updateTable(){return;},render(data,column,index){let value=data&&this.isValidField(column.field)?data[column.field]:undefined;if(column.render){return column.render(data,column,index,value)}
return this.renderData(data,column,index);},reset(noCfg){this.initReady=false;this.$emit('reset',this);if(!noCfg){this.setConfig(false);}
this.$nextTick(()=>{this.init();})},addColumn(obj){let def=this.defaultObject();if(obj.aggregate&&!Array.isArray(obj.aggregate)){obj.aggregate=[obj.aggregate];}
for(let n in obj){def[bbn.fn.camelize(n)]=obj[n];}
if(!!obj.buttons){def.filterable=false;def.sortable=false;}
this.cols.push(def);},isBeforeAggregated(groupIndex,idx){return this.isAggregated&&((this.groupCols[groupIndex].cols[idx+1]&&(this.groupCols[groupIndex].cols[idx+1].field===this.isAggregated))||(!this.groupCols[groupIndex].cols[idx+1]&&this.groupCols[groupIndex+1]&&this.groupCols[groupIndex+1].cols[0]&&(this.groupCols[groupIndex+1].cols[0].field===this.isAggregated)));},getDimensionWidth(width){if(bbn.fn.isNumber(width)&&width){return parseInt(width);}
...
return r;},resizeWidth(){let currentTot=this.groupCols[0].width+this.groupCols[1].width+this.groupCols[2].width,parentWidth=this.$el.offsetParent?this.$el.offsetParent.getBoundingClientRect().width:this.lastKnownCtWidth,diff=parentWidth-this.borderLeft-this.borderRight-currentTot,numDynCols=this.currentColumns.filter(c=>(c.width===undefined)&&!c.isExpander&&!c.isSelection&&!c.hidden).length,numStaticCols=this.currentColumns.filter(c=>!!c.width&&!c.isExpander&&!c.isSelection&&!c.hidden).length,newWidth=numDynCols||numStaticCols?(diff /(numDynCols||numStaticCols)):0;if(newWidth){this.isResizingWidth=true;bbn.fn.each(this.groupCols,(groupCol,groupIdx)=>{let sum=0,sumRight=0,sumLeft=0;bbn.fn.each((groupIdx!==2)?groupCol.cols:groupCol.cols.slice().reverse(),col=>{if(!col.hidden){if(!col.isExpander&&!col.isSelection&&((!!numDynCols&&(col.width===undefined))||(!numDynCols&&!!numStaticCols&&!!col.width))){let tmp=col.realWidth+newWidth;if((col.width!==undefined)&&(!bbn.fn.isString(col.width)||bbn.fn.isNumber(bbn.fn.substr(col.width,-1)))){if(tmp<parseFloat(col.width)){tmp=parseFloat(col.width);}}
...
sum+=col.realWidth;if(groupIdx===0){this.$set(col,'left',sumLeft);sumLeft+=col.realWidth;}
if(groupIdx===2){this.$set(col,'right',sumRight);sumRight+=col.realWidth;}}})
this.$set(this.groupCols[groupIdx],'width',sum);sum=0;sumLeft=0;sumRight=0;});this.isResizingWidth=false;}
return this;},isExpanded(d){if(this.allExpanded){return true;}
if(!this.expander&&((this.group===false)||!this.groupable)){return true;}
if(this.expander&&!this.groupable){return this.currentExpanded.includes(d.index);}
if(this.groupable&&(this.group!==false)&&this.cols[this.group]&&this.cols[this.group].field){if(d.data[this.cols[this.group].field]!==undefined){return this.currentExpandedValues.includes(d.data[this.cols[this.group].field]);}
return true;}
if((d.isGrouped||d.groupAggregated)&&this.currentExpanded.includes(d.link)){return true;}
...
if(a.width){if(bbn.fn.isString(a.width)&&(bbn.fn.substr(a.width,-1)==='%')){a.realWidth=Math.floor(parentWidth*this.getDimensionWidth(a.width)/ 100);if(a.realWidth<(bbn.fn.isMobile()?this.minimumColumnWidthMobile:this.minimumColumnWidth)){a.realWidth=bbn.fn.isMobile()?this.minimumColumnWidthMobile:this.minimumColumnWidth;}}
...
if(a.fixed){if((a.fixed==='left')||((a.fixed!=='right')&&(this.fixedDefaultSide==='left'))){if(a.buttons!==undefined){}
groupCols[0].cols.push(a);if(!a.hidden){groupCols[0].visible++;}}
else{if(a.buttons!==undefined){}
groupCols[2].cols.push(a);if(!a.hidden){groupCols[2].visible++;}}}
else{if(a.buttons!==undefined){}
groupCols[1].cols.push(a);if(!a.hidden){groupCols[1].visible++;}}}}});let firstGroup=groupCols[0].visible?0:1;if(this.selection){let o={isExpander:false,isSelection:true,title:' ',filterable:false,width:40,realWidth:40};if(firstGroup===0){o.fixed=true;o.isLeft=true;}
...
else if(i===2){c.right=sumRight;sumRight+=c.realWidth;}}});a.width=sum;sum=0;sumLeft=0;sumRight=0;});this.groupCols.splice(0);this.$set(this,'groupCols',groupCols);this.colButtons=colButtons;this.isAggregated=isAggregated;this.aggregatedColumns=aggregatedColumns;this.resizeWidth();this.$nextTick(()=>{this.initReady=true;if(with_data){this.$once('dataloaded',()=>{this.initStarted=false;});this.updateData();}
else{this.initStarted=false;}});});},'init',1000);},keydown(e){if(this.isBatch&&this.editedRow&&(e.which===9)||(e.which===13)){e.preventDefault();}},show(colIndexes,hide){if(!Array.isArray(colIndexes)){colIndexes=[colIndexes];}
...
return!col.buttons&&col.field;},focusRow(ev,idx){if(ev.target.tagName!=='BUTTON'){this.focusedRow=idx;}},blurRow(ev,idx){if(ev.target.tagName!=='BUTTON'){this.focusedRow=false;}},clickCell(col,colIndex,dataIndex){if(this.filteredData[dataIndex]){this.$emit('click-row',this.filteredData[dataIndex].data,dataIndex);this.$emit('click-cell',col,colIndex,dataIndex);}},dbclickCell(col,colIndex,dataIndex,data,itemIndex,force){if(this.zoomable&&(!!col.zoomable||force)){let obj={title:col.title||col.ftitle,minHeight:'20%',minWidth:'20%'};if(!!col.component){obj.component=col.component;obj.source=bbn.fn.isFunction(col.mapper)?col.mapper(data):data;obj.componentOptions=col.options;}
else if(bbn.fn.isFunction(col.render)){obj.content=`<div class="bbn-spadded">${col.render(data, col, itemIndex)}</div>`;}
else{obj.content=`<div class="bbn-spadded">${data[col.field]}</div>`;}
this.getPopup().open(obj);}},focusout(idx){this.clickedTd=null;if((idx===undefined)||(idx===this.focusedRow)){this.focused=false;setTimeout(()=>{if(!this.focused){this.focusedRow=false;}},50);}},focusin(idx,e){if(!e.target.closest('td')||!e.target.closest('td').classList.contains('bbn-table-buttons')||e.target.closest('td').classList.contains('bbn-table-edit-buttons')){this.focused=true;this.clickedTd=e.target;if(this.focusedRow!==idx){this.focusedRow=idx;}}},listOnBeforeMount(){},checkAll(){bbn.fn.each(this.items,(a,i)=>{this.checkSelection(i,true);})},uncheckAll(){bbn.fn.each(this.items,(a,i)=>{this.checkSelection(i,false);})},getDataIndex(itemIndex){return this.items[itemIndex]?this.items[itemIndex].index:-1;},setFocusedElement(ev){if(this.editable&&(this.editMode==='inline')&&(this.tmpRow||this.editedRow)&&(ev.target.tagName!=='TR')&&(ev.target.tagName!=='TD')){let e=ev.target.closest('td'),pos=e.getBoundingClientRect();this.focuseElementX=pos.x;this.focusedElementY=pos.y-pos.height;this.focusedElement=ev.target;}},getColOptions(data,col,idx){if(col.options){return bbn.fn.isFunction(col.options)?col.options(data,col,idx):col.options;}
return{};},onUserResize(e){let d=e.target._bbn.directives.resizable.options.data,nextCol=this.groupCols[d.groupColIndex].cols[d.columnIndex+1],nextColSize=nextCol?nextCol.realWidth+e.detail.movement:0;if((d.column.realWidth!==e.detail.size)&&(e.detail.size>=this.defaultColumnWidth)&&(!nextCol||(nextColSize>=this.defaultColumnWidth))){this.groupCols[d.groupColIndex].cols[d.columnIndex].realWidth=e.detail.size;if(nextCol){this.groupCols[d.groupColIndex].cols[d.columnIndex+1].realWidth=nextColSize;}
this.$forceUpdate();}},onButtonsMenuSelect(item,data,col,i,ev,floater){ev.preventDefault();this._execCommand(item,data,col,i,ev);floater.closeAll();}},created(){this.componentClass.push('bbn-resize-emitter');if(this.$slots.default){for(let node of this.$slots.default){if(node.componentOptions&&(node.componentOptions.tag==='bbns-column')){this.addColumn(node.componentOptions.propsData);}
else if((node.tag==='bbns-column')&&node.data&&node.data.attrs){this.addColumn(node.data.attrs);}
...
if(this.editable&&(this.editMode==='inline')){if(bbn.fn.isNumber(oldIndex)&&this.items[oldIndex]){let idx=this.items[oldIndex].index;if((this.editedIndex===idx)&&((idx===-1)||this.isModified(idx))){if(this.autoSave){this.saveInline();}
else if(this.autoReset){this.cancel();}
else{this.$emit('change',this.items[oldIndex].data,idx);}}}
this.editedRow=false;if(bbn.fn.isNumber(newIndex)&&this.items[newIndex]&&!this.items[newIndex].group&&!this.items[newIndex].expander){let comeFromAfter=bbn.fn.isNumber(oldIndex)&&(newIndex===(oldIndex-1));this.$nextTick(()=>{this.edit(this.items[newIndex].data,null,newIndex);this.$nextTick(()=>{let ele=this.clickedTd||this.getTr(newIndex);let nextInputs=ele?ele.querySelectorAll('input'):[];let nextInput;bbn.fn.each(nextInputs,a=>{if(a.offsetWidth){nextInput=a;if(!comeFromAfter){return false;}}});if(nextInput){nextInput.focus();}});});}}},lastKnownCtWidth(){if(this.groupCols.length&&!this.initStarted&&(this.groupCols[0].cols.length||this.groupCols[1].cols.length||this.groupCols[2].cols.length)){this.$nextTick(()=>{this.resizeWidth();})}}},components:{tableDots:{name:'table-dots',template:`
<div class="bbn-c bbn-lg"
  ...
  <i class="nf nf-mdi-dots_horizontal bbn-p bbn-primary-text-alt"/>
        ...
        `,props:{source:{type:Object}},data(){return{visible:false,table:this.closest('bbn-table')}},methods:{checkVisibility(){if(this.table.maxRowHeight&&this.table.zoomable){let td=this.$el.closest('td');if(!!td&&!!td.firstElementChild&&!!td.firstElementChild.firstElementChild){let styleFirst=window.getComputedStyle(td.firstElementChild),styleSecond=window.getComputedStyle(td.firstElementChild.firstElementChild);this.visible=(parseFloat(styleSecond.height)+parseFloat(styleFirst.paddingTop)+parseFloat(styleFirst.paddingBottom))>this.table.maxRowHeight;if(this.visible){td.firstElementChild.firstElementChild.style.setProperty('height','calc('+this.table.maxRowHeight+'px - 2.3rem)');td.firstElementChild.firstElementChild.style.overflow='hidden';}}}
else{this.visible=false;}}},mounted(){this.$nextTick(()=>{this.checkVisibility();});}}}});})(window.bbn,window.Vue);if(bbn_resolve){bbn_resolve("ok");}})(bbn);}