...
else if(this.expander){let exp=bbn.fn.isFunction(this.expander)?this.expander(data[i],i):this.expander;isExpanded=exp?this.currentExpanded.includes(data[i].index):false;}
...
i++;}
let fdata=[];res.forEach(d=>{if(d.group||d.expander||this.isExpanded(d)||d.aggregated||(this.isExpanded(d)&&d.groupAggregated)||(!d.expander&&!!d.expansion&&this.isExpanded(bbn.fn.getRow(res,{index:d.expanderIndex,expander:true}))&&(!bbn.fn.isFunction(this.expander)||!!this.expander(d)))){if(fdata.length){d.rowIndex=fdata[fdata.length-1].rowIndex+1;}
fdata.push(d)}});return fdata;},hasExpander(){return this.expander||(this.groupable&&(typeof this.group==='number')&&this.cols[this.group]);},currentColumns(){let r=[];bbn.fn.each(this.groupCols,(a,i)=>{bbn.fn.each(a.cols,b=>{r.push(bbn.fn.extend(true,{},b,{fixed:i!==1,isLeft:i===0,isRight:i===2,realWidth:b.realWidth?b.realWidth+'px':'auto'}));});});return r;},expanderColumnVisible(){if(this.items&&this.items.length){return!!this.items.filter(i=>!!i.expander).length}
...
if(a.fixed){if((a.fixed==='left')||((a.fixed!=='right')&&(this.fixedDefaultSide==='left'))){if(a.buttons!==undefined){colButtons=groupCols[0].cols.length;}
groupCols[0].cols.push(a);if(!a.hidden){groupCols[0].visible++;}}
else{if(a.buttons!==undefined){colButtons=groupCols[0].cols.length+groupCols[1].cols.length+groupCols[2].cols.length;}
groupCols[2].cols.push(a);if(!a.hidden){groupCols[2].visible++;}}}
else{if(a.buttons!==undefined){colButtons=groupCols[0].cols.length+groupCols[1].cols.length;}
...
this.editedRow=false;if(bbn.fn.isNumber(newIndex)&&this.items[newIndex]&&!this.items[newIndex].group&&!this.items[newIndex].expander){let comeFromAfter=bbn.fn.isNumber(oldIndex)&&(newIndex===(oldIndex-1));this.$nextTick(()=>{this.edit(this.items[newIndex].data,null,newIndex);this.$nextTick(()=>{let tr=this.getTr(newIndex),nextInputs=tr?tr.querySelectorAll('input'):[],nextInput;bbn.fn.each(nextInputs,a=>{if(a.offsetWidth){nextInput=a;if(!comeFromAfter){return false;}}});if(nextInput){nextInput.focus();}});});}}},lastKnownCtWidth(){if(this.groupCols.length&&!this.initStarted&&(this.groupCols[0].cols.length||this.groupCols[1].cols.length||this.groupCols[2].cols.length)){this.$nextTick(()=>{this.resizeWidth();})}},},components:{tableDots:{name:'table-dots',template:`
