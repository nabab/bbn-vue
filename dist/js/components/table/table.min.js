...
return false}},methods:{convertActions(arr,data,col,idx){return bbn.fn.map(arr,a=>{let b=bbn.fn.clone(a);if(a.action&&bbn.fn.isFunction(a.action)){b.action=e=>{this._execCommand(a,data,col,idx,e);};}
...
return!col.buttons&&col.field;},focusRow(ev,idx){if(ev.target.tagName!=='BUTTON'){this.focusedRow=idx;}},blurRow(ev,idx){if(ev.target.tagName!=='BUTTON'){this.focusedRow=false;}},clickCell(col,colIndex,dataIndex){if(this.filteredData[dataIndex]){this.$emit('click-row',this.filteredData[dataIndex].data,dataIndex);this.$emit('click-cell',col,colIndex,dataIndex);}},dbclickCell(col,colIndex,dataIndex,data,itemIndex){if(this.zoomable&&!!col.zoomable){bbn.fn.log('mirko',col,colIndex,dataIndex);let obj={width:'70%',height:'70%',title:col.title||col.ftitle};if(!!col.component){obj.component=col.component;obj.source=bbn.fn.isFunction(col.mapper)?col.mapper(data):data;obj.componentOptions=col.options;}
...
this.editedRow=false;if(bbn.fn.isNumber(newIndex)&&this.items[newIndex]&&!this.items[newIndex].group&&!this.items[newIndex].expander){let comeFromAfter=bbn.fn.isNumber(oldIndex)&&(newIndex===(oldIndex-1));this.$nextTick(()=>{this.edit(this.items[newIndex].data,null,newIndex);this.$nextTick(()=>{let tr=this.getTr(newIndex),nextInputs=tr?tr.querySelectorAll('input'):[],nextInput;bbn.fn.each(nextInputs,a=>{if(a.offsetWidth){nextInput=a;if(!comeFromAfter){return false;}}});if(nextInput){nextInput.focus();}});});}}},lastKnownCtWidth(){if(this.groupCols.length&&!this.initStarted&&(this.groupCols[0].cols.length||this.groupCols[1].cols.length||this.groupCols[2].cols.length)){this.$nextTick(()=>{this.resizeWidth();})}}}});})(window.bbn,window.Vue);if(bbn_resolve){bbn_resolve("ok");}})(bbn);}