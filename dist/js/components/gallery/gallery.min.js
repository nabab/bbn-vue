...
else{this.$emit(this.selectingMode,this.currentSelectedData);this.setSelecting(false);}}},onResize(){this.width=this.$refs.gallery.offsetWidth;},resetSearch(){this.currentSearch='';}},mounted(){this.$nextTick(()=>{this.onResize();this.ready=true;});},watch:{currentSearch(newVal){if(this.searchTimeout){clearTimeout(this.searchTimeout);}
...
else if(newVal){this.currentFilters.conditions.push({field:this.searchName,operator:'contains',value:newVal})}},1000)},itemWidth(val){this.currentItemWidth=val;}},components:{galleryCol:{name:'gallery-col',template:`
<div :style="colStyle">
...
return style;},imgStyle(){return{width:this.loaded?'100%':0,height:this.loaded?'':0,margin:0,display:'block',visibility:this.loaded?'visible':'hidden'}},isObj(){return bbn.fn.isObject(this.source);},showOverlay(){return this.col.gallery.overlay&&this.isObj&&(this.source.data[this.col.gallery.overlayName]!==undefined);},isSelected(){return this.col.gallery.currentSelected.includes(!!this.col.gallery.uid?this.source.data[this.col.gallery.uid]:this.source.index);},imgSrc(){let src='';if(bbn.fn.isString(this.source.data)){src=this.source.data;}
else{let prop=this.col.gallery.pathName||'thumb'||'content';if(this.source.data[prop]){src=this.source.data[prop];}}
if(!!src&&bbn.fn.isString(src)){return`${src}${src.indexOf('?') > -1 ? '&' : '?'}w=${this.col.gallery.currentItemWidth}&thumb=1`;}
return null;}},methods:{isFunction:bbn.fn.isFunction,action(ev){if(this.col.gallery.isSelecting){let id=!!this.col.gallery.uid?this.source.data[this.col.gallery.uid]:this.source.index;if(this.isSelected){this.col.gallery.currentSelected.splice(this.col.gallery.currentSelected.indexOf(id),1);if(!!this.col.gallery.uid){let idx=bbn.fn.search(this.col.gallery.currentSelectedData,this.col.gallery.uid,id);if(idx>-1){this.col.gallery.currentSelectedData.splice(idx,1);}}}
...
else{this.col.gallery.$emit('clickItem',this.source);}}}}}},galleryZoom:{name:'gallery-zoom',template:`
...
<div class="bbn-rel">
  ...
  <img :src="imgSrc"
       class="bbn-radius bbn-bordered">
        ...
        `,props:{source:{type:[String,Number],required:true}},computed:{gallery(){return this.closest('bbn-gallery');},imgSrc(){if(this.gallery){let data={},src='';if(!!this.gallery.uid){data=bbn.fn.getRow(this.gallery.currentSelectedData,this.gallery.uid,this.source);}
if(bbn.fn.isString(data)){src=data;}
else{let prop=this.gallery.pathName||'thumb'||'content';if(data[prop]){src=data[prop];}}
if(!!src&&bbn.fn.isString(src)){return`${src}${src.indexOf('?') > -1 ? '&' : '?'}w=70&thumb=1`;}}
return null;}},methods:{unselect(){if(this.gallery){this.gallery.currentSelected.splice(this.gallery.currentSelected.indexOf(this.source),1);}}}}}})})(bbn);if(bbn_resolve){bbn_resolve("ok");}})(bbn);}