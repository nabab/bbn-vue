...
this.setValue(val?dayjs(val).format(this.getValueFormat(dayjs(val).format(this.currentValueFormat))):'');},setTime(val){val=dayjs(val,'HH:mm'+(this.showSecond?':ss':''));if(this.value){let mom=dayjs(this.value.toString(),this.getValueFormat(this.value.toString()));val=dayjs(dayjs(dayjs(val).date(mom.date())).month(mom.month())).year(mom.year());}
...
if(!bbn.fn.isNull(this.second)&&ss){ss.onResize();ss.scrollTo(0,this.getRef('secondActive'));}},400)})}}}}}});})(bbn);if(bbn_resolve){bbn_resolve("ok");}})(bbn);}