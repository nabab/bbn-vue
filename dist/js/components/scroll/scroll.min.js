...
this.interval=setInterval(()=>{if(this.scrollable&&this.$el.offsetParent){let container=this.getRef('scrollContainer');let contentWidth=Math.min(container.scrollWidth,this.maxWidth);let contentHeight=Math.min(container.scrollHeight,this.maxHeight);if((contentWidth&&(this.contentWidth!==contentWidth)&&(!this.contentWidth||(Math.abs(contentWidth-this.contentWidth)>3)))||(contentHeight&&(this.contentHeight!==contentHeight)&&(!this.contentHeight||(Math.abs(contentHeight-this.contentHeight)>3)))){let e=new Event('resizeContent',{cancelable:true});this.$emit('resizeContent',e,{width:contentWidth,height:contentHeight});if(!e.defaultPrevented){this.onResize(true);}}}},1000);this.scrollReady=true;}},scrollable(newVal){if(newVal){this.initSize();}},containerWidth(){let x=this.getRef('xScroller');if(x){x.onResize();}},containerHeight(){let y=this.getRef('yScroller');if(y){y.onResize();}},currentX(x){if(!x){this.$emit('reachLeft');}
