...
return cfg;},containerClass(){let cls='bbn-scroll-container';if(this.disabled){cls+=' bbn-scroll-disabled';}
if(this.ready&&!this.isDragging){cls+=' bbn-scroll-not-dragged';}
if(this.isScrolling||(!this.isMeasuring&&!this.scrollable)){cls+=' bbn-overlay';}
...
this.setScrollDelay();}}
if(this.scrollable&&e){e.stopImmediatePropagation();}},setScrollDelay(){clearTimeout(this.scrollTimeout);this.scrollTimeout=setTimeout(()=>{this.afterScroll();},this.scrollInitial.touched==='finished'?100:500);},afterScroll(){bbn.fn.log("SCROLL END!!");if(this.fullPage&&this.scrollInitial){if(this.hasScrollX&&(this.currentX!==this.scrollInitial.x)){let r1=this.scrollInitial.x?Math.round(this.scrollInitial.x / this.containerWidth):0;let r2=this.currentX?Math.round(this.currentX / this.containerWidth):0;let left;if(r1!==r2){left=r2*this.containerWidth;}
else if(this.scrollInitial.x<this.currentX){left=(r1+1)*this.containerWidth;}
else if(this.scrollInitial.x>this.currentX){left=(r1-1)*this.containerWidth;}
if(bbn.fn.isNumber(left)&&(left!==this.currentX)){this.$refs.xScroller.scrollTo(left,true);}}
else if(this.hasScrollY&&(this.currentY!==this.scrollInitial.y)){let r1=this.scrollInitial.y?Math.round(this.scrollInitial.y / this.containerHeight):0;let r2=this.currentY?Math.round(this.currentY / this.containerHeight):0;let top;if(r1!==r2){top=r2*this.containerHeight;}
else if(this.scrollInitial.y<this.currentY){top=(r1+1)*this.containerHeight;}
else if(this.scrollInitial.y>this.currentY){top=(r1-1)*this.containerHeight;}
bbn.fn.log("scroll endf",this.scrollInitial.y,this.currentY,r1,r2,top,'-------');if(bbn.fn.isNumber(top)&&(top!==this.currentY)){this.$refs.yScroller.scrollTo(top,true);}}
...
return this.keepCool(()=>{if(this.scrollable&&(this.setContainerMeasures()||force)){this.setResizeMeasures();let container=this.$el;let content=this.getRef('scrollContent');let ct=this.getRef('scrollContainer');if(!content){return;}
let x=ct.scrollLeft;let y=ct.scrollTop;let contentBox=content.getBoundingClientRect()
