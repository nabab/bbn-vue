...
this.interval=setInterval(()=>{if(this.scrollable&&this.$el.offsetParent){let content=this.getRef('scrollContent');let contentBox=content?content.getBoundingClientRect():{};let contentWidth=contentBox.width;let contentHeight=contentBox.height;if((contentWidth&&(this.contentWidth!==contentWidth)&&(!this.contentWidth||(Math.abs(contentWidth-this.contentWidth)>3)))||(contentWidth&&(this.contentWidth!==contentHeight)&&(!this.contentHeight||(Math.abs(contentHeight-this.contentHeight)>3)))){this.onResize(true);}}},1000);this.scrollReady=true;setTimeout(()=>{this.afterReady=true;},1000);}},scrollable(newVal){if(newVal){this.initSize();}},containerWidth(){let x=this.getRef('xScroller');if(x){x.onResize();}},containerHeight(){let y=this.getRef('yScroller');if(y){y.onResize();}},currentX(x){if(!x){this.$emit('reachLeft');}
