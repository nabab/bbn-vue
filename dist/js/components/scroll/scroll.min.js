...
this.isMeasuring=false;resolve({w:this.naturalWidth,h:this.naturalHeight});});});},hasX(){return this.scrollable&&((this.axis==='both')||(this.axis==='x'));},hasY(){return this.scrollable&&((this.axis==='both')||(this.axis==='y'));},initSize(){return this.onResize(true).then(()=>{this.ready=true;});},onResize(force){if(!this.ready){return new Promise(resolve=>{setTimeout(()=>{resolve();},1);});}
...
this.$emit('scrolly',y);},stepX(val){this.currentStepX=bbn.fn.isDom(val)?val.clientHeight:val;},stepY(val){this.currentStepY=bbn.fn.isDom(val)?val.clientHeight:val;}}});})(bbn);if(bbn_resolve){bbn_resolve("ok");}})(bbn);}