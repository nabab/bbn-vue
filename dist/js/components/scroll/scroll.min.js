(bbn_resolve)=>{((bbn)=>{let script=document.createElement('script');script.innerHTML=`<div :class="elementClass"
     :style="elementStyle"
>
  <div :class="{
...
</div>`;script.setAttribute('id','bbn-tpl-component-scroll');script.setAttribute('type','text/x-template');document.body.insertAdjacentElement('beforeend',script);let css=document.createElement('link');css.setAttribute('rel',"stylesheet");css.setAttribute('href',bbn.vue.libURL+"dist/js/components/scroll/scroll.css");document.head.insertAdjacentElement('beforeend',css);(function(bbn){"use strict";Vue.component('bbn-scroll',{name:'bbn-scroll',mixins:[bbn.vue.basicComponent,bbn.vue.resizerComponent,bbn.vue.keepCoolComponent,bbn.vue.eventsComponent],props:{maxWidth:{type:Number},maxHeight:{type:Number},minWidth:{type:Number},minHeight:{type:Number},width:{type:Number},height:{type:Number},classes:{type:String,default:""},speed:{type:Number,default:50},axis:{type:String,default:"both"},scrollAlso:{type:[HTMLElement,Array,Function],default(){return[];}},x:{type:Number,default:0},y:{type:Number,default:0},hidden:{type:[Boolean,String],default:false},barColor:{type:String},latency:{type:Number,default:25},scrollable:{type:Boolean,default:true},fullPage:{type:Boolean,default:false},offsetX:{type:[Number,Array],default:0},offsetY:{type:[Number,Array],default:0},},data(){return{readyDelay:false,afterReady:false,show:false,currentX:this.x,currentY:this.y,scrollPos:'0px',containerPadding:'0px',hiddenX:(this.hidden===true)||((this.hidden==='x')),hiddenY:(this.hidden===true)||((this.hidden==='y')),currentWidth:this.width||null,currentHeight:this.height||null,naturalWidth:0,naturalHeight:0,containerWidth:0,containerHeight:0,contentWidth:0,contentHeight:0,isMeasuring:false,hasScroll:false,hasScrollX:false,hasScrollY:false,promise:false,isScrolling:false,isDragging:false,isFocused:false,previousTouch:{x:null,y:null},interval:null,scrollReady:false,touchX:false,touchY:false,touched:0,touchDirection:null};},computed:{elementClass(){let st=this.componentClass.join(' ')+' bbn-overlay';if(!this.ready){st+=' bbn-invisible';}
...
return false;},onTouchstart(e){bbn.fn.log("touch start");bbn.fn.log("TOUCHSTART");if(!this.scrollable||this.disabled){return;}
...
this.touched=(new Date()).getTime();}},page(direction){if(!this.isScrolling){this.isScrolling=true;let x=this.currentX;let y=this.currentY;let xRatio=x?Math.round(x / this.containerWidth):0;let yRatio=y?Math.round(y / this.containerHeight):0;x=xRatio*this.containerWidth;y=yRatio*this.containerHeight;if(x>this.contentHeight-this.containerWidth){x=this.contentHeight-this.containerWidth;}
if(y>this.contentHeight-this.containerHeight){y=this.contentHeight-this.containerHeight;}
let p;switch(direction){case'down':this.currentY=y+this.containerHeight;p=this.$refs.yScroller.scrollTo(this.currentY,true);break;case'up':this.currentY=y-this.containerHeight;p=this.$refs.yScroller.scrollTo(this.currentY,true);break;case'right':this.currentX=x+this.containerWidth;p=this.$refs.xScroller.scrollTo(this.currentX,true);break;case'left':this.currentX=x-this.containerWidth;p=this.$refs.xScroller.scrollTo(this.currentX,true);break;}
p.then(()=>{this.touched=false;this.isScrolling=false;this.$emit('scroll');});}},onTouchend(e){if(!this.scrollable||this.disabled){return;}
if(this.fullPage&&this.touchDirection){this.page(this.touchDirection);}
else if(!this.isScrolling){this.touched=false;}
this.touchDirection=null;this.touchY=null;},onTouchmove(e){bbn.fn.log("touch move");if(!this.scrollable||this.disabled){return;}
if(this.fullPage){e.preventDefault();if(!this.touchDirection&&e.targetTouches&&e.targetTouches.length){let ev=e.targetTouches[0];if(this.hasScrollY&&(this.touchY!==ev.clientY)){this.lastDirections={touchY:this.touchY,clientY:ev.clientY};this.touchDirection=this.touchY>ev.clientY?'down':'up';}
else if(this.hasScrollX&&(this.touchX!==ev.clientX)){this.touchDirection=this.touchX>ev.clientX?'right':'left';}}}
if(this.scrollable&&e){e.stopImmediatePropagation();}
this.$emit('touchmove',e);},preventKeyIfScrolling(e){if(this.isScrolling&&(32>=e.key<=40)){e.preventDefault();}},onScroll(e){if(!this.ready||(this.scrollable===false)){return;}
if(this.fullPage&&(this.touched||this.isScrolling)){if(this.isScrolling){e.preventDefault();}
return;}
let ct=this.getRef('scrollContainer');let x=ct.scrollLeft;let y=ct.scrollTop;let direction;bbn.fn.log("Scroll",this.hasScrollX,this.hasScrollY,y,this.currentY);if(this.hasScrollX&&(x!==this.currentX)){if(this.fullPage&&this.afterReady&&!this.hashJustChanged()){if(x>this.currentX){direction='right';}
else if(x<this.currentX){direction='left';}}
this.currentX=x;}
else if(this.hasScrollY&&(y!==this.currentY)){if(this.fullPage&&this.afterReady&&!this.hashJustChanged()){if(y>this.currentY){direction='down';}
else if(y<this.currentY){direction='up';}}
this.currentY=y;}
if(this.scrollable&&e){e.stopImmediatePropagation();}
if(direction){this.page(direction);}
this.$emit('scroll',e);},scrollTo(x,y,anim){return new Promise(resolve=>{if(!this.hasScroll||!this.ready){return;}
if(this.hasScrollX&&(x!==undefined)&&(x!==null)&&this.$refs.xScroller){this.$refs.xScroller.scrollTo(x,anim).then(()=>{try{resolve();}
catch(e){}});}
if(this.hasScrollY&&(y!==undefined)&&(y!==null)&&this.$refs.yScroller){this.$refs.yScroller.scrollTo(y,anim).then(()=>{try{resolve();}
...
else{this.naturalWidth=this.$el.offsetWidth;this.naturalHeight=this.$el.offsetHeight;this.isMeasuring=false;resolve({w:this.naturalWidth,h:this.naturalHeight});}});});},hasX(){return this.scrollable&&((this.axis==='both')||(this.axis==='x'));},hasY(){return this.scrollable&&((this.axis==='both')||(this.axis==='y'));},initSize(){return this.onResize().then(()=>{this.ready=true;});},onResize(force){if(!this.ready){return new Promise((resolve)=>{setTimeout(()=>{resolve();},1);});}
return this.keepCool(()=>{if(this.scrollable&&(this.setContainerMeasures()||force)){this.setResizeMeasures();let container=this.$el;let content=this.getRef('scrollContent');let ct=this.getRef('scrollContainer');let x=ct.scrollLeft;let y=ct.scrollTop;let contentBox=content.getBoundingClientRect()
let containerBox=container.getBoundingClientRect()
this.contentWidth=contentBox.width;this.contentHeight=contentBox.height;this.containerWidth=containerBox.width;this.containerHeight=containerBox.height;if(this.scrollable){if((this.axis==='both')||(this.axis==='x')&&(this.contentWidth>this.containerWidth)){this.hasScrollX=true;this.$nextTick(()=>{if(this.$refs.xScroller){this.$refs.xScroller.onResize();}})}
...
if(x!==this.currentX){this.currentX=x;}
if(y!==this.currentY){this.currentY=y;}
...
this.interval=setInterval(()=>{if(this.scrollable&&this.$el.offsetParent){let content=this.getRef('scrollContent');let contentBox=content?content.getBoundingClientRect():{};let contentWidth=contentBox.width;let contentHeight=contentBox.height;if((contentWidth&&(this.contentWidth!==contentWidth)&&(!this.contentWidth||(Math.abs(contentWidth-this.contentWidth)>3)))||(contentWidth&&(this.contentWidth!==contentHeight)&&(!this.contentHeight||(Math.abs(contentHeight-this.contentHeight)>3)))){this.onResize(true);}}},1000);setTimeout(()=>{this.scrollReady=true;setTimeout(()=>{this.afterReady=true;},1000);},1000);},100);}},scrollable(newVal){if(newVal){this.initSize();}},containerWidth(){let x=this.getRef('xScroller');if(x){x.onResize()}},containerHeight(){let y=this.getRef('yScroller');if(y){y.onResize()}},currentX(x){if(!x){this.$emit('reachLeft');}
...
this.$emit('scrolly',y);}}});})(bbn);if(bbn_resolve){bbn_resolve("ok");}})(bbn);}