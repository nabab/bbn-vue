  ...
  <div class="bbn-cms-block-icons bbn-vmiddle">
    ...
    <div class="bbn-overlay bbn-vmiddle">
...
</div>`;script.setAttribute('id','bbn-tpl-component-cms-block');script.setAttribute('type','text/x-template');document.body.insertAdjacentElement('beforeend',script);let css=document.createElement('link');css.setAttribute('rel','stylesheet');css.setAttribute('href',bbn.vue.libURL+'dist/js/components/cms-block/cms-block.css');document.head.insertAdjacentElement('beforeend',css);(function(bbn){"use strict";let titleTemplates={h1:`<h1 v-text="source.content"></h1>`,h2:`<h2 v-text="source.content"></h2>`,h3:`<h3 v-text="source.content"></h3>`,h4:`<h4 v-text="source.content"></h4>`,h5:`<h5 v-text="source.content"></h5>`,},htmlTemplates={p:`<p v-html="source.content"></p>`,span:`<span v-html="source.content"></span>`},templates={text:{view:`<div v-html="source.content || '&nbsp;'"/>`,edit:`<bbn-textarea class="bbn-w-100"
                           v-model="source.content"/>`},html:{view:`<div @click="$parent.editMode" @mouseover="$parent.mouseover" @mouseleave="$parent.mouseleave"
      ...
      `},line:{view:`<div class="component-container"><hr :style="style"></div>`,edit:`<div class="block-line-edit component-container">
          ...
          </div>`},};let borderStyle=[{"text":"hidden","value":"hidden"},{"text":"dotted","value":"dotted"},{"text":"dashed","value":"dashed"},{"text":"solid","value":"solid"},{"text":"double","value":"double"},{"text":"groove","value":"groove"},{"text":"ridge","value":"ridge"}];Vue.component('bbn-cms-block',{mixins:[bbn.vue.basicComponent],props:{source:{type:Object,required:true},title:{type:String,default:''},index:{type:Number,},path:{type:String,default:''},linkURL:{type:String,default:''},},data(){return{over:false,edit:false,isAdmin:true,editing:true,width:'100%',height:'100%',ready:true,initialSource:{}}},computed:{changed(){return!bbn.fn.isSame(this.initialSource,this.source)},type(){return this.source.type||'text'},parent(){return this.ready?this.closest('bbn-container').getComponent():null;}},methods:{onMyMouseEnter(){alert('enter')},mouseleave(){this.over=false},mouseover(){this.over=true},mouseenter(){alert('enter')},selectImg(st){bbn.fn.link(st);},alert(){alert('test')},_setEvents(){bbn.fn.log("setEvenbt")
document.addEventListener('mousedown',this.checkMouseDown);document.addEventListener('touchstart',this.checkMouseDown);document.addEventListener('keydown',this.checkKeyCode);},checkKeyCode(e){bbn.fn.log("checkKeyCode")
if(e.keyCode===27){this.edit=false;}},checkMouseDown(e){if(!e.target.closest(".bbn-cms-block-edit")){this.edit=false;alert(this.edit)}
else{alert(this.edit)
this.editMode();}},editBlock(){bbn.fn.log("editBlock")
if(this.changed){appui.success(bbn._('Block changed'))
this.$nextTick(()=>{this.edit=false;})}
else{this.edit=false;}},cancelEdit(){bbn.fn.log("cancelEdit")
bbn.fn.iterate(this.initialSource,(v,i)=>{this.source[i]=v;this.edit=false;})},editMode(){bbn.fn.log("editMode")
let blocks=this.closest('bbn-container').getComponent().findAll('bbn-cms-block');bbn.fn.each(blocks,(v,i)=>{v.edit=false;v.over=false;})
this.edit=true;},component(type){return{props:{source:{},},template:this.edit?templates[type]['edit']:templates[type]['view'],data(){let tmp=Object.keys(titleTemplates).map((a)=>{return a={text:a,value:a}});return{muted:true,autoplay:false,align:'',tags:tmp,image:[],tinyNumbers:[{text:'1',value:1},{text:'2',value:2},{text:'3',value:3},{text:'4',value:4}],borderStyle:borderStyle,ref:(new Date()).getTime(),show:true,currentCarouselIdx:0}},computed:{edit(){return this.$parent.edit},path(){return this.$parent.path},linkURL(){return this.$parent.linkURL},carouselSource(){if(this.source.source&&(this.source.type==='carousel')){let res=[];var i,j,temparray,chunk=3;for(i=0,j=this.source.source.length;i<j;i+=chunk){temparray=this.source.source.slice(i,i+chunk);res.push(temparray);}
...
return'cols-3'}
else if(this.mobile){return'cols-2'}},youtube(){return this.source.src.indexOf('youtube')>-1},alignClass(){let st='bbn-c';if(this.source.align==='left'){st='bbn-l'}
...
else{if(this.source.style['border-width']){st+='border-width:'+this.source.style['border-width']+(bbn.fn.isNumber(this.source.content['border-width'])?'px;':';');}}}
...
else{this.source.content=c.image.name;}
...
if(this.$parent.edit){if((this.source.type==='image')&&this.source.content&&this.source.content.length){let extension=bbn.fn.substr(this.source.content,this.source.content.lastIndexOf('.'),this.source.content.length)
this.image.push({"name":this.source.content,"size":574906,"extension":extension});}
else if((this.source.type==='gallery')&&this.source.content&&this.source.content.length){}}},mounted(){if((this.source.type==='gallery')||(this.source.type==='carousel')){this.makeSquareImg();}},}},},mounted(){this.initialSource=bbn.fn.extend({},this.source);this.ready=true;if(bbn.fn.isEmpty(this.source.style)){bbn.fn.warning(this.source.type)
this.source.style={};}
...
bbn.fn.log("I AM THE BLOCK! ",this.source);},watch: {changed(){bbn.fn.log("changed")},type(){bbn.fn.log("type")},edit(val){}},});})(bbn);if(bbn_resolve){bbn_resolve("ok");}})(bbn);}