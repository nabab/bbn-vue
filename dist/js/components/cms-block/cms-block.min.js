(bbn_resolve)=>{((bbn)=>{let script=document.createElement('script');script.innerHTML=`<div :class="[componentClass, 'bbn-block']"
     :style="{
       width: width,
       height: height
     }">
...
else{this.edit=false;}},cancelEdit(){bbn.fn.iterate(this.initialSource,(v,i)=>{this.source[i]=v;this.edit=false;})},editMode(){let blocks=this.closest('bbn-container').getComponent().findAll('bbn-cms-block');bbn.fn.each(blocks,(v,i)=>{v.edit=false;v.over=false;})
...
bbn.fn.log("I AM THE BLOCK! ",this.source);},watch: {edit(val){bbn.fn.error('watch')
