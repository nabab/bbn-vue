...
else if(res&&(this.selected===idx)){if(this.views.length){let newIdx=false;bbn.fn.each(this.history,a=>{let tmp=this.getIndex(a);if(tmp!==false){newIdx=tmp;return false;}});if(newIdx===false){while(idx>=0){if(this.views[idx]){newIdx=idx;break;}
idx--;}}
if(this.views[newIdx]){this.activateIndex(this.views[idx]?idx:idx-1);}}
...
else{cp.setCurrent(url);this.activeContainer=cp;}}});if(this.activeContainer){this.activeContainer.show();if(this.scrollable&&this.nav&&!this.breadcrumb){let scroll=this.getRef('horizontal-scroll');if(scroll.ready){this.getRef('horizontal-scroll').scrollTo(this.getRef('tab-'+this.activeContainer.currentIndex));}
else if(scroll){scroll.$on('ready',()=>{setTimeout(()=>{this.getRef('horizontal-scroll').scrollTo(this.getRef('tab-'+this.activeContainer.currentIndex));},100);})}}}}
...
bbn.fn.iterate(bbn.fn.extend(this.getDefaultView(),kept),(a,n)=>{if(view[n]!==a){this.$set(view,n,a);}});if(this.urls[url]){this.urls[url].isInit=false;}}
...
if((d.url!==d.current)&&this.urls[d.current]){bbn.fn.warning("DELETING VIEW CASE.... "+d.current+' '+this.urls[d.current].currentIndex,d.url,bbn.fn.search(this.views,{idx:this.urls[d.current].idx}));this.remove(this.urls[d.current].currentIndex,true);callRealInit=false;this.$on('registered',url=>{if(url===d.url){this.$off('registered',url);this.realInit(url);}})}
...
return res;},retrieveDirtyContainers(){this.dirtyContainers.splice(0,this.dirtyContainers.length);bbn.fn.iterate(this.urls,v=>{if(v.dirty){this.dirtyContainers.push({idx:v.currentIndex,url:v.url});}});},getMenuFn(idx){if(!this.nav||!this.views[idx]||(this.views[idx].menu===false)){return[];}
...
else if(this.observationTower){this.observationTower.observerClear(obs);}},enter(container){},containerComponentMount(){let ct=this.getRef('container');ct.init();this.$nextTick(()=>{ct.show();})},cutTitle(title){return bbn.fn.shorten(title,this.maxTitleLength)},getTabTitle(obj){let t='';if(obj.notext||(obj.title.length>this.maxTitleLength)){t+=obj.title;}
if(obj.ftitle){t+=(t.length?' - ':'')+obj.ftitle;}
return t||null;},getFullTitle(obj){let t='';if(obj.title){t+=obj.title;}
if(obj.ftitle){t+=(t.length?' - ':'')+obj.ftitle;}
...
if(this.isVisual&&(this.orientation==='auto')&&!this.lockedOrientation){this.visualOrientation=this.lastKnownWidth>this.lastKnownHeight?'left':'top';}},'resize',50);},getView(url){return bbn.fn.getRow(this.views,{url:url})},visualStyleContainer(ct){if(!ct.visible||this.visualShowAll){return{zoom:0.1};}
...
if(this.parent){this.parent.registerRouter(this);}
...
if(this.parent){this.parent.unregisterRouter(this);}},watch:{selected(idx){if(this.views[idx]){bbn.fn.map(bbn.fn.filter(this.views,{selected:true}),a=>{if(a.idx!==idx){a.selected=false;}});if(!this.views[idx].selected){this.views[idx].selected=true;}}
