...
`;script.setAttribute('id','bbn-tpl-component-datepicker');script.setAttribute('type','text/x-template');document.body.insertAdjacentElement('beforeend',script);let css=document.createElement('link');css.setAttribute('rel',"stylesheet");css.setAttribute('href',bbn.vue.libURL+"dist/js/components/datepicker/datepicker.css");document.head.insertAdjacentElement('beforeend',css);(function(bbn){"use strict";Vue.component('bbn-datepicker',{mixins:[bbn.vue.basicComponent,bbn.vue.inputComponent,bbn.vue.eventsComponent],props:{source:{type:[String,Array],default(){return[];}},format:{type:String},valueFormat:{type:[String,Function]},mask:{type:String},max:{type:String},min:{type:String},type:{type:String,default:'days',validator:(m)=>['days','weeks','months','years'].includes(m)},disableDates:{type:[Array,Function]},datesRange:{type:Array,default(){return[];}},autosize:{type:Boolean,default:true},onlyEvents:{type:Boolean,default:false}},data(){return{isOpened:false,maskedMounted:false,inputValue:'',oldInputValue:''}},computed:{currentMask(){if(this.mask){return this.mask;}
...
return'DD/MM/YYYY';},inputValueChanged(){return this.inputValue!==this.oldInputValue;}},methods:{getValueFormat(val){return bbn.fn.isFunction(this.valueFormat)?this.valueFormat(val):this.currentValueFormat;},setDate(val){this.setValue(moment(val,'YYYY-MM-DD').isValid()?moment(val,'YYYY-MM-DD').format(this.getValueFormat(val)):'');},setValue(val){let format=!!val?this.getValueFormat(val.toString()):false,value=format?(moment(val.toString(),format).isValid()?moment(val.toString(),format).format(format):''):'';if(value){if(this.min&&(value<this.min)){value=this.min;}
