...
return'';},getMenuFn(idx){if(this.router){return this.router.getMenuFn(idx);}}},watch:{value(v){this.$nextTick(()=>{this.selectedBarColor=this.source[v]?this.getFontColor(v):null;})}},updated(){if(!this.selectedBarColor&&this.source[this.value]){this.selectedBarColor=this.getFontColor(this.value);}},mounted(){this.router=this.closest('bbn-router');this.ready=true;setTimeout(()=>{if(this.source[this.value]){this.selectedBarColor=this.getFontColor(this.value);}},500)}});})(bbn);if(bbn_resolve){bbn_resolve("ok");}})(bbn);}