(bbn_resolve)=>{((bbn)=>{let script=document.createElement('script');script.innerHTML=`<div :class="[componentClass, {'bbn-tabs-scrollable': scrollable, 'bbn-tabs-vertical': isVertical}]">
...
`;script.setAttribute('id','bbn-tpl-component-tabs');script.setAttribute('type','text/x-template');document.body.insertAdjacentElement('beforeend',script);let css=document.createElement('link');css.setAttribute('rel','stylesheet');css.setAttribute('href',bbn.vue.libURL+'dist/js/components/tabs/tabs.css');document.head.insertAdjacentElement('beforeend',css);(bbn=>{"use strict";Vue.component('bbn-tabs',{mixins:[bbn.vue.basicComponent,bbn.vue.inputComponent],props:{source:{type:Array,default(){return[]}},scrollable:{type:Boolean,default:false},closable:{type:Boolean,default:true},limit:{type:Number},maxTitleLength:{type:Number,default:35},position:{type:String,default:'top'},vertical:{type:Boolean,default:false}},data(){return{valueToSet:this.value,selectedBarColor:null}},computed:{scrollCfg(){return this.scrollable?{axis:'x',container:true,hidden:true}:{};},isVertical(){return(this.position==='left')||(this.position==='right');}},methods:{numProperties:bbn.fn.numProperties,getTabTitle(obj){let t='';if(obj.notext||(obj.title.length>this.maxTitleLength)){t+=obj.title;}
...
else{scroll.scrollBefore(true);}}},cutTitle(title){return bbn.fn.shorten(title,this.maxTitleLength)},getFontColor(idx){if(bbn.fn.isNumber(idx)&&this.source[idx]){if(this.source[idx].fcolor){return this.source[idx].fcolor;}}
return'';},getMenuFn(idx){if(this.router){return this.router.getMenuFn(idx);}},onScrollReady(){bbn.fn.log("on scroll, ready");setTimeout(()=>{this.updateScroll();},1500);},updateScroll(){if(this.scrollable){const scroll=this.getRef('horizontal-scroll');const tab=this.getRef('tab-'+this.value);if(scroll&&tab){const x=tab.offsetLeft;if((x<scroll.currentX)||(x>(scroll.currentX+scroll.containerWidth))){scroll.scrollTo(tab.offsetLeft,0,true);}}}}},watch:{value(v){this.$nextTick(()=>{this.selectedBarColor=this.source[v]?this.getFontColor(v):null;this.updateScroll();})}},updated(){if(!this.selectedBarColor&&this.source[this.value]){this.selectedBarColor=this.getFontColor(this.value);}},mounted(){this.router=this.closest('bbn-router');this.ready=true;setTimeout(()=>{if(this.source[this.value]){this.selectedBarColor=this.getFontColor(this.value);}},500)}});})(bbn);if(bbn_resolve){bbn_resolve("ok");}})(bbn);}