(bbn_resolve)=>{((bbn)=>{let script=document.createElement('script');script.innerHTML=`<div :class="[{'bbn-widget': true, 'bbn-h-100': true}, componentClass]">
...
`;script.setAttribute('id','bbn-tpl-component-treemenu');script.setAttribute('type','text/x-template');document.body.insertAdjacentElement('beforeend',script);let css=document.createElement('link');css.setAttribute('rel',"stylesheet");css.setAttribute('href',bbn.vue.libURL+"dist/js/components/treemenu/treemenu.css");document.head.insertAdjacentElement('beforeend',css);(bbn=>{"use strict";Vue.component('bbn-treemenu',{name:'bbn-treemenu',mixins:[bbn.vue.basicComponent,bbn.vue.resizerComponent],props:{placeholder:{type:String,default:"Search"},source:{type:[String,Array,Function],default(){return[];}},shortcuts:{type:Boolean,default:false},search:{type:Boolean,default:true},menus:{type:Array,default(){return[];}},current:{type:String}},data(){let isAjax=!Array.isArray(this.source)
...
let obj={url:node.data.link,icon:node.icon,text:node.text,id:node.data.id};return[{text:bbn._('Create a shortcut'),icon:'nf nf-fa-external_link_alt',action:()=>{this.$emit('shortcut',obj);}}];},mapSrc(data,level){data.cls='bbn-treemenu-'+(level>6?x:level);if(level<3){data.cls+=' bbn-bottom-sspace';}
...
else{this.reset();}})},focusSearch(){if(!bbn.fn.isMobile()){let search=this.getRef('search');if(search){search.focus();}}}},mounted(){this.onResize();this.ready=true;},watch:{currentMenu(val){if((val!==null)&&bbn.fn.isVue(this.getRef('tree'))){this.reset();}}}});})(bbn);if(bbn_resolve){bbn_resolve("ok");}})(bbn);}