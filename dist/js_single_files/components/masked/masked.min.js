((bbn)=>{let script=document.createElement('script');script.innerHTML=`<span :class="[componentClass, 'bbn-textbox', {'bbn-disabled': !!isDisabled}]">
  <input ref="element"
         :disabled="isDisabled"
         :required="required"
         :readonly="readonly"
...
`;script.setAttribute('id','bbn-tpl-component-masked');script.setAttribute('type','text/x-template');document.body.insertAdjacentElement('beforeend',script);(function(bbn){"use strict"
Vue.component('bbn-masked',{mixins:[bbn.vue.basicComponent,bbn.vue.inputComponent,bbn.vue.eventsComponent],props:{mask:{type:String,required:true,validator:val=>!!val.length},promptChar:{type:String,default:'_'},pattern:{type:String}},data(){return{escape:'\\',patterns:{0:{pattern:'[0-9]'},9:{pattern:'[0-9\\s]'},'#':{pattern:'[0-9\\s\\+\\-]'},'L':{pattern:'[a-zA-Z]'},'?':{pattern:'[a-zA-Z\\s]'},'&':{pattern:'[0-9a-zA-Z\\S]'},'C':{pattern:'.'},'A':{pattern:'[0-9a-zA-Z]'},'a':{pattern:'[0-9a-zA-Z\\s]'},'.':{static:bbn.env&&bbn.env.money&&bbn.env.money.decimal?bbn.env.money.decimal:'.'},',':{static:bbn.env&&bbn.env.money&&bbn.env.money.thousands?bbn.env.money.thousands:','},'$':{static:bbn.env&&bbn.env.money&&bbn.env.money.currency?bbn.env.money.currency:'€'}},inputValue:''}},computed:{escapePos(){if(!this.mask){return[];}
...
return!!(this.patterns[k]&&this.patterns[k].pattern&&char.match(this.patterns[k].pattern)&&(char!==this.promptChar))},isSpecialKey(keyCode){switch(keyCode){case 8:case 9:case 16:case 18:case 35:case 36:case 37:case 39:case 46:return true
...
if(i<=end){idxEnd++}}}
return{start:idxStart<0?0:idxStart,end:idxEnd}},keydownEvent(event){if(!this.isDisabled&&!this.readonly){if(!this.isShiftKey(event.keyCode)&&!this.isControlKey(event.keyCode)&&!this.isArrowKey(event.keyCode)&&!this.isTabKey(event.keyCode)&&!event.ctrlKey&&!this.isEnterKey(event.keyCode)){let isSelection=this.$refs.element.selectionStart!==this.$refs.element.selectionEnd,value=this.value?this.value.toString():''
...
this.keydown(event)}},keyupEvent(event){if(!this.isDisabled&&!this.readonly){if(!this.isShiftKey(event.keyCode)&&!this.isControlKey(event.keyCode)&&!this.isTabKey(event.keyCode)&&!event.ctrlKey){let pos=this.$refs.element.selectionStart
...
this.$nextTick(()=>{this.emitInput(this.raw())
this.$refs.element.setSelectionRange(pos,pos)})}},blurEvent(event){if(!this.isDisabled&&!this.readonly){if(!this.value){this.inputValue='';}
this.blur(event)}},focusEvent(event){if(!this.isDisabled&&!this.readonly){this.setInputValue()
this.$nextTick(()=>{this.$refs.element.setSelectionRange(0,0)})
this.focus(event)}},pasteEvent(event){if(!this.isDisabled&&!this.readonly){let text=event.clipboardData?event.clipboardData.getData('text'):'',pos=this.getPos(this.$refs.element.selectionStart),p=this.getIdxRange(0,pos),val=this.value.toString()
event.preventDefault()
...
this.$nextTick(()=>{this.$refs.element.setSelectionRange(pos,pos)})}},cutEvent(event){if(!this.isDisabled&&!this.readonly){let sel=document.getSelection(),text=sel.toString(),oriPos=this.$refs.element.selectionStart,pos=this.getPos(oriPos),p=this.getIdxRange(0,pos),val=this.value.toString()
event.preventDefault()
...
if(value){bbn.fn.each([...value],(c,i)=>{if(!this.bannedPos.includes(i)&&this.patterns[this.mask[this.posLink[i]]]&&this.patterns[this.mask[this.posLink[i]]].pattern){if(c.match(this.patterns[this.mask[this.posLink[i]]].pattern)&&(c!==this.promptChar)){ret+=c}}})}
return ret}},mounted(){if(this.value){this.setInputValue()}
this.ready=true},watch:{value(newVal,oldVal){if(newVal!==oldVal){this.setInputValue()}},mask(){this.$nextTick(()=>{let pos=this.value.length;this.getRef('element').setSelectionRange(pos,pos);});}}});})(bbn);})(bbn);