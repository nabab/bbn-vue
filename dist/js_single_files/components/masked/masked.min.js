...
this.keydown(event)}},keyupEvent(event){if(!this.disabled&&!this.readonly){bbn.fn.log('ciao',event)
if(!this.isShiftKey(event.keyCode)&&!this.isControlKey(event.keyCode)&&!this.isTabKey(event.keyCode)&&!event.ctrlKey){bbn.fn.log('ciao2',event)
...
this.ready=true},watch:{value(newVal,oldVal){if(newVal!==oldVal){this.setInputValue()}}}});})(bbn);})(bbn);