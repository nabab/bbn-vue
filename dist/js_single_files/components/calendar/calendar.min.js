((bbn)=>{let script=document.createElement('script');script.innerHTML=`<div :class="[componentClass, 'bbn-w-100']"
     v-show="ready"
>
...
return obj;},_makeDays(){let items=[],c=moment(this.currentDate.format('YYYY-MM-01')),sunday=moment(c).day('Sunday').weekday();for(let i=1;i<=6;i++){if(i>1){c.add(6,'d');}
items=items.concat(Array.from({length:7},(v,k)=>{let w=c.weekday(k),val=w.format(this.currentCfg.valueFormat),isHidden=!this.extraItems&&(w.get('month')!==this.currentDate.get('month'))?true:false;return this._makeItem(w.get('date'),val,isHidden,k===sunday,false,this.extraItems&&(w.get('month')!==this.currentDate.get('month')));}));}
this.gridStyle='grid-template-columns: repeat(7, 1fr); grid-template-rows: max-content repeat(6, 1fr);';this.currentLabelsDates=Array.from({length:7},(v,i)=>moment(this.currentDate).weekday(i));this.$set(this,'items',items);},_makeWeeks(){let c=moment(this.currentDate),sunday=moment(c).day('Sunday').weekday(),items=Array.from({length:7},(v,k)=>{let w=c.weekday(k);return this._makeItem(w.get('date'),w.format(this.currentCfg.valueFormat),false,k===sunday,false,false);});this.gridStyle='grid-template-columns: repeat(7, 1fr); grid-template-rows: max-content auto';this.currentLabelsDates=Array.from({length:7},(v,i)=>moment(this.currentDate).weekday(i));this.$set(this,'items',items);},_makeMonths(){let c=moment(this.currentDate.format('YYYY-01-01')),items=Array.from({length:12},(v,k)=>{let w=c.month(k);return this._makeItem(w.format('MMM'),w.format(this.currentCfg.valueFormat),!this.extraItems&&(w.get('year')!==this.currentDate.get('year')),false,false,this.extraItems&&(w.get('year')!==this.currentDate.get('year')));});this.gridStyle='grid-template-columns: repeat(3, 1fr); grid-template-rows: repeat(4, 1fr);';this.currentLabelsDates=[];this.$set(this,'items',items);},_makeYears(){let c=moment(this.currentDate.format('YYYY-01-01')),year=c.format('YYYY'),items=Array.from({length:12},(v,k)=>{let w=c.year(year-6+k);return this._makeItem(w.format('YYYY'),w.format(this.currentCfg.valueFormat),false,false,k===0||k===11,false);});this.gridStyle='grid-template-columns: repeat(3, 1fr); grid-template-rows: repeat(4, 1fr);';this.currentLabelsDates=[];this.$set(this,'items',items);},getCfg(type){let m=type||this.type,cfg={};switch(m){case'days':bbn.fn.extend(cfg,{make:this._makeDays,step:[1,'M'],stepSkip:[1,'y'],stepEvent:[1,'d'],stepText:bbn._('month'),stepSkipText:bbn._('year'),titleFormat:'MMM YYYY',valueFormat:'YYYY-MM-DD',labelsFormatDefault:'ddd',labelsFormatLetter:'dd',labelsFormatAbbr:'ddd',labelsFormatFull:'dddd',startFormat:'YYYY-MM-01',endFormat:()=>{return'YYYY-MM-'+this.currentDate.daysInMonth()},startExtra:[7,'d'],endExtra:[15,'d']});break;case'weeks':bbn.fn.extend(cfg,{make:this._makeWeeks,step:[1,'w'],stepEvent:[1,'w'],stepText:bbn._('week'),titleFormat:'MMM YYYY',valueFormat:'YYYY-MM-DD',labelsFormatDefault:'ddd',labelsFormatLetter:'dd',labelsFormatAbbr:'ddd',labelsFormatFull:'dddd',startFormat:()=>{return moment(this.currentDate).weekday(0).format('YYYY-MM-DD');},endFormat:()=>{return moment(this.currentDate).weekday(6).format('YYYY-MM-DD');}});break;case'months':bbn.fn.extend(cfg,{make:this._makeMonths,step:[1,'y'],stepEvent:[1,'M'],stepText:bbn._('year'),titleFormat:'YYYY',valueFormat:'YYYY-MM',startFormat:'YYYY-01-01',endFormat:'YYYY-12-31'});break;case'years':bbn.fn.extend(cfg,{make:this._makeYears,step:[10,'y'],stepEvent:[1,'y'],stepText:bbn._('decade'),titleFormat:()=>{let from=this.currentDate.format('YYYY')-5,to=from+9;return from+'-'+to;},valueFormat:'YYYY',startFormat:()=>{return(this.currentDate.format('YYYY')-5)+'-01-01';},endFormat:()=>{return(this.currentDate.format('YYYY')+4)+'-12-31';}});break;}
...
return false;},create(){if(!this.ready){this.$once('dataloaded',()=>{this.init();this.$nextTick(()=>{if(!this.date&&(this.max||this.min)){if(this.max&&!this.min&&(this.max<this.currentDate.format(this.currentCfg.valueFormat))){this.currentDate=moment(this.max,this.currentCfg.valueFormat);}
if(this.min&&!this.max&&(this.min>this.currentDate.format(this.currentCfg.valueFormat))){this.currentDate=moment(this.min,this.currentCfg.valueFormat);}
this.currentCfg.make();this.setTitle();}
this.ready=true;});});this.updateData();}},makeEvents(){this.$set(this,'events',{});bbn.fn.each(this.currentData,d=>{let tmpStart=moment(d.data[this.startField],this.startFormat).format(this.currentCfg.valueFormat),tmpEnd=moment(d.data[this.endField],this.endFormat).format(this.currentCfg.valueFormat);if(this.events[tmpStart]===undefined){this.events[tmpStart]=[];}
this.events[tmpStart].push(d.data);if(tmpStart!==tmpEnd){let mom=moment(tmpStart,this.currentCfg.valueFormat).add(...this.currentCfg.stepEvent),tmp=mom.format(this.currentCfg.valueFormat);while(tmp<=tmpEnd){if(this.events[tmp]===undefined){this.events[tmp]=[];}
this.events[tmp].push(d.data);mom.add(...this.currentCfg.stepEvent);tmp=mom.format(this.currentCfg.valueFormat);}}});},init(){if(this.currentCfg&&bbn.fn.isFunction(this.currentCfg.make)){if(this.selection&&this.autoSelection&&this.currentCfg.valueFormat){this.currentValue=this.value?moment(this.value,this.currentCfg.valueFormat).format(this.currentCfg.valueFormat):'';this.$emit('input',this.currentValue);}
...
this.$once('dataloaded',()=>{this.init();});this.updateData();},next(skip){skip=typeof skip==='boolean'?skip:false;if(this.currentCfg&&this.currentCfg.step&&bbn.fn.isFunction(this.currentCfg.make)){let check=moment(this.currentDate).add(...this.currentCfg[skip&&this.currentCfg.stepSkip?'stepSkip':'step']);if(this.max&&(check.format(this.currentCfg.valueFormat)>this.max)){this.currentDate=moment(this.max,this.currentCfg.valueFormat);}
else{this.currentDate.add(...this.currentCfg[skip&&this.currentCfg.stepSkip?'stepSkip':'step']);}
let ev=new Event('next',{cancelable:true});this.$emit('next',ev,this);if(ev.defaultPrevented){return false;}
this.refresh();}},prev(skip){skip=typeof skip==='boolean'?skip:false;if(this.currentCfg&&this.currentCfg.step&&bbn.fn.isFunction(this.currentCfg.make)){let check=moment(this.currentDate).subtract(...this.currentCfg[skip&&this.currentCfg.stepSkip?'stepSkip':'step']);if(this.min&&(check.format(this.currentCfg.valueFormat)<this.min)){this.currentDate=moment(this.min,this.currentCfg.valueFormat);}
else{this.currentDate.subtract(...this.currentCfg[skip&&this.currentCfg.stepSkip?'stepSkip':'step']);}
let ev=new Event('prev');this.$emit('prev',ev,this);if(ev.defaultPrevented){return false;}
this.refresh();}},select(val,notEmit){if(this.selection&&val){val=val===this.currentValue?'':val;this.currentValue=val;this.$emit('input',val);if(!notEmit){this.$emit('selected',val,this);}}},getPostData(){let start=moment(this.currentDate.format(bbn.fn.isFunction(this.currentCfg.startFormat)?this.currentCfg.startFormat():this.currentCfg.startFormat)),end=moment(this.currentDate.format(bbn.fn.isFunction(this.currentCfg.endFormat)?this.currentCfg.endFormat():this.currentCfg.endFormat)),data={};if(this.extraItems){if(this.currentCfg.startExtra){start.subtract(...this.currentCfg.startExtra);}
...
else{this.currentLabels=[];}},onResize(){return new Promise((resolve,reject)=>{this.setLabels(this.currentLabelsDates);resolve();});}},beforeCreate(){if(bbn.env&&bbn.env.lang&&(bbn.env.lang!==moment.locale())){moment.locale(bbn.env.lang);}},mounted(){this.create();},watch:{type(newVal){this.ready=false;this.create();},currentLabelsDates(newVal){this.setLabels(newVal);},value(newVal,oldVal){if(newVal!==oldVal){this.currentValue=newVal;}},currentData(){this.init();}}})})(bbn);})(bbn);