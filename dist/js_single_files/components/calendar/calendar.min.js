...
return obj;},_makeDays(){let items=[],c=dayjs(this.currentDate.format('YYYY-MM-01')),sunday=dayjs(c).day('Sunday').weekday();for(let i=1;i<=6;i++){if(i>1){c.add(6,'d');}
...
this.$once('dataloaded',()=>{this.init();});this.updateData();},next(skip){skip=typeof skip==='boolean'?skip:false;if(this.currentCfg&&this.currentCfg.step&&bbn.fn.isFunction(this.currentCfg.make)){let check=dayjs(this.currentDate).add(...this.currentCfg[skip&&this.currentCfg.stepSkip?'stepSkip':'step']);if(this.max&&(check.format(this.currentCfg.valueFormat)>this.max)){this.currentDate=dayjs(this.max,this.currentCfg.valueFormat);}
...
this.refresh();}},prev(skip){skip=typeof skip==='boolean'?skip:false;if(this.currentCfg&&this.currentCfg.step&&bbn.fn.isFunction(this.currentCfg.make)){let check=dayjs(this.currentDate).subtract(...this.currentCfg[skip&&this.currentCfg.stepSkip?'stepSkip':'step']);if(this.min&&(check.format(this.currentCfg.valueFormat)<this.min)){this.currentDate=dayjs(this.min,this.currentCfg.valueFormat);}
