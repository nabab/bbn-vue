((bbn)=>{let script=document.createElement('script');script.innerHTML=`<div :class="['bbn-overlay' , componentClass, {'bbn-unselectable': isSorting}]"
...
`;script.setAttribute('id','bbn-tpl-component-dashboard');script.setAttribute('type','text/x-template');document.body.insertAdjacentElement('beforeend',script);(function(bbn){"use strict";var limits=[5,10,15,20,25,30,40,50];Vue.component('bbn-dashboard',{mixins:[bbn.vue.basicComponent,bbn.vue.resizerComponent,bbn.vue.localStorageComponent],props:{components:{type:Object,default(){return{};}},max:{type:Number},selectable:{type:Boolean,default:true},closable:{type:Boolean,default:true},sortable:{type:Boolean,default:true},scrollable:{type:Boolean,default:true},source:{default(){return[];}},url:{},loadedConfig:{type:Object},order:{type:Array,default(){return[];}},cfg:{type:Object,default(){return{sortable:true,url:false,source:[],components:{}};}}},data(){return{originalSource:[],menu:[],isRefreshing:false,widgets:[],currentOrder:this.order.slice(),sortTargetIndex:null,sortOriginIndex:null,isSorting:false,isDragging:false,sortTimeout:false,sortingElement:false,sortHelperWidth:0,sortHelperHeight:0,sortHelperX:0,sortHelperY:0,currentSlots:[],resizeTimeout:false,numCols:1};},computed:{originalOrder(){return bbn.fn.map(this.originalSource,d=>d.key);},isOrderChanged(){if(!this.originalOrder){return false;}
return JSON.stringify(this.originalOrder)!==JSON.stringify(this.currentOrder);}},methods:{setConfig(uid,config){this.setStorage({order:config.order},uid);},closeWidget(uid,widget){let ev=new Event('close',{cancelable:true});this.$emit('close',uid,widget);if(!ev.defaultPrevented){this.updateWidget(uid,{hidden:true});}},getWidget(key){let idx=bbn.fn.search(this.widgets,{key:key});if(idx>-1){return this.widgets[idx];}
...
order.push(a.key);});this.currentOrder=order;if(this.url){return this.post(this.url+'order',{id_dashboard:this.code,order:order},d=>{if(d&&d.data&&d.data.success){appui.success();}
else{appui.error();}});}
...
else if(this.sortTargetIndex!==null){this.sortTargetIndex=null;}},updateWidget(key,cfg){let idx=bbn.fn.search(this.widgets||[],'key',key),params={id:key,cfg:bbn.fn.extend({},cfg),id_dashboard:this.code},no_save=['items','num','start','index'];if(idx>-1){bbn.fn.each(no_save,function(a,i){if(cfg[a]!==undefined){delete params.cfg[a];}});let numProps=bbn.fn.numProperties(params.cfg);if(numProps){if((numProps===1)&&params.cfg.limit&&(this.widgets[idx].limit===undefined)){this.widgets[idx].limit=params.cfg.limit;return;}
bbn.fn.iterate(params.cfg,(a,k)=>{if(this.widgets[idx][k]===undefined){this.$set(this.widgets[idx],k,a);}
...
if(!this.sortable){obj.sortable=false;}
