((bbn)=>{let script=document.createElement('script');script.innerHTML=`<bbn-scroll axis="x" :class="componentClass" ref="scroll">
  <!-- Each tree pane -->
...
let loadUrl=this.closest('bbn-appui').find('bbn-loadbar').currentItem.url;if(bbn.fn.isNumber(i)&&(loadUrl==='ide/finder')){bbn.fn.abort(this.closest('bbn-appui').find('bbn-loadbar').currentItem.key);appui.success(bbn._('Current request aborted'))
this.currentFile=false;this.dirs.splice(i,1);}
else if(loadUrl.indexOf('ide/actions/finder/file')===0){this.currentFile=false;this.isLoading=false;bbn.fn.abort(this.closest('bbn-appui').find('bbn-loadbar').currentItem.key);}},context(a,button){this.currentContextPath=button.closest('bbn-context').data.path;},closePreview(){this.currentFile=false;this.isLoading=false;},refresh(name){let trees=this.findAll('bbn-tree');if(trees.length){let tree=bbn.fn.filter(trees,(a)=>{return a.data.name===name;})
...
else if(!this.copied){this.alert(bbn._('The clipboard is empty!'));}},download(n){bbn.fn.postOut(this.root+'actions/finder/download/'+n.data.value,{value:n.data.value,file:n.data.file,path:this.currentPath!==n.data.value+'/'?this.currentPath:'',origin:this.origin,destination:this.origin+'download/'+moment().format('x')+'/'})},edit(node){        this.editingNode=false;let oldValue=node.data.value,tmp=node.closest('bbn-tree').data.path,path='';if(tmp.indexOf('/')===0){path=tmp.substr(1,tmp.length);}
else{path=tmp+'/';}
...
setTimeout(()=>{this.add('');},250);},dirs(){this.updateScroll();},currentFile(){this.updateScroll();},currentPath(v){this.$emit('change',v);}},components:{form:{name:'form',template:`
        ...
        </bbn-form>`,props:['source','data'],data(){return{dirIdx:false,}},computed:{dirs(){return this.closest('bbn-container').getComponent().dirs;},finder(){return this.closest('bbn-container').find('bbn-finder')}},        methods:{success(d){if(d.success){let trees=this.closest('bbn-container').getComponent().findAll('bbn-tree');if(d.data&&d.data.new_dir){bbn.fn.happy('mpod')
