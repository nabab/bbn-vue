...
return size;}
return 0;},maxSliderPos(){return this.shouldBother?this.containerSize-this.sliderSize-this.offsetStart-this.offsetEnd:0;},offsetStart(){return(bbn.fn.isArray(this.offset)?this.offset[0]:this.offset);},offsetEnd(){return(bbn.fn.isArray(this.offset)?this.offset[1]:this.offset);},barStyle(){return{opacity:this.show&&this.shouldBother?1:0,[this.isVertical?'top':'left']:this.offsetStart+'px',[this.isVertical?'right':'bottom']:this.offsetEnd+'px'}},sliderStyle(){let res={};if(this.shouldBother){res[this.isVertical?'height':'width']=this.sliderSize+'px';res[this.isVertical?'top':'left']=this.sliderPos+'px';if(this.color){res.backgroundColor=this.color;}}
...
this.setSliderPos(this.containerPos*this.ratio);bbn.fn.each(this.scrollableElements(),a=>{if(a!==container){a[prop]=this.containerPos;}});this.overContent();}},adjustFromBar(anim){if(this.shouldBother){this.$nextTick(()=>{this.containerPos=(this.sliderPos / this.ratio);let prop=this.isVertical?'scrollTop':'scrollLeft';if(this.scroller){this.scrollTo(this.containerPos,anim);}
else{this.realContainer[prop]=this.containerPos;bbn.fn.each(this.scrollableElements(),a=>{a[prop]=this.containerPos;});}
let e=new Event('scroll');this.$emit('scroll'+(this.isVertical?'y':'x'),e,this.containerPos);})}},jump(e,precise){if(this.realContainer){let isRail=e.target===this.$el;if(isRail){let position=this.$refs.scrollSlider.getBoundingClientRect();let clickPoint=this.isVertical?e.pageY:e.pageX;let isBefore=clickPoint<position[this.isVertical?'top':'left'];let isAfter=clickPoint>position[this.isVertical?'bottom':'right'];if(isBefore||isAfter){let movement=isBefore?-(position[this.isVertical?'top':'left']-clickPoint):clickPoint-(position[this.isVertical?'top':'left'])-(position[this.isVertical?'height':'width']);if(!precise){if(isBefore){this.scrollBefore(true);}
else{this.scrollAfter(true);}}
else{this.scrollTo(Math.round((this.sliderPos+movement)/ this.ratio));}}}}},scrollLevel(before,anim){if(this.sliderSize){let movement=Math.round(this.sliderSize-(this.sliderSize*0.1));if(before){movement=-movement;}
this.scrollTo(Math.round((this.sliderPos+movement)/ this.ratio),true);}},scrollBefore(anim){return this.scrollLevel(true,anim);},scrollAfter(anim){return this.scrollLevel(false,anim);},scrollableElements(){let tmp=this.scrollAlso;if(bbn.fn.isFunction(tmp)){tmp=tmp();}
...
this.nextLevel=Math.round(newPos);this.realContainer['scroll'+(this.isVertical?'Top':'Left')]=this.nextLevel;},1000 / 60);});},scrollTo(val,anim){return new Promise(resolve=>{if(this.shouldBother){if(this.animationInterval){clearInterval(this.animationInterval);}
...
if(bbn.fn.isNumber(num)){if(num<0){num=0;}
else if(num>(this.contentSize-this.containerSize+100)){num=this.contentSize-this.containerSize;}
this.containerPos=num;if(anim){this.smoothScrollTo(num).then(()=>{resolve();});}
else{this.sliderPos=this.containerPos*this.ratio;this.nextLevel=Math.round(num);this.realContainer['scroll'+(this.isVertical?'Top':'Left')]=num;resolve();}}}});},scrollStart(anim){this.scrollTo(0,anim);},scrollEnd(anim){this.scrollTo(this.contentSize-this.containerSize,anim);}},watch:{isOverSlider(v){if(!this.dragging){if(!v){this.overContent();}
else if(!this.show){this.showSlider();}}},container(){this.initContainer();},show(v){if(v){this.onResize();}},sliderPos(){this.showSlider();},dragging(v){if(this.scroller){this.scroller.isDragging=v}}},created(){this.componentClass.push(this.orientation);},mounted(){this.initContainer();document.addEventListener("mousemove",this.onDrag);document.addEventListener("touchmove",this.onDrag);document.addEventListener("mouseup",this.stopDrag);document.addEventListener("touchend",this.stopDrag);this.onResize();this.overContent();},beforeDestroy(){if(this.realContainer&&this.isInit){if(!this.container&&this.scroller){this.scroller.$off("resize",this.onResize);this.scroller.$off("scroll",this.adjustFromContainer);this.scroller.$off("mousemove",this.overContent);}
...
bbn.fn.each(this.scrollableElements(),a=>{a.removeEventListener('scroll',this.adjustFromContainer,{passive:true});a.removeEventListener('mousemove',this.overContent,{passive:true});});}
document.removeEventListener("mousemove",this.onDrag);document.removeEventListener("touchmove",this.onDrag);document.removeEventListener("mouseup",this.stopDrag);document.removeEventListener("touchend",this.stopDrag);},});})(window.bbn,window.Vue);})(bbn);