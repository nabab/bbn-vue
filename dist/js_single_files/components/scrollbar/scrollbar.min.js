...
this.nextLevel=Math.round(newPos);this.realContainer['scroll'+(this.isVertical?'Top':'Left')]=this.nextLevel;},1000 / 60);});},scrollTo(val,anim=false){return new Promise(resolve=>{if(this.shouldBother){if(this.animationInterval){clearInterval(this.animationInterval);}
...
else{this.sliderPos=this.containerPos*this.ratio;this.nextLevel=Math.round(num);this.realContainer['scroll'+(this.isVertical?'Top':'Left')]=num;resolve();}}}});},scrollStart(){this.scrollTo(0);},scrollEnd(){this.scrollTo(this.contentSize-this.containerSize);}},watch:{isOverSlider(v){if(!this.dragging){if(!v){this.overContent();}
