...
else{anim.width=this.size+'%';anim.left=this.position+'%';}}},scrollTo(val){if(this.shouldBother){let num=0;let ele=false;if(bbn.fn.isVue(val)&&val.$el){ele=val.$el;}
...
else if(num>(this.contentSize-this.containerSize)){num=this.contentSize-this.containerSize;}
this.realContainer['scroll'+(this.isVertical?'Top':'Left')]=num;this.containerPos=num;this.sliderPos=this.containerPos*this.ratio;}}},scrollStart(){this.scrollTo(0);},scrollEnd(){this.scrollTo(this.contentSize-this.containerSize);}},watch:{isOverSlider(v){if(!this.dragging){if(!v){this.overContent();}
