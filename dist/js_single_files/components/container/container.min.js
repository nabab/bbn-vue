...
`;script.setAttribute('id','bbn-tpl-component-container');script.setAttribute('type','text/x-template');document.body.insertAdjacentElement('beforeend',script);(function(bbn,Vue){"use strict";let componentsList=[];Vue.component("bbn-container",{name:'bbn-container',mixins:[bbn.vue.basicComponent,bbn.vue.resizerComponent,bbn.vue.viewComponent,bbn.vue.observerComponent],props:{idx:{type:Number},last:{type:Number},uid:{type:String,default(){return bbn.fn.randomString();}},visual:{type:Boolean,default:false}},data(){return{isVisible:this.loading,router:null,visible:this.loading,dirty:false,isComponent:null,fullScreen:false,componentName:this.randomName(),popups:[],isComponentActive:false,isLoaded:!this.load||this.loaded,isPinned:this.pinned,isStatic:this.static,currentURL:this.current||this.url,isOver:false,_bbn_container:null,thumbnail:false,forms:[],errorStatus:null,currentTitle:this.title,currentIcon:this.icon,currentIndex:this.idx};},computed:{visualStyle(){if(this.visual){let r=this.router;if((r.views.length>1)&&(!this.visible||r.visualShowAll)){return{zoom:0.1,width:'100%',height:'auto',overflow:'hidden'};}
...
return true;},close(){this.router.close(this.currentIndex);},setCurrent(url){if(url.indexOf(this.url)===0){this.currentURL=url;return true;}
...
if(fcolor){this.router.$set(view,"fcolor",fcolor);}}}},popup(){let popup=this.getPopup();return arguments.length?popup.open.apply(popup,arguments):popup;},getComponent(){return this.getRef('component');},enter(){this.$parent.enter(this);},pin(){this.router.pin(this.currentIndex);},unpin(){this.router.unpin(this.currentIndex);},reload(){this.router.reload(this.currentIndex);},addMenu(obj){if((this.currentIndex>-1)&&obj.text&&this.$parent.views&&this.$parent.views[this.currentIndex]){if(this.$parent.views[this.currentIndex].menu===undefined){this.$parent.views[this.currentIndex].menu=[];}
let menu=this.$parent.views[this.currentIndex].menu||[],idx=bbn.fn.isFunction(menu)?-1:bbn.fn.search(menu||[],{text:obj.text});if(idx===-1){if(bbn.fn.isFunction(menu)){this.$parent.views[this.currentIndex].menu=()=>{let items=menu()||[];if(bbn.fn.search(items,obj)===-1){if(!obj.key){obj.key=bbn.fn.randomInt(99999,99999999999);}
items.push(obj);}
return items;};}
...
return false;},deleteMenu(key){if((this.currentIndex>-1)&&this.router.views&&this.router.views[this.currentIndex]){let menu=this.router.views[this.currentIndex].menu||[];if(bbn.fn.isFunction(menu)){menu=()=>{let items=menu()||[];let idx=bbn.fn.search(items,"key",key);if(idx>-1){items.splice(idx,1);this.router.views[this.currentIndex].menu=items;this.$parent.$forceUpdate();return true;}};}
else{let idx=bbn.fn.search(menu,"key",key);if(idx>-1){menu.splice(idx,1);this.router.views[this.currentIndex].menu=menu;this.$parent.$forceUpdate();return true;}}}
...
this.ready=true;}},showMenu(){return this.router.getMenuFn(this.currentIndex);},setScreenshot(){if(!this._screenshotInterval&&this.visual&&this.router.db){let url=this.getFullURL();this.router.db.selectOne('containers','time',{url:url}).then(time=>{if((bbn.fn.timestamp()-(time||0))>3600000){this.saveScreenshot(0.1,10000);}}).catch(()=>{this.saveScreenshot(0.1,10000);});this._screenshotInterval=setInterval(()=>{this.saveScreenshot(0.1);},300000);}},unsetScreenshot(){if(this._screenshotInterval){clearInterval(this._screenshotInterval);this._screenshotInterval=false;if(this._screenshotTimeout){clearTimeout(this._screenshotTimeout);this._screenshotTimeout=false;}}},async saveScreenshot(scale=0.1,timeout=0){if(this.router.db){let img=await this.takeScreenshot(scale,timeout,true);let num_tries=0;while(!img&&(num_tries<5)){num_tries++;img=await this.takeScreenshot(scale,5000);}
if(!img){throw new Error(bbn._("Impossible to take the screenshot of "+this.getFullCurrentURL()));}
...
this._screenshotTimeout=setTimeout(()=>{let exit=()=>{this._screenshotTimeout=false;resolve(false);};if((this.currentIndex===this.router.selected)&&this.visible&&window.html2canvas&&bbn.fn.isActiveInterface(600)&&!this.router.visualShowAll){let scroll=this.getRef('scroll');if(!scroll){return exit();}
if(scroll.$el){scroll=scroll.$el;}
let w=scroll.clientWidth;let h=scroll.clientHeight;let ct=this.getRef('canvasSource');if(!ct||!w||!h){return exit();}
ct.style.width=w+'px !important';ct.style.height=h+'px !important';bbn.fn.log("Screenshot of "+this.getFullURL());html2canvas(ct,{width:w,height:h,scale:scale}).then(canvas=>{ct.style.width=null;ct.style.height=null;this._screenshotTimeout=false;if(!image){resolve(canvas);return;}
...
else{exit();}},timeout)})},updateScreenshot(){if(this.visual&&this.router.db){let url=this.getFullURL();this.router.db.selectOne('containers','image',{url:url}).then(res=>{if(res){this.thumbnail=res;}});}}},created(){this.componentClass.push('bbn-resize-emitter');if(this.isComponent){componentsList.push(this.componentName);}
...
else{this.unsetScreenshot();}}},idx(v){if(this.visual){this.isOver=false;}
...
this.$nextTick(()=>{this.selfEmit(true)})},dirty(v){let view=this.router.getView(this.url);if(view){view.dirty=v;this.router.retrieveDirtyContainers();}},isInit(v){if(!v){this.isLoaded=!this.load||this.loaded;this.dirty=false;}}},});})(bbn,Vue);})(bbn);