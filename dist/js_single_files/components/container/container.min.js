...
this.ready=true;}}},created(){this.componentClass.push('bbn-resize-emitter');if(this.isComponent){componentsList.push(this.componentName);}
...
if(!this.router.ready){this.router.$on('ready',()=>{bbn.fn.log("ROUTER NOT READY")
this.router.register(this);});}
else{bbn.fn.log("ROUTER READY")
this.router.register(this);}},beforeDestroy(){this.router.unregister(this);if(this.isComponent){let idx=componentsList.indexOf(this.componentName);if(idx>-1){componentsList.splice(idx,1);}}},watch:{title(v){this.currentTitle=v;},current(newVal){if(newVal.indexOf(this.url)===0){this.currentURL=newVal;}},currentURL(newVal,oldVal){if(!newVal||(newVal.indexOf(this.url)!==0)){this.currentURL=this.url;}},ready(v){bbn.fn.log("READY CONTAINER "+this.url);if(v){if(this.onMount){this.onMount();}}},load(nv,ov){},visible(nv,ov){this.$nextTick(()=>{this.$emit(nv?'view':'unview',this);if(nv){this.$nextTick(()=>{this.onResize();});}});},content(newVal,oldVal){if(newVal){this.isComponentActive=false;}},fullScreen(newVal){let fn=(e)=>{if(e.keyCode===27){this.fullScreen=false;}};if(newVal){document.body.addEventListener('keydown',fn);}
else{document.body.removeEventListener('keydown',fn);}
this.$nextTick(()=>{this.selfEmit(true)})},dirty(v){this.router.views[this.currentIndex].dirty=v;this.router.retrieveDirtyContainers();}},});})(bbn,Vue);})(bbn);