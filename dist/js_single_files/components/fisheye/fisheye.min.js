((bbn)=>{let script=document.createElement('script');script.innerHTML=`<div :class="[componentClass, 'bbn-100']"
     :style="{zIndex: zIndex}">
...
</div>`;script.setAttribute('id','bbn-tpl-component-fisheye');script.setAttribute('type','text/x-template');document.body.insertAdjacentElement('beforeend',script);(function(bbn){"use strict";Vue.component('bbn-fisheye',{mixins:[bbn.vue.basicComponent,bbn.vue.resizerComponent,bbn.vue.listComponent],props:{source:{type:Array,default(){return[];}},removable:{type:Boolean,default:false},fixedLeft:{type:Array,default(){return[];}},fixedRight:{type:Array,default(){return[];}},zIndex:{type:Number,default:1},scrollable:{type:Boolean,default:false},mobile:{type:Boolean,default:false}},data(){return{menu:false,widget:false,overBin:false,droppableBin:false,timeout:false,binTimeout:false,visibleBin:false,visibleText:-1,draggedIdx:-1,showIcons:!this.mobile,visibleFloater:false,floaterTop:0};},computed:{items(){let items=[];let i=0;bbn.fn.each(this.fixedLeft,a=>{items.push({data:a,fixed:true,index:i});i++;});bbn.fn.each(this.filteredData,a=>{items.push({data:a.data,fixed:false,index:i});i++;});bbn.fn.each(this.fixedRight,a=>{items.push({data:a,fixed:true,index:i});i++;});return items;},binPosition(){return this.showIcons?'top: 15rem':'bottom: calc(-'+bbn.env.height+'px + 5rem)';}},methods:{onClick(it){if(it.url){bbn.fn.link(it.url);}
if(it.action&&bbn.fn.isFunction(it.action)){it.action();}
this.visibleFloater=false;},mouseover(idx){if(!bbn.fn.isMobile()&&(this.visibleText!==idx)){clearTimeout(this.timeout);this.visibleText=-1;this.timeout=setTimeout(()=>{this.visibleText=idx;},500);}},mouseout(){clearTimeout(this.timeout);this.visibleText=-1;},dragleave(){setTimeout(()=>{this.overBin=false;},500);},dragstart(idx,e){if(this.removable){this.draggedIdx=idx;this.visibleBin=true;}
...
this.visibleFloater=!this.visibleFloater;},onScrollMounted(){this.$nextTick(()=>{this.checkMeasures();})}},mounted(){this.setResizeMeasures();this.setContainerMeasures();this.$nextTick(()=>{this.ready=true;this.checkMeasures();})},watch:{source(){this.updateData();},lastKnownWidth(newVal){this.checkMeasures();},items(){if(this.ready){this.$nextTick(()=>{this.checkMeasures();})}}}});})(bbn);})(bbn);