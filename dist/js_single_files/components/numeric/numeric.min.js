((bbn)=>{let script=document.createElement('script');script.innerHTML=`<div :class="[componentClass, 'bbn-iblock', 'bbn-textbox', {'bbn-disabled': !!isDisabled}]"
     @click.stop>
...
</div>`;script.setAttribute('id','bbn-tpl-component-numeric');script.setAttribute('type','text/x-template');document.body.insertAdjacentElement('beforeend',script);(function(bbn){"use strict";Vue.component('bbn-numeric',{mixins:[bbn.vue.basicComponent,bbn.vue.eventsComponent,bbn.vue.inputComponent],props:{decimals:{type:[Number,String],default:0},unit:{type:String,default:''},max:{type:[Number,String]},min:{type:[Number,String]},step:{type:Number,default:1},spinners:{type:Boolean,default:true},onlySpinners:{type:Boolean,default:false},inputmode:{type:String,default:'decimal'},suffix:{type:String}},data(){let isPercentage=this.unit==='%',decimals=isPercentage?parseInt(this.decimals)+2:this.decimals,value=this.value;if(!this.decimals&&this.step&&(this.step<1)){let step=this.step;while(step<1){step*=10;decimals++;}}
...
return{editMode:false,inputValue:value,currentValue:this.value===null?'':(bbn.fn.isNumber(this.value)?parseFloat(this.value).toFixed(this.decimals):this.value),currentDecimals:decimals}},computed:{disableDecrease(){return(bbn.fn.isNumber(this.min)&&(parseFloat(this.currentValue)<=this.min))||!!this.readonly||this.isDisabled;},disableIncrease(){return(bbn.fn.isNumber(this.max)&&(parseFloat(this.currentValue)>=this.max))||!!this.readonly||this.isDisabled;},pattern(){let p='^';if(!bbn.fn.isNumber(this.min)||(this.min<0)){p+='\\-?';}
...
this.keydown(e);},_focus(e){bbn.fn.log('aaaaaa');if(!this.isDisabled&&!this.readonly&&!this.onlySpinners){this.currentValue=this.value===null?'':(bbn.fn.isNumber(this.value)?parseFloat(this.value).toFixed(this.decimals):this.value);this.editMode=true;this.$nextTick(()=>{this.focus(e);this.selectText();})}},_blur(e){this.checkMinMax();this.checkDecimals();this.editMode=false;this.$nextTick(()=>{this.blur(e);})},_change(e){this.checkMinMax();this.checkDecimals();this.editMode=false;this.$nextTick(()=>{this.change(e);});},checkDecimals(){if(this.currentDecimals&&this.currentValue&&parseFloat(this.currentValue).toString().match('^\-?[0-9]+\.0{0,'+this.currentDecimals+'}[1-9]{0}$')){this.currentValue=parseFloat(this.currentValue).toFixed(0);}},checkMinMax(){let w=false;if(bbn.fn.isNumber(this.max)&&(this.currentValue!=='')&&(this.currentValue!==null)&&(parseFloat(this.currentValue)>parseFloat(this.max))){this.currentValue=parseFloat(this.max).toFixed(this.currentDecimals);w=true;}
