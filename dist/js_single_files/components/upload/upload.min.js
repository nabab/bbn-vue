((bbn)=>{let script=document.createElement('script');script.innerHTML=`<div :class="[componentClass, 'bbn-box', 'bbn-spadded']">
    <div v-if="ready"
         style="z-index: 2">
...
</div>`;script.setAttribute('id','bbn-tpl-component-upload');script.setAttribute('type','text/x-template');document.body.insertAdjacentElement('beforeend',script);((bbn)=>{"use strict";Vue.component('bbn-upload',{mixins:[bbn.vue.basicComponent,bbn.vue.inputComponent],props:{value:{type:[Array,String],default(){return[];}},saveUrl:{type:String,default:null},removeUrl:{type:String,default:null},downloadUrl:{type:String,default:null},autoUpload:{type:Boolean,default:true},multiple:{type:Boolean,default:true},max:{type:Number,default:0},disabled:{type:Boolean,default:false},editable:{type:Boolean,default:false},downloadable:{type:Boolean,default:false},eliminable:{type:Boolean,default:true},uploadable:{type:Boolean,default:true},icons:{type:Boolean,default:true},thumbs:{type:Boolean,default:true},thumbNot:{type:String},maxSize:{type:Number,default:60},thumbWaiting:{type:String},json:{type:Boolean,default:false},text:{type:Object,default(){return{}}},icon:{type:String,default:'nf nf-fa-upload'},extensions:{type:Array,default(){return[];}},accept:{type:String,default:'*'},paste:{type:Boolean,default:false},dragDrop:{type:Boolean,default:true},data:{type:Object},showFilesize:{type:Boolean,default:true}},data(){return{currentData:[],uploading:false};},computed:{currentText(){return bbn.fn.extend({uploadButton:bbn._('Choose files'),dropHere:bbn._('Drop files here'),pasteContainer:bbn._('Click on the container and then press CTRL+V keys to paste the file'),uploadOrDrop:bbn._('Choose files or drop files here'),retry:bbn._('Retry'),editFilename:bbn._('Edit filename'),remove:bbn._('Delete'),removeConfirm:bbn._('Are you sure you want to delete this file?'),empty:bbn._('no files'),download:bbn._('Download'),save:bbn._('Save'),cancel:bbn._('Cancel'),filename:bbn._('Write the filename without the extension'),upload:bbn._('Upload')},this.text);},canAddFile(){return this.uploadable&&!this.uploading&&(this.multiple&&(!this.max||(this.filesCount<this.max)))||(!this.multiple&&!this.filesCount)},filesReady(){return this.currentData.filter(d=>{return d.status==='ready'})},filesProgress(){return this.currentData.filter(d=>{return d.status==='progress'})},filesError(){return this.currentData.filter(d=>{return d.status==='error'})},filesSuccess(){return this.currentData.filter(d=>{return d.status==='success'})},filesCount(){return this.filesReady.length+this.filesProgress.length+this.filesSuccess.length},isEnabled(){return!this.disabled&&!this.readonly}},methods:{_setStatus(id,status){if(id){let idx=bbn.fn.search(this.currentData,{id:id})
if(idx>-1){this.$set(this.currentData[idx],'status',status)
return true}}
return false},_makeFile(file,fromUser,status,fromPaste){return{id:bbn.fn.randomInt(1000,9999),data:file,status:status||'ready',fromUser:fromUser,fromPaste:!!fromPaste,edit:false,progress:0}},_makeFiles(files,fromUser,status){if(!this.ready||!this.disabled){if(files instanceof FileList){files=Object.values(files)}
...
this.$nextTick(()=>{bbn.fn.each(this.filesProgress,fr=>{if((id===undefined)||(fr.id===id)){if(this.saveUrl){let ev=new Event('beforeUpload',{cancelable:true});this.$emit('beforeUpload',ev,fr);if(!ev.defaultPrevented){bbn.fn.upload(this.saveUrl,bbn.fn.extend(true,{},this.data?this.data:{},{file:fr.data}),(res)=>{let f=false;if(res.data.file||res.data.fichier){f=res.data.file||res.data.fichier}
...
return true}}
return false},setStatusReady(id){return this._setStatus(id,'ready')},setStatusError(id){return this._setStatus(id,'error')},setStatusSuccess(id){return this._setStatus(id,'success')},setStatusProgress(id){return this._setStatus(id,'progress')},setValue(){let value=bbn.fn.map(this.filesSuccess,f=>{if(f.data instanceof File){return{name:f.data.name,size:f.data.size,extension:f.data.name.substr(f.data.name.lastIndexOf('.'))}}
return bbn.fn.extend(true,{},f.data,{size:f.data.size,extension:f.data.name.substr(f.data.name.lastIndexOf('.'))});})
...
if(bbn.fn.isObject(file)){this.$set(file,'progress',progress)}}},edit(file){file.edit=file.data.name.substr(0,file.data.name.lastIndexOf('.'))},saveEdit(file){const name=`${file.edit}.${this.getFileExt(file)}`;if(file.edit&&(file.name!==name)){let old=bbn.fn.extend(true,{},file.data)
...
default:return'nf nf-fa-file'}},getFileExt(file){return file.fromUser?file.data.name.substring(file.data.name.lastIndexOf('.')+1):file.data.extension.substr(1)}},mounted(){this.$nextTick(()=>{if(this.value){this._makeFiles(this.getValue(),false,'success')}
...
this.$nextTick(()=>{this.setValue();})}}}}});})(bbn);})(bbn);