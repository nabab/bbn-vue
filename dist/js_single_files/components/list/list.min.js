...
return s;}},methods:{_updateIconSituation(){let hasIcons=false;bbn.fn.each(this.filteredData,(a)=>{if(a.data.icon){hasIcons=true;return false;}});if(hasIcons!==this.hasIcons){this.hasIcons=hasIcons;}},mouseenter(e,idx){if(!this.isOver){e.target.addEventListener('mousemove',()=>{this.overIdx=idx;this.isOver=true;},{once:true});}
...
if(v!==undefined){if(item.selected){if(this.selected.includes(v)){this.selected.splice(this.selected.indexOf(v),1);}
else{if(this.unique&&(this.mode==='free')){this.selected.splice(0,this.selected.length);}
