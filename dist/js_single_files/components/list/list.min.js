...
return data;},},methods:{_updateIconSituation(){let hasIcons=false;bbn.fn.each(this.filteredData,a=>{if(a.data&&a.data.icon){hasIcons=true;return false;}});if(hasIcons!==this.hasIcons){this.hasIcons=hasIcons;}},mouseenter(e,idx){bbn.fn.log("Nouse ener");if(!this.isOver){e.target.addEventListener('mousemove',()=>{this.overIdx=idx;this.isOver=true;},{once:true});}
else{this.overIdx=idx;this.isOver=true;}},resetOverIdx(){bbn.fn.log("Reset OverIdx");if(this.suggest===false){this.overIdx=-1;}
else if(this.suggest===true){this.overIdx=0;}
else if(this.filteredData[this.suggest]){this.overIdx=this.suggest;}},mouseleave(){bbn.fn.log("Nouse leave");this.isOver=false;this.resetOverIdx();},isSelected(idx){let r=false;if(this.filteredData[idx]){if(this.selection){if(this.filteredData[idx].selected&&(this.mode!=='options')){r=true;}}
...
else if(bbn.fn.isFunction(item.data.action)){if(this.actionArguments){item.data.action(...this.actionArguments);}
else{item.data.action(idx,item.data);}}}
else if(item.data.url){bbn.fn.link(item.data.url);}}}}},unselect(value){bbn.fn.each(bbn.fn.filter(this.currentData,a=>{return this.selected.includes(a.data[this.sourceValue]);}),a=>{if(a.selected){a.selected=false;}});this.selected.splice(0,this.selected.length);}},created(){this.$on('dataloaded',()=>{this._updateIconSituation();});},mounted(){this.$nextTick(()=>{if(this.$parent.$options&&(this.$parent.$options._componentTag==='bbn-scroll')){this.hasScroll=true;}
this.ready=true;setTimeout(()=>{this.resetOverIdx();},50);});},watch:{overIdx(newVal,oldVal){bbn.fn.log("overIdx is changing")
this.keepCool(()=>{bbn.fn.log("Scroll to?");if(this.hasScroll&&newVal&&!this.isOver){bbn.fn.log("Scroll to!")
this.closest('bbn-scroll').scrollTo(null,this.getRef('li'+newVal));}},'overIdx',50)},source:{deep:true,handler(){if(this.isAutobind){this.updateData();}}},}});})(window.Vue,window.bbn);})(bbn);