...
</div>`;script.setAttribute('id','bbn-tpl-component-panelbar');script.setAttribute('type','text/x-template');document.body.insertAdjacentElement('beforeend',script);(function(bbn){"use strict";Vue.component('bbn-panelbar',{mixins:[bbn.vue.basicComponent,bbn.vue.localStorageComponent,bbn.vue.resizerComponent],props:{multiple:{type:Boolean,default:false},flex:{type:Boolean,default:false},source:{type:Array},scrollable:{type:Boolean,default:true},align:{type:String,default:'center'},opened:{type:Number,},headerComponent:{type:String},headerOptions:{type:Object},component:{type:String},componentOptions:{type:Object}},data(){return{selectedValues:[],size:null,selected:null,preselected:null,childHeight:0};},computed:{headers(){return this.$refs['header']}},methods:{isSelected(idx){if(!this.multiple){return this.selected===idx}
else{return this.selectedValues.includes(idx)}},onResize(){this.size=this.$el.clientHeight;},multiselect(idx){if(!this.selectedValues.includes(idx)){this.selectedValues.push(idx)}
else{this.selectedValues.splice(this.selectedValues.indexOf(idx),1)}},select(idx){if(this.selected!==idx){this.preselected=idx;if(this.selected===null){setTimeout(()=>{this.selected=idx;},300);}
else{this.selected=idx;}
this.$emit('select',idx,this.source[idx])
if(!this.flex){this.$nextTick(()=>{this.getStyle(idx)})}}
else{this.preselected=null;this.selected=null;}},getStyle(idx){if(!this.multiple){if((idx!==null)&&(idx===this.preselected)&&(this.flex||((this.source[idx]!==undefined)&&(this.source[idx].flex===true)))){return this.size?{height:this.size+'px',overflow:'hidden'}:{};}
...
else{return{height:'0px',overflow:'hidden'};}}
else{if(this.selectedValues.includes(idx)&&(idx!==null)&&(this.flex||((this.source[idx]!==undefined)&&(this.source[idx].flex===true)))){return this.size?{height:this.size+'px',overflow:'hidden'}:{};}
else if(this.selectedValues.includes(idx)&&(idx!==null)&&(!this.flex||((this.source[idx]!==undefined)&&(this.source[idx].flex===false)))){let children=this.getRef('container').children,res=[],childHeight=0;bbn.fn.each(children,a=>{if(a.classList.contains('bbn-border-box')){res.push(a)}})
this.$nextTick(()=>{if(res[idx]&&res[idx].firstElementChild.clientHeight){this.childHeight=res[idx].firstElementChild.clientHeight}
return this.size?{height:this.childHeight+'px',overflow:'hidden'}:{};})}
else{return{height:'0px',overflow:'hidden'};}}}},mounted(){if(this.opened!==undefined){this.$nextTick(()=>{this.select(this.opened)})}},watch:{selected(v,o){if(v!==null){setTimeout(()=>{this.headers[v].style.overflow=null;},300)}}}});})(bbn);})(bbn);