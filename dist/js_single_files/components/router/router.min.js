...
Vue.component("bbn-router",{name:'bbn-router',mixins:[bbn.vue.basicComponent,bbn.vue.localStorageComponent,bbn.vue.closeComponent,bbn.vue.observerComponent,bbn.vue.resizerComponent,bbn.vue.keepCoolComponent],props:{auto:{type:Boolean,default:true},url:{type:String,default:''},autoload:{type:Boolean,default:true},root:{type:String,default:''},def:{type:String},source:{type:Array,default(){return[];}},single:{type:Boolean,default:false},maxTotal:{type:Number,default:25},nav:{type:Boolean,default:false},visual:{type:Boolean,default:false},scrollable:{type:Boolean,default:false},scrollContent:{type:Boolean,default:true},storageName:{type:String,default:'__ROOT__'},confirmLeave:{type:[Boolean,String,Function],default:bbn._("Are you sure you want to discard the changes you made in this page?")},historyMaxLength:{type:Number,default:10},menu:{type:[Array,Function],default:function(){return[];}},breadcrumb:{type:Boolean,default:false},master:{type:Boolean,default:false},postBaseUrl:{type:Boolean,default:true},component:{type:[String,Object]},componentSource:{type:Object},componentUrl:{type:String},maxTitleLength:{type:Number,default:20},urlNavigation:{type:Boolean,default:true},ignoreDirty:{type:Boolean,default:false},visualSize:{type:Number,default:180},orientation:{type:String,default(){return'auto'},validator(v){return!!bbn.fn.getRow(possibleOrientations,{name:v})}},bcolor:{type:String,default:'#666'},fcolor:{type:String,default:'#EEE'}},data(){return{db:null,numRegistered:0,hasRealContainers:false,hasFakeContainers:false,hasEmptyURL:false,cfgViews:[].concat(this.source),slotViews:[],views:[],urls:{},currentTitle:'',currentURL:this.url||'',baseURL:this.formatBaseURL(this.root),parents:[],parent:null,router:null,parentContainer:null,visible:true,activeContainer:null,isLoading:false,routed:false,isRouting:false,isInit:false,selected:null,dirtyContainers:[],history:[],iconsReady:false,isBreadcrumb:this.breadcrumb,breadcrumbWatcher:false,breadcrumbsList:[],visualShowAll:false,visualOrientation:this.orientation!=='auto'?this.orientation:null,lockedOrientation:false,isVisual:this.visual};},computed:{fullBaseURL(){let vm=this,base='',tmp;while(tmp=vm.baseURL){base=tmp+base;if(!vm.parents.length){break;}
vm=vm.parents[0];}
...
return{minHeight:'100%',display:'grid',gridColumnGap:'0.5em',gridRowGap:'0.5em',gridTemplateRows:'repeat('+this.numVisualRows+', 1fr)',gridTemplateColumns:'repeat('+this.numVisualCols+', 1fr)'}},numVisualRows(){if(this.isVisual){return Math.ceil(this.lastKnownHeight / this.visualSize);}
return 1;},numVisualCols(){if(this.isVisual){return Math.ceil(this.lastKnownWidth / this.visualSize);}
return 1;},numVisuals(){if(this.isVisual){if(['left','right'].includes(this.visualOrientation)){return this.numVisualRows;}
else{return this.numVisualCols;}}},visualList(){if(!this.isVisual){return[];}
...
return{view:a,visible:visible}});}},methods:{isNumber:bbn.fn.isNumber,numProperties:bbn.fn.numProperties,remove(misc,force){let idx=this.getIndex(misc);if(idx>-1){let onBeforeClose=new Event('beforeClose',{cancelable:!force});let onClose=new Event('close');this.$emit('beforeClose',idx,onBeforeClose);if(force||!onBeforeClose.defaultPrevented){if(!this.ignoreDirty&&this.isDirty&&this.views[idx].dirty&&!force){this.confirm(this.confirmLeave,()=>{let forms=this.urls[this.views[idx].url].forms;if(Array.isArray(forms)&&forms.length){bbn.fn.each(forms,(f,k)=>{f.reset();});}
this.$nextTick(()=>{this.$emit('close',idx,onClose);});});}
else if(this.views[idx]&&!this.views[idx].real){this.$emit('close',idx,onClose);let url=this.views[idx].url;this.views.splice(idx,1);this.$delete(this.urls,url);this.fixIndexes()
return true;}}}
return false;},close(idx,force,noCfg){let res=this.remove(idx,force);if(res&&(this.selected>idx)){this.selected--;}
else if(res&&(this.selected===idx)){if(this.views.length){bbn.fn.each(this.history,a=>{let tmp=this.getIndex(a);if(tmp!==false){idx=tmp;return false;}});this.activateIndex(this.views[idx]?idx:idx-1);}
else{this.selected=false;}}
if(!noCfg){this.setConfig();}
return res;},add(obj,idx){let index;if((typeof(obj)==='object')&&bbn.fn.isString(obj.url)){obj.url=bbn.fn.replaceAll('//','/',obj.url);if(obj.$options){if(!obj.current&&!obj.currentURL){if(bbn.env.path.indexOf(this.getFullBaseURL()+(obj.url?obj.url+'/':''))===0){obj.currentURL=bbn.fn.substr(bbn.env.path,this.getFullBaseURL().length);}
...
bbn.fn.iterate(this.getDefaultView(),(a,n)=>{if(obj[n]===undefined){this.$set(obj,n,a);}});obj.uid=obj.url+'-'+bbn.fn.randomString();if(this.single){this.views.splice(0,this.views.length,obj);}
else if(isValid){this.views.splice(obj.idx,0,obj);}
...
this.numRegistered++;this.urls[cp.url]=cp;if(this.isVisual){cp.$on('view',()=>{this.visualShowAll=false;if(this.activeContainer&&(this.activeContainer.url!==cp.url)){this.activeContainer.hide();}})}
let idx=this.search(cp.url);if(idx===false){this.add(cp);}
else{cp.currentIndex=idx;}
if(this.numRegistered===this.views.length){this.init(this.single?cp.url:this.getDefaultURL());}
...
if(idx!==false){}},getRoute(url,force){if(!bbn.fn.isString(url)){throw Error(bbn._('The component bbn-container must have a valid URL defined'));}
...
if(st){this.urls[st].setCurrent(url);this.realRoute(url,st,force,bits[1]);}}}}},realRoute(url,st,force,anchor){if(!bbn.fn.isString(url)&&!bbn.fn.isNumber(url)){throw Error(bbn._('The component bbn-container must have a valid URL defined'));}
if(this.urls[st]){if(url!==this.currentURL){this.currentURL=url;}
if(!this.routed){this.routed=true;this.$emit("route1",this);}
this.activate(url,this.urls[st]);if(this.urls[st]){this.urls[st].currentURL=url;this.urls[st].init();this.$nextTick(()=>{let child=this.urls[st].find('bbn-router');if(child){child.route(bbn.fn.substr(url,st.length+1),force);}
...
else if(url!==this.activeContainer.currentURL){this.activeContainer.setCurrent(url);}},changeURL(url,title,replace){if(!bbn.fn.isString(url)){throw Error(bbn._('The component bbn-container must have a valid URL defined'));}
...
return fullURL;},isValidIndex(idx){return(typeof idx==='number')&&(this.views[idx]!==undefined);},activateDefault(){let idx=this.getIndex('',true);if(this.isValidIndex(idx)){this.activate(this.views[idx].current?this.views[idx].current:this.views[idx].url);}},activateIndex(idx){if(this.isValidIndex(idx)){this.route(this.urls[this.views[idx].url]?this.urls[this.views[idx].url].current:this.views[idx].current);}},getVue(idx){if(idx===undefined){idx=this.selected;}
...
return r>-1?r:false;},callRouter(url,st){if(!bbn.fn.isString(url)){throw Error(bbn._('The component bbn-container must have a valid URL defined'));}
...
view=this.views[idx];if(force){let kept={loading:true,loaded:false,load:true,url:view.url,current:url,selected:true,title:view.title,static:view.static,pinned:view.pinned,index:idx};if(view.icon){kept.icon=view.icon;}
if(view.bcolor){kept.bcolor=view.bcolor;}
if(view.fcolor){kept.fcolor=view.fcolor;}
bbn.fn.iterate(bbn.fn.extend(this.getDefaultView(),kept),(a,n)=>{if(view[n]!==a){this.$set(view,n,a);}});if(this.urls[url]){this.urls[url].isLoading=true;this.urls[url].isLoaded=false;this.urls[url].isInit=false;}}
if((index!==undefined)&&(idx!==index)){this.move(idx,index);idx=index;}}
else{toAdd=true;idx=index===undefined?this.views.length:index;}
if(toAdd){this.add({url:url,title:view&&view.title?view.title:bbn._('Loading'),load:true,loading:true,real:false,scrollable:!this.single,current:url,error:false,loaded:false,hidden:false},idx);}
else if(!this.views[idx].loading){this.views[idx].loading=true;}
this.selected=this.single?0:idx;this.$forceUpdate();this.$emit('update',this.views);let dataObj=this.postBaseUrl?{_bbn_baseURL:this.fullBaseURL}:{};return this.post(finalURL,dataObj,d=>{let callRealInit=true;this.isLoading=false;if(d.url){d.url=this.parseURL(d.url);}
if(!d.url){d.url=url;}
if(url.indexOf(d.url)===0){d.current=url;}
if(d.data&&bbn.fn.numProperties(d.data)){d.source=d.data;delete d.data;}
if((d.url!==d.current)&&this.urls[d.current]){bbn.fn.warning("DELETING VIEW CASE.... "+d.current+' '+this.urls[d.current].idx,d.url,bbn.fn.search(this.views,{idx:this.urls[d.current].idx}));this.remove(this.urls[d.current].idx,true);callRealInit=false;this.$on('registered',url=>{if(url===d.url){this.$off('registered',url);this.realInit(url);}})}
...
this.$nextTick(()=>{if(callRealInit){this.realInit(d.url);}})})},(xhr,textStatus,errorThrown)=>{this.isLoading=false;this.alert(textStatus);let idx=this.search(this.parseURL(finalURL));if(idx!==false){let url=this.views[idx].url;if(this.urls[url]){this.remove(idx);}}},()=>{this.isLoading=false;});}},realInit(url){if(this.urls[url]){this.urls[url].setLoaded(true);this.urls[url].init();this.callRouter(this.urls[url].current,url);this.$emit('update',this.views);}
else{}},reload(idx){this.$nextTick(()=>{if(this.views[idx]&&this.views[idx].load&&this.urls[this.views[idx].url]&&this.urls[this.views[idx].url].isLoaded){let url=this.views[idx].current;setTimeout(()=>{this.load(url,true,idx);},100);}});},getDefaultURL(){if(this.url){return this.url;}
if(this.parentContainer&&(this.parentContainer.currentURL!==this.parentContainer.url)){return bbn.fn.substr(this.parentContainer.currentURL,this.parentContainer.url.length+1);}
...
if(this.getVue(idx).fullScreen){items.push({text:bbn._("Exit full screen"),key:"reduce",icon:"nf nf-mdi-arrow_collapse",action:()=>{this.getVue(idx).fullScreen=false;}});}
else{items.push({text:bbn._("Enlarge"),key:"enlarge",icon:"nf nf-mdi-arrow_expand_all",action:()=>{this.getVue(idx).fullScreen=true;}});}
...
if(others){items.push({text:bbn._("Close Others"),key:"close_others",icon:"nf nf-mdi-close_circle_outline",action:()=>{this.closeAllBut(idx);}});if(!this.isVisual){let directions=[];if(idx){if(idx>1){directions.push({text:bbn._("First"),key:"move_first",icon:"nf nf-mdi-close_circle_outline",action:()=>{this.move(idx,0);}});}
...
if(!this.isVisual){items.push({text:bbn._('Switch to')+' '+(this.isBreadcrumb?bbn._('tabs'):bbn._('breadcrumb'))+' '+bbn._('mode'),key:'switch',icon:this.isBreadcrumb?'nf nf-mdi-tab':'nf nf-fa-ellipsis_h',action:()=>{this.itsMaster.isBreadcrumb=!this.itsMaster.isBreadcrumb;}});if(!this.parents.length){let go=()=>{this.isVisual=true;setTimeout(()=>{this.getVue(this.selected).init();this.getVue(this.selected).show();this.onResize();},250)};items.push({text:bbn._('Switch to')+' '+bbn._('visual')+' '+bbn._('mode'),key:'visual',icon:'nf nf-fa-eye',action:()=>{if(!this.isDirty){go();return;}
this.confirm(bbn._("The pages already loaded will be reinitialized")+'<br>'+bbn._("If you have any unsaved work opened it will be lost.")+'<br>'+bbn._("Is it ok to continue?"),()=>{go()});}});}}
else{const go=()=>{this.isVisual=false;this.itsMaster.isBreadcrumb=false;setTimeout(()=>{this.getVue(this.selected).init();this.getVue(this.selected).show();this.onResize();},250)};let visualPositions=[];bbn.fn.each(possibleOrientations,a=>{if((a.name==='auto')&&(this.orientation==='auto')&&!this.lockedOrientation){return;}
if(this.visualOrientation!==a.name){visualPositions.push({text:a.text,icon:a.name==='auto'?'nf nf-mdi-auto_fix':'nf nf-mdi-border_'+a.name,action:()=>{if(a.name==='auto'){this.lockedOrientation=false;}
else{this.visualOrientation=a.name;this.lockedOrientation=true;}
this.onResize();this.setConfig();}});}});items.push({text:bbn._("Change visual blocks' position"),icon:'nf nf-mdi-cursor_move',items:visualPositions});items.push({text:bbn._('Switch to')+' '+bbn._('tabs')+' '+bbn._('mode'),key:'tabs',icon:'nf nf-mdi-tab',action:()=>{if(!this.isDirty){go();return;}
this.confirm(bbn._("The pages already loaded will be reinitialized")+'<br>'+bbn._("You should save your unsaved content or it will be lost.")+'<br>'+bbn._("Is it ok to continue?"),()=>{go();});}});items.push({text:bbn._('Switch to')+' '+bbn._('breadcrumb')+' '+bbn._('mode'),key:'breadcrumbs',icon:'nf nf-fa-ellipsis_h',action:()=>{if(!this.isDirty){go();return;}
this.confirm(bbn._("The pages already loaded will be reloaded")+'<br>'+bbn._("If you have any unsaved work opened it will be lost.")+'<br>'+bbn._("Is it ok to continue?"),()=>{go();});}});}
return items;},setConfig(){if(this.autoload&&this.isInit){this.setStorage(this.getConfig(),this.parentContainer?this.parentContainer.getFullURL():this.storageName);}},getConfig(){let cfg={baseURL:this.parentContainer?this.parentContainer.getFullURL():this.storageName,views:[],breadcrumb:this.isBreadcrumb,visual:this.isVisual,orientation:this.lockedOrientation?this.visualOrientation:null};bbn.fn.each(this.views,(obj,i)=>{if(obj.url&&obj.load){let res={url:obj.url,icon:obj.icon||false,notext:obj.notext||false,load:true,loaded:false,title:obj.title?obj.title:bbn._('Untitled'),static:!!obj.static,pinned:!!obj.pinned,current:obj.current?obj.current:obj.url,cfg:{},real:obj.real,last:obj.last};if(obj.bcolor){res.bcolor=obj.bcolor;}
...
return this.getRef('tab-'+idx);},closeAll(force){for(let i=this.views.length-1;i>=0;i--){if(!this.views[i].static&&!this.views[i].pinned){this.close(i,force,true);}}
...
this.setConfig();},pin(idx){if(this.isValidIndex(idx)){let ev=new CustomEvent('beforePin',{cancelable:true});this.$emit('beforePin',idx,ev);if(!ev.defaultPrevented){this.views[idx].pinned=true;this.setConfig();this.$emit('pin',idx);}}},unpin(idx){if(this.isValidIndex(idx)){let ev=new CustomEvent('beforeUnpin',{cancelable:true});this.$emit('beforeUnpin',idx,ev);if(!ev.defaultPrevented){this.views[idx].pinned=false;this.setConfig();this.$emit('unpin',idx);}}},registerBreadcrumb(bc){let url=bbn.fn.substr(bc.baseURL,0,bc.baseURL.length-1);this.breadcrumbsList.push(bc);if(this.itsMaster&&!this.master){this.itsMaster.breadcrumbsList.push(bc);}},unregisterBreadcrumb(bc){let url=bbn.fn.substr(bc.baseURL,0,bc.baseURL.length-1);let idx=bbn.fn.search(this.breadcrumbsList,{baseURL:bc.baseURL});if(idx!==-1){this.breadcrumbsList.splice(idx,1);}
...
bbn.fn.each(this.views,(t,i)=>{if(!t.hidden&&(t.idx!==this.selected)){list.push({view:t,key:t.url,parents:parents,children:bbn.fn.map(this.getBreadcrumbs(i),c=>{return{view:c.views[c.selected],maxTitleLength:c.maxTitleLength}}),maxTitleLength:this.maxTitleLength,action:()=>{this.activateIndex(t.idx);},closeAction:()=>{return this.close(t.idx)}})}});return list;},getParents(){return this.parent?[...this.parent.getParents(),this.parent]:[]},openMenu(ev){let ele=ev.target.parentElement.parentElement,e=new MouseEvent("contextmenu",{bubbles:true,cancelable:true,view:window});ele.dispatchEvent(e);},onResize(){this.keepCool(()=>{if(this.setResizeMeasures()&&this.setContainerMeasures()){this.$emit('resize');}
if(this.isVisual&&(this.orientation==='auto')&&!this.lockedOrientation){this.visualOrientation=this.lastKnownWidth>this.lastKnownHeight?'left':'top';}},'resize',50);},visualStyleContainer(ct){if(!ct.visible||this.visualShowAll){return{zoom:0.1};}
...
return{gridColumnStart:coord[0],gridColumnEnd:coord[1],gridRowStart:coord[2],gridRowEnd:coord[3],zoom:1}}},created(){this.componentClass.push('bbn-resize-emitter');this.$on('route',url=>{if(this.nav){this.setConfig();let i=this.history.indexOf(url);if(i>-1){this.history.splice(i,1);}
...
this.ready=true;if(!this.views.length){this.init(url);}},beforeDestroy(){if(!this.master&&this.parent){this.parent.unregisterBreadcrumb(this);}},watch:{selected(idx){if(this.views[idx]){bbn.fn.map(bbn.fn.filter(this.views,{selected:true}),a=>{if(a.idx!==idx){a.selected=false;}});if(!this.views[idx].selected){this.views[idx].selected=true;}}
else{throw new Error("The view with index "+idx+" doesn't exist");}},currentTitle(v){if(!this.parent){document.title=v+' - '+bbn.env.siteTitle;}},currentURL(newVal,oldVal){if(this.ready){this.$nextTick(()=>{if(this.activeContainer){this.changeURL(newVal,this.activeContainer.title);}
else if(this.isLoading){this.changeURL(newVal,bbn._("Loading"));}
let idx=this.search(newVal);if(!this.single&&(idx!==false)){this.selected=idx;this.views[this.selected].last=bbn.fn.timestamp();}
this.$emit('change',newVal);});this.$emit('route',newVal);}},url(newVal){if(this.ready){bbn.fn.log("CHANGING ROUTER URL");this.route(newVal);}},isDirty(v){if(this.parentContainer){this.parentContainer.dirty=v;}},itsMaster(newVal,oldVal){if(this.nav&&(newVal!==oldVal)){this.isBreadcrumb=newVal?newVal.isBreadcrumb:this.breadcrumb;if(this.breadcrumbWatcher){this.breadcrumbWatcher();}
if(newVal){this.breadcrumbWatcher=this.$watch('itsMaster.isBreadcrumb',isB=>{this.isBreadcrumb=isB;});}}},isBreadcrumb(newVal){this.$nextTick(()=>{this.setConfig();})},isVisual(){this.$nextTick(()=>{this.setConfig();})}},components:{listItem:{template:`
...
else{this.closest('bbn-floater').close();}}}}},shortTitle(src){return src.view.title.length>src.maxTitleLength?bbn.fn.shorten(src.view.title,src.maxTitleLength):src.view.title;}}}}});})(bbn,Vue);})(bbn);