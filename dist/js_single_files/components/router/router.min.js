...
this.fixIndexes()}},move(from,to){if(!bbn.fn.isNumber(from,to)||(from===to)||!this.views[from]||!this.views[to]){return;}
...
this.views[i].idx=i;}}
this.setConfig();},search(url){if(!bbn.fn.isString(url)){bbn.fn.log("error in search with url",url);throw Error(bbn._('The component bbn-container must have a valid URL defined'));}
...
if(others){items.push({text:bbn._("Close Others"),key:"close_others",icon:"nf nf-mdi-close_circle_outline",action:()=>{this.closeAllBut(idx);}});let directions=[];if(idx){if(idx>1){directions.push({text:bbn._("First"),key:"move_first",icon:"nf nf-mdi-close_circle_outline",action:()=>{this.move(idx,0);}});}
...
items.push({text:bbn._('Switch to ')+(this.isBreadcrumb?bbn._('tabs'):bbn._('breadcrumb'))+' '+bbn._('mode'),key:'switch',icon:this.isBreadcrumb?'nf nf-mdi-tab':'nf nf-fa-ellipsis_h',action:()=>{this.itsMaster.isBreadcrumb=!this.itsMaster.isBreadcrumb;}});return items;},close(idx,force,noCfg){let res=this.remove(idx,force);if(res&&(this.selected>idx)){this.selected--;}
