...
this.activate(url,this.urls[st]);if(this.urls[st]){this.urls[st].currentURL=url;this.$nextTick(()=>{let child=this.urls[st].find('bbn-router');if(child){child.route(bbn.fn.substr(url,st.length+1),force);}
...
if(this.parent){let containers=this.ancestors('bbn-container');url=bbn.fn.substr(this.getFullBaseURL(),this.router.baseURL.length)+url;bbn.fn.log("CALL ROOT ROUTER WITH URL "+url);this.router.realRoute(url,containers[containers.length-1].url,true);}
...
this.selected=idx;this.$forceUpdate();this.$emit('update',this.views);let dataObj=this.postBaseUrl?{_bbn_baseURL:this.fullBaseURL}:{};return this.post(finalURL,dataObj,d=>{let callRealInit=true;this.isLoading=false;if(d.url){d.url=this.parseURL(d.url);}
if(!d.url){d.url=url;}
...
this.$nextTick(()=>{if(callRealInit){this.realInit(d.url);}})})},(xhr,textStatus,errorThrown)=>{this.isLoading=false;this.alert(textStatus);let idx=this.search(this.parseURL(finalURL));if(idx!==false){let url=this.views[idx].url;if(this.urls[url]){this.close(idx,true);}}},()=>{this.isLoading=false;});}},realInit(url){if(this.urls[url]){this.urls[url].setLoaded(true);this.urls[url].init();this.callRouter(this.urls[url].current,url);this.$emit('update',this.views);}
else{}},reload(idx){this.$nextTick(()=>{if(this.views[idx]&&this.views[idx].load&&this.urls[this.views[idx].url]&&this.urls[this.views[idx].url].isLoaded){let url=this.views[idx].current;setTimeout(()=>{this.load(url,true,idx);},100);}});},getDefaultURL(){let url='';if(this.autoload){url=this.parseURL(bbn.env.path);}
...
else if(container){items.push({text:bbn._("Enlarge"),key:"enlarge",icon:"nf nf-mdi-arrow_expand_all",action:()=>{container.fullScreen=true;}});}
...
if(!this.isVisual){items.push({text:bbn._('Switch to')+' '+(this.isBreadcrumb?bbn._('tabs'):bbn._('breadcrumb'))+' '+bbn._('mode'),key:'switch',icon:this.isBreadcrumb?'nf nf-mdi-tab':'nf nf-fa-ellipsis_h',action:()=>{this.itsMaster.isBreadcrumb=!this.itsMaster.isBreadcrumb;}});if(!this.parents.length){items.push({text:bbn._('Switch to')+' '+bbn._('visual')+' '+bbn._('mode'),key:'visual',icon:'nf nf-fa-eye',action:()=>{this.isVisual=true;setTimeout(()=>{this.getVue(this.selected).show();this.onResize();},250)}});}}
else{const toNoVisual=()=>{this.isVisual=false;this.itsMaster.isBreadcrumb=false;setTimeout(()=>{this.getVue(this.selected).show();this.onResize();},250)};let visualPositions=[];bbn.fn.each(possibleOrientations,a=>{if((a.name==='auto')&&(this.orientation==='auto')&&!this.lockedOrientation){return;}
...
this.onResize();this.setConfig();}});}});items.push({text:bbn._("Change visual blocks' position"),icon:'nf nf-mdi-cursor_move',items:visualPositions});items.push({text:bbn._('Switch to')+' '+bbn._('tabs')+' '+bbn._('mode'),key:'tabs',icon:'nf nf-mdi-tab',action:()=>{toNoVisual();}});items.push({text:bbn._('Switch to')+' '+bbn._('breadcrumb')+' '+bbn._('mode'),key:'breadcrumbs',icon:'nf nf-fa-ellipsis_h',action:()=>{toNoVisual();}});}
...
return'';},scrollTabs(dir){let scroll=this.getRef('horizontal-scroll');if(scroll){if(dir==='right'){scroll.scrollAfter(true);}
else{scroll.scrollBefore(true);}}},getTab(idx){if(idx===undefined){idx=this.selected;}
...
let idx=this.search(newVal);if(idx!==false){this.selected=idx;this.views[this.selected].last=bbn.fn.timestamp();}
