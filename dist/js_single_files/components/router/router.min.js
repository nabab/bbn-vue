...
</div>`;script.setAttribute('id','bbn-tpl-component-router');script.setAttribute('type','text/x-template');document.body.insertAdjacentElement('beforeend',script);(function(bbn,Vue){"use strict";Vue.component("bbn-router",{name:'bbn-router',mixins:[bbn.vue.basicComponent,bbn.vue.localStorageComponent,bbn.vue.closeComponent,bbn.vue.observerComponent],props:{auto:{type:Boolean,default:true},url:{type:String,default:''},autoload:{type:Boolean,default:true},root:{type:String,default:''},def:{type:String},source:{type:Array,default(){return[];}},single:{type:Boolean,default:false},nav:{type:Boolean,default:false},scrollable:{type:Boolean,default:false},storageName:{type:String,default:'__ROOT__'},confirmLeave:{type:[Boolean,String,Function],default:bbn._("Are you sure you want to discard the changes you made in this page?")},hideAdvertUrl:{type:String},historyMaxLength:{type:Number,default:10},menu:{type:[Array,Function],default:function(){return[];}},breadcrumb:{type:Boolean,default:false},master:{type:Boolean,default:false},postBaseUrl:{type:Boolean,default:true},showSwitch:{type:Boolean,default:true},component:{type:[String,Object]},componentSource:{type:Object},componentUrl:{type:String},maxTitleLength:{type:Number,default:20}},data(){return{numRegistered:0,hasRealContainers:false,hasFakeContainers:false,hasEmptyURL:false,cfgViews:[].concat(this.source),slotViews:[],views:[],urls:{},currentTitle:'',currentURL:this.url||'',baseURL:this.setBaseURL(this.root),parents:[],parent:null,router:null,parentContainer:null,visible:true,activeContainer:null,isLoading:false,routed:false,isRouting:false,isInit:false,selected:null,dirtyContainers:[],history:[],iconsReady:false,isBreadcrumb:this.breadcrumb,breadcrumbWatcher:false,breadcrumbsList:[]};},computed:{fullBaseURL(){let vm=this,base='',tmp;while(tmp=vm.baseURL){base=tmp+base;if(!vm.parents.length){break;}
vm=vm.parents[0];}
...
bbn.fn.log("changeURL",url,title,this.parentContainer?["FROM PQARENT",this.parentContainer.currentTitle,this.parentContainer.title]:this.currentTitle);if(this.views[this.selected]&&((url.indexOf(this.views[this.selected].url+'/')===0)||(url===this.views[this.selected].url))){this.$set(this.views[this.selected],'current',url);}
if(this.parentContainer){this.parentContainer.currentTitle=title+' < '+this.parentContainer.title;this.parent.changeURL(this.baseURL+url,this.parentContainer.currentTitle,replace);}
...
if(!ev2.defaultPrevented){if(this.isDirty&&this.views[idx].dirty&&!ev.defaultPrevented&&!force){ev.preventDefault();this.confirm(this.confirmLeave,()=>{let forms=this.urls[this.views[idx].url].findAll('bbn-form');if(Array.isArray(forms)&&forms.length){bbn.fn.each(forms,(f,k)=>{f.reset();});}
