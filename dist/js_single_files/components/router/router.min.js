...
return{display:'grid',gridColumnGap:'0.5em',gridRowGap:'0.5em',gridTemplateRows:'repeat('+this.numVisualRows+', 1fr)',gridTemplateColumns:'repeat('+this.numVisualCols+', 1fr)'}},numVisualRows(){if(this.isVisual){return Math.ceil(this.lastKnownHeight / this.visualSize);}
...
let moreViewsThanSlots=this.numVisuals<this.views.length;let numAvailableSlots=this.numVisuals-(moreViewsThanSlots?1:0);return bbn.fn.map(bbn.fn.multiorder(this.views,{selected:'desc',static:'desc',pinned:'desc',last:'desc',idx:'asc'}),(a,i)=>{let visible=false;if(this.visualShowAll||(i<=numAvailableSlots)||(this.selected===a.index)){visible=true;}
return{view:a,visible:visible}});}},methods:{isNumber:bbn.fn.isNumber,numProperties:bbn.fn.numProperties,remove(misc,force){let idx=this.getIndex(misc);if(idx>-1){let onBeforeClose=new Event('beforeClose',{cancelable:!force});let onClose=new Event('close');this.$emit('beforeClose',idx,onBeforeClose);if(force||!onBeforeClose.defaultPrevented){if(!this.ignoreDirty&&this.isDirty&&this.views[idx].dirty&&!force){ev.preventDefault();this.confirm(this.confirmLeave,()=>{let forms=this.urls[this.views[idx].url].forms;if(Array.isArray(forms)&&forms.length){bbn.fn.each(forms,(f,k)=>{f.reset();});}
this.$nextTick(()=>{this.$emit('close',idx,onClose);this.close(idx,true);});});}
else{if(this.views[idx]){if(this.views[idx].real){return;let url=this.views[idx].url;this.views.splice(idx,1);this.$delete(this.urls,url);}
else{this.$emit('close',idx,onClose);let url=this.views[idx].url;this.views.splice(idx,1);this.$delete(this.urls,url);}
this.fixIndexes()
return true;}}}}
...
bbn.fn.iterate(this.getDefaultView(),(a,n)=>{if(obj[n]===undefined){this.$set(obj,n,a);}});obj.uid=bbn.fn.randomString();if(isValid){this.views.splice(obj.idx,0,obj);}
else{this.views.push(obj);}}}
this.fixIndexes()}},register(cp,fake){if(fake){this.add(cp);return;}
...
if(this.isInit&&(this.views.length!==this.numRegistered)){this.isInit=false;}},getRoute(url,force){if(!bbn.fn.isString(url)){throw Error(bbn._('The component bbn-container must have a valid URL defined'));}
...
else if(index!==undefined){this.move(idx,index);}}
else{toAdd=true;idx=index===undefined?this.views.length:index;}
if(toAdd){this.add({url:url,title:view&&view.title?view.title:bbn._('Loading'),load:true,loading:true,visible:true,real:false,scrollable:!this.single,current:url,error:false,loaded:false,hidden:false},idx);}
...
else{throw new Error(bbn._("Impossible to find the container for URL")+' '+url);}},reload(idx){this.$nextTick(()=>{if(this.views[idx]&&this.views[idx].load&&this.urls[this.views[idx].url]&&this.urls[this.views[idx].url].isLoaded){let url=this.views[idx].current;this.remove(idx);setTimeout(()=>{this.load(url,true,idx);},250);}});},getDefaultURL(){if(this.url){return this.url;}
...
if(!this.isVisual){items.push({text:bbn._('Switch to')+' '+(this.isBreadcrumb?bbn._('tabs'):bbn._('breadcrumb'))+' '+bbn._('mode'),key:'switch',icon:this.isBreadcrumb?'nf nf-mdi-tab':'nf nf-fa-ellipsis_h',action:()=>{this.itsMaster.isBreadcrumb=!this.itsMaster.isBreadcrumb;}});if(!this.parents.length){let go=()=>{this.isVisual=true;this.onResize();setTimeout(()=>{this.activateIndex(this.getIndex(this.getFullCurrentURL()));},250)};items.push({text:bbn._('Switch to')+' '+bbn._('visual')+' '+bbn._('mode'),key:'visual',icon:'nf nf-fa-eye',action:()=>{if(!this.isDirty){go();return;}
...
else{const go=()=>{this.isVisual=false;this.itsMaster.isBreadcrumb=false;setTimeout(()=>{this.activateIndex(this.getIndex(this.getFullCurrentURL()));},250)};let visualPositions=[];bbn.fn.each(possibleOrientations,a=>{if((a.name==='auto')&&(this.orientation==='auto')&&!this.lockedOrientation){return;}
...
else{if(db){bbn.db.open('bbn').then(r=>{this.db=r;},err=>{bbn.fn.log("Connection error in router",err);});}
...
return st;}});}
let tmp=[];let storage=this.getStorage(this.parentContainer?this.parentContainer.getFullURL():this.storageName);if(storage){if(storage.breadcrumb!==undefined){this.isBreadcrumb=storage.breadcrumb;}
