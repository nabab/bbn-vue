...
this.$forceUpdate();this.$nextTick(()=>{this.isResizing=false;this.$nextTick(()=>{this.setResizeMeasures();this.$forceUpdate();this.$nextTick(()=>{this.updatePosition();if(!this.isResized){this.isResized=true;bbn.fn.log("Floater: resized gets TRUE");}
bbn.fn.log("Floater: resizing again");this.$emit('resize');if(!wasInit){if(this.onOpen){this.onOpen(this);}
this.$emit('open',this);}});});});}
...
return bbn.vue.listComponent.methods.updateData.apply(this);}},created(){this.componentClass.push('bbn-resize-emitter');this.isResizing=true;},mounted(){if(this.isVisible){this.ready=true;bbn.fn.log("Floater: IS ready");}
this.$nextTick(()=>{let ancestors=this.ancestors('bbn-floater');if(this.element){let ct=ancestors.length?ancestors[ancestors.length-1]:this;let scroll=ct.closest('bbn-scroll');if(scroll){scroll.$once('scroll',()=>{this.close();});}}});},beforeDestroy(){if(this.onClose){this.onClose(this);}},updated(){},watch:{left(){this.updatePosition();},right(){this.updatePosition();},top(){this.updatePosition();},bottom(){this.updatePosition();},source:{deep:true,handler(){if(this.currentData.length){this.updateData();}}},visible(v){this.currentVisible=v;},isResized(){bbn.fn.log("Floater: IS resized");},isVisible(v){if(v){if(!this.ready){this.ready=true;}
else{this.onResize(true);}
if(this.onOpen){this.onOpen(this);}
this.$emit('open',this);}
else if(this.onClose){this.onClose(this);}},scrollReady(v){bbn.fn.log("Floater: SCROLL ready");if(v){let nb=this.mountedComponents.length;let to=null;let fn=()=>{if(to){clearTimeout(to);}
