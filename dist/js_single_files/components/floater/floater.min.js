...
return{width:'100%',height:'100%'}},hasContent(){return!!(this.content||this.component||this.filteredData.length||this.$slots.default);},isVisible(){return this.currentVisible&&this.hasContent;},isHorizontal(){if(!!this.position&&!this.position.startsWith('top')&&!this.position.startsWith('bottom')){return(this.position==='left')||(this.position==='right');}
...
this.$nextTick(()=>{let ancestors=this.ancestors('bbn-floater');if(this.element){let ct=ancestors.length?ancestors[ancestors.length-1]:this;let scroll=ct.closest('bbn-scroll');if(scroll){scroll.$once('scroll',()=>{this.close();});}}});},beforeDestroy(){if(this.onClose){this.onClose(this);}},updated(){},watch:{left(){this.updatePosition();},right(){this.updatePosition();},top(){this.updatePosition();},bottom(){this.updatePosition();},source:{deep:true,handler(){if(this.currentData.length){this.updateData();}}},visible(v){this.currentVisible=v;},isVisible(v){if(v){if(!this.ready){this.ready=true;}
else{this.onResize(true);}
if(this.onOpen){this.onOpen(this);}
this.$emit('open',this);}
else if(this.onClose){this.onClose(this);}},scrollReady(v){if(v){let nb=this.mountedComponents.length;let to=null;let fn=()=>{if(to){clearTimeout(to);}
