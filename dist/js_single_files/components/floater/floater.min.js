...
s.opacity=this.isResized?1:0;return s;},containerStyle(){return{width:'100%',height:'100%'}},hasContent(){return!!(this.content||this.component||this.filteredData.length||this.$slots.default);},isVisible(){return this.currentVisible&&this.hasContent;},isHorizontal(){if(!!this.position&&!this.position.startsWith('top')&&!this.position.startsWith('bottom')){return(this.position==='left')||(this.position==='right');}
...
let naturalWidth;let naturalHeight;if(!this.isResized){scroll.$el.style.width=this.formatSize(this.currentMaxWidth||'100%');scroll.$el.style.height=this.formatSize(this.currentMinHeight||'0px');let containerEle=scroll.getRef('scrollContainer');let contentEle=scroll.getRef('scrollContent');naturalHeight=containerEle.scrollHeight;if(!naturalHeight){this.isResizing=false;resolve();return;}
...
scroll.$el.style.width=null;scroll.$el.style.height=null;}
else{let contentEle=scroll.getRef('scrollContent');naturalWidth=contentEle.scrollWidth;naturalHeight=contentEle.scrollHeight;}
let dimensions={w:naturalWidth,h:naturalHeight};let scrollChange=false;if(this.scrollWidth!==dimensions.w){scrollChange=true;this.scrollWidth=dimensions.w;}
...
if(size!==this['real'+a.camel]){this['real'+a.camel]=size;}});if(ok&&(r.x.res!==null)&&(r.y.res!==null)){this.currentLeft=Math.ceil(r.x.res);this.currentTop=Math.ceil(r.y.res);}},scrollResize(e,dimensions){if(!this.scrollResized){this.scrollResized=true;}
e.preventDefault();if((dimensions.width!==this.scrollWidth)||(dimensions.height!==this.scrollHeight)){setTimeout(()=>{this.onResize(true);},50)}
else{}},addClose(fn){for(let i=0;i<arguments.length;i++){if(typeof arguments[i]==='function'){this.closingFunctions.push(arguments[i])}}},removeClose(fn){if(!fn){this.closingFunctions=[];}
...
if(this.forms.length&&!confirm){this.forms[0].closePopup();}
...
this.$emit('open',this);}
else if(this.onClose){this.onClose(this);}},scrollReady(v){if(v){setTimeout(()=>{this.realResize();},50);}},element(newVal){if(newVal&&this.ready){this.currentVisible=false;this.$forceUpdate();this.$nextTick(()=>{this.currentVisible=true;});}},isOver(v,oldV){if(this.autoHide&&this.isResized&&this.ready&&!this.isResizing){if(v&&this.mouseLeaveTimeout){clearTimeout(this.mouseLeaveTimeout);}
else if(!v){this.mouseLeaveTimeout=setTimeout(()=>{if(!this.isOver){this.close();}
this.mouseLeaveTimeout=false;},bbn.fn.isNumber(this.autoHide)?this.autoHide:1500);}}},hasButtons(){this.lastKnownCtWidth=0;this.lastKnownCtHeight=0;this.realResize();},filteredData:{deep:true,handler(){this.realResize()}},filteredTotal(){if(this.isVisible){this.onResize(true);}}}});})(window.Vue,window.bbn);})(bbn);