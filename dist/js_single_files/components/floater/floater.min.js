...
return new Promise((resolve)=>{if(go){if(this.definedWidth&&this.definedHeight){if((this.realWidth!==this.definedWidth)||(this.realHeight!==this.definedHeight)){this.currentWidth=this.definedWidth;this.realWidth=this.definedWidth;this.currentHeight=this.definedHeight;this.realHeight=this.definedHeight;this.updatePosition();resolve(1);return;}
resolve(0);return;}
else{if(this.resizerFn){clearTimeout(this.resizerFn);}
this.resizerFn=setTimeout(()=>{this.resizerFn=false;let scroll=this.getRef('scroll');bbn.fn.log(scroll);if(!scroll||!scroll.ready){resolve(0);return;}
scroll.getNaturalDimensions().then(()=>{let dimensions={w:scroll.naturalWidth,h:scroll.naturalHeight};let footer;if(this.footer){footer=this.getRef('footer');}
else if(this.currentButtons.length){footer=this.getRef('buttons');}
if(footer){dimensions.h+=footer.clientHeight||0;}
let scrollChange=false;if(this.scrollWidth!==dimensions.w){scrollChange=true;this.scrollWidth=dimensions.w;}
...
this.close(true);},select(item,idx,dataIndex){if(item&&!item.disabled&&!item[this.children]){let ev=new Event('select',{cancelable:true});if(this.onSelect){this.onSelect(item,idx,dataIndex,ev);}
else{this.$emit("select",item,idx,dataIndex,ev);}
