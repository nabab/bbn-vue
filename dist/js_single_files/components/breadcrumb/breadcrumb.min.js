...
</div>`;script.setAttribute('id','bbn-tpl-component-breadcrumb');script.setAttribute('type','text/x-template');document.body.insertAdjacentElement('beforeend',script);(function(bbn,Vue){"use strict";Vue.component("bbn-breadcrumb",{name:'bbn-breadcrumb',mixins:[bbn.vue.basicComponent,bbn.vue.resizerComponent,bbn.vue.localStorageComponent,bbn.vue.closeComponent,bbn.vue.observerComponent],props:{source:{type:Array,reuired:true},value:{},content:{type:Boolean,default:true},scrollable:{type:Boolean,default:false},maxTitleLength:{type:Number,default:20},menu:{type:Function},master:{type:Boolean,default:false}},data(){return{selected:false,iconsReady:false,parents:[],subBreadcrumbs:{},allSubBreadcrumbs:[],parent:false}},computed:{scrollCfg(){return this.scrollable?{axis:'x',container:true,hidden:true}:{};},router(){if(this.$parent&&(this.$parent.$options.name==='bbn-router')){return this.$parent;}
...
this.$nextTick(()=>{if(!this.master&&this.router){this.parent=this.router.parent.itsBreadcrumb;this.router.parent.itsBreadcrumb.register(this,this.router.baseURL.substr(0,this.router.baseURL.length-1));}})
this.ready=true;setTimeout(()=>{this.iconsReady=true;},1000);},beforeDestroy(){if(!this.master&&this.router&&this.router.parent&&this.router.parent.itsBreadcrumb){this.router.parent.itsBreadcrumb.unregister(this,this.router.baseURL.substr(0,this.router.baseURL.length-1));}},watch:{selected(newVal,oldVal){if(newVal!==oldVal){this.$emit('select',newVal);}},value(newVal,oldVal){if(newVal!==oldVal){this.selected=newVal}}},components:{listItem:{template:`
  ...
  <span class="bbn-flex-width bbn-vmiddle">
    ...
    >
      <i class="nf nf-fa-times"></i>
        ...
        `,props:{source:{type:Object,required:true}},methods:{close(){this.source.closeAction();this.closest('bbn-context').showFloater=false;}}}}});})(bbn,Vue);})(bbn);