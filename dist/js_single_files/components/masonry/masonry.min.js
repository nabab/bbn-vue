((bbn)=>{let script=document.createElement('script');script.innerHTML=`<div :class="[componentClass, {'bbn-ovaerlay': scrollable}]">
  <slot></slot>
  <div v-if="isLoading"
...
</div>`;script.setAttribute('id','bbn-tpl-component-masonry');script.setAttribute('type','text/x-template');document.body.insertAdjacentElement('beforeend',script);(bbn=>{"use strict";if(Vue.options.components['bbns-masonry']===undefined){Vue.component('bbns-masonry',{template:'<div class="bbns-masonry bbn-hidden"><slot></slot></div>'});}
