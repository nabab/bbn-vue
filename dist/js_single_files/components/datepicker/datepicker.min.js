...
this.oldInputValue=this.inputValue;this.updateCalendar();},clear(){this.setValue('');this.$nextTick(()=>{this.$set(this.getRef('element'),'inputValue','');})}},beforeCreate(){if(bbn.env&&bbn.env.lang&&(bbn.env.lang!==moment.locale())){moment.locale(bbn.env.lang);}},mounted(){this.ready=true;},watch:{min(){this.setValue(this.value||'');this.updateCalendar();},max(){this.setValue(this.value||'');this.updateCalendar();},valueFormat(){this.setValue(this.value||'');},maskedMounted(newVal){if(newVal){this.setInputValue(this.value);}},value(newVal){this.setInputValue(newVal);}}});})(bbn);};document.head.insertAdjacentElement("beforeend",script_dep);})(bbn);