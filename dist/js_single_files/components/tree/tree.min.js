...
return false;},move(node,target,force=false){let ev=new Event("move",{cancelable:!force});this.tree.$emit('move',node,target,ev);if(!ev.defaultPrevented){let idx=node.idx;let parent=node.parent;let nodes=node.findAll('bbn-tree-node');let expanded=nodes.filter(n=>!!n.isExpanded);let selected=nodes.filter(n=>!!n.isSelected);let toPath=[];if((idx>=0)&&parent){if(!target.numChildren){target.$set(target,'numChildren',1);}
...
if(this.tree.isAjax){let nodeSource=bbn.fn.extend(true,{},parent.currentData[idx]);nodeSource.num=targetTree.currentData.length||1;nodeSource.index=nodeSource.num-1;targetTree.currentData.push(nodeSource);}
...
else{this.isInit=true;}},watch:{activeNode(newVal){if(newVal&&this.isRoot){let scroll=this.getRef('scroll');if(scroll){}}},selectedNode(newVal){if(newVal&&this.isRoot){let scroll=this.getRef('scroll');if(scroll){}}},path(newVal){if(!this.isLoaded){if(this.isRoot||this.ready){this.openPath();}}
...
if(this.tree!==this.parent){this.parent.currentSelected.push(this);if(emit){this.parent.$emit('select',this);}}}}},removeFromSelected(emit=true,storage=true){let idx=this.tree.currentSelected.indexOf(this);let idx2=this.parent.currentSelected.indexOf(this);let ev=new Event('beforeUnselect',{cancelable:true});if(emit){this.tree.$emit('beforeUnselect',this,ev);}
