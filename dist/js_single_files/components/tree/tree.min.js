...
return o;});return items;},reset(){this.isLoaded=false;this.updateData();},resize(){let scroll=this.tree.getRef('scroll');if(scroll&&bbn.fn.isFunction(scroll.onResize)){scroll.onResize();}},onOpen(){this.resize();this.$emit('open');this.tree.$emit('open',this);},onClose(){this.resize();this.$emit('close');this.tree.$emit('close',this);},findNode(props,expand){let ret=false;if(this.isRoot||(this.node.numChildren&&bbn.fn.isObject(props))){if(expand&&!this.isRoot&&!this.node.isExpanded){this.node.isExpanded=true;}
...
else{bbn.fn.each(cp.$children,node=>{let tree=node.getRef('tree');if(tree){ret=tree.findNode(props,expand);if(ret){return false;}}})}}}
...
let children=target.source.data[this.tree.children];if(!children){target.source.data[this.tree.children]=[];children=target.source.data[this.tree.children]}
if(!target.isExpanded){targetTree.$once('dataloaded',()=>{nodeSource.num=targetTree.currentData.length||1;nodeSource.index=nodeSource.num-1;children.push(nodeSource);});}
else{nodeSource.num=targetTree.currentData.length||1;nodeSource.index=nodeSource.num-1;children.push(nodeSource);}}
...
return true;}
return false;},clickOnNode(ev){this.tree.$emit('nodeClick',this,ev);},dblClickOnNode(ev){this.tree.$emit('nodeDblclick',this,ev);},getIcon(){return this.source.icon||(!!this.numChildren?(this.isExpanded?'nf nf-fa-folder_open':'nf nf-fa-folder'):'nf nf-fa-file');},remove(){if(!this.parent.isAjax){this.parent.currentData.splice(this.idx,1);}},getCls(source,tree){return source.cls!==undefined?source.cls:(bbn.fn.isFunction(tree.cls)?tree.cls(source,this.tree,this.parent):tree.cls||'');}},created(){this.parent=this.closest('bbn-tree');this.tree=this.parent.tree||this.parent;if(this.source.selected){this.addToSelected();}
