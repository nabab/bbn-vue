...
else{node=!node?this.node:node;let tree=node.getRef('tree');if(tree){tree.isLoaded=false;return tree.updateData();}}},load(){this.updateData();},openPath(){return;this.$nextTick(()=>{if(this.path.length){let current=bbn.fn.extend(true,[],this.path);if(bbn.fn.isString(current[0])){current=[current];}
...
else if(!this.tree.isAjax){targetTree.updateData();if(targetTree!==parent){parent.updateData();}
callOpenPath=true;}
...
if(state){this._setCurrentState(state);}},expandPath(path,field,select=false){field=field||this.uid||false;if(field&&path&&path.length){if(!bbn.fn.isArray(path)){path=[path];}
...
else if(!isLast){tree.expandPath(currentPaths,field,select);}}}}})}}}},selectPath(path,field){this.expandPath(path,field,true);},init(){if(this.node.isExpanded||this.isRoot||bbn.fn.count(Object.values(this.currentState),{expanded:true})||bbn.fn.count(Object.values(this.currentState),{selected:true})){return this.updateData().then(()=>{this.isInit=true;if(bbn.fn.numProperties(this.currentState)&&this.filteredData.length){setTimeout(()=>{bbn.fn.iterate(this.currentState,(o,uid)=>{let it=this.uid!==undefined?this.findNode({[this.uid]:uid}):false;if(it){if((o.items&&bbn.fn.numProperties(o.items))||o.expanded){it.isExpanded=true;}
