...
</div>`;script.setAttribute('id','bbn-tpl-component-tabs');script.setAttribute('type','text/x-template');document.body.insertAdjacentElement('beforeend',script);(function(bbn,Vue){"use strict";Vue.component("bbn-tabs",{name:'bbn-tabs',mixins:[bbn.vue.basicComponent,bbn.vue.localStorageComponent,bbn.vue.closeComponent,bbn.vue.observerComponent,bbn.vue.resizerComponent],props:{source:{type:Array,reuired:true},value:{type:[Number,Boolean]},content:{type:Boolean,default:true},scrollable:{type:Boolean,default:false},maxTitleLength:{type:Number,default:20},menu:{type:Function}},data(){return{selected:null,iconsReady:false,}},computed:{scrollCfg(){return this.scrollable?{axis:'x',container:true,hidden:true}:{};},},methods:{numProperties:bbn.fn.numProperties,cutTitle(title){return bbn.fn.shorten(title,this.maxTitleLength)},getMenuFn(idx){return this.menu?this.menu(idx):[];},getFullTitle(obj){let t='';if(obj.noText||(obj.title.length>this.maxTitleLength)){t+=obj.title;}
...
setTimeout(()=>{this.iconsReady=true;},1000);},watch:{selected(newVal,oldVal){if((newVal!==oldVal)&&bbn.fn.isNumber(newVal)&&this.ready){this.$emit('select',newVal);}},value(newVal,oldVal){if((newVal!==oldVal)&&bbn.fn.isNumber(newVal)&&this.ready){this.selected=newVal}}},});})(bbn,Vue);})(bbn);