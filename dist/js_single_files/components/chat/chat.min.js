((bbn)=>{let script=document.createElement('script');script.innerHTML=`<div :class="[componentClass, 'bbn-vmiddle']">
...
`;script.setAttribute('id','bbn-tpl-component-chat');script.setAttribute('type','text/x-template');document.body.insertAdjacentElement('beforeend',script);((bbn,Vue)=>{"use strict";let cp=false;Vue.component('bbn-chat',{mixins:[bbn.vue.basicComponent,bbn.vue.localStorageComponent,bbn.vue.resizerComponent,bbn.vue.serviceWorkerComponent],props:{userId:{type:String,},users:{type:Array,default(){return[];}},groups:{type:Array,default(){return[];}},onlineUsers:{type:Array,default(){return[];},},online:{type:Boolean,default:true},url:{type:String}},data(){let isDark=false;if(appui&&appui.theme&&appui.themes){isDark=bbn.fn.getField(appui.themes,'isDark',{value:appui.theme});}
return{currentOnline:this.online,mainWindowVisible:false,lastChat:null,currentChats:[],chatsHash:null,onlineUsersHash:null,usersReceived:false,chatsReceived:false,bottomCoord:'',isDarkTheme:isDark}},computed:{usersOnlineWithoutMe(){return this.onlineUsers.filter((a)=>{return a!==this.userId;}).map((a)=>{return bbn.fn.isObject(a)?a:{value:a}})},visibleWindows(){return this.currentChats.filter(c=>c.visible);},mainMenu(){let res=[];if(this.currentOnline){res.push({icon:'nf nf-oct-organization',text:bbn._('New group chat'),action:()=>{this.getPopup().open({title:bbn._('New group chat'),component:this.$options.components.newGroup,width:300,height:400})}});}
...
if(chat.messages.length){for(let i=chat.messages.length-1;i>-1;i--){if((chat.messages[i].user!==this.userId)&&(chat.messages[i].unread!==undefined)){if(!chat.messages[i].unread){break;}
